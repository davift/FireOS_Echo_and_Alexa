...
/*
** Function floating() checks if the ascii-string contains
** a numeric (positive) floating point value.
** Returns 1 (true) if so, or 0 (false).
*/
int floating(char*ns)
{
	register char *s = ns;
	int onedecimalpoint = 0;

	while (*s)
		if (*s < '0' || *s > '9')
		{
			if (!onedecimalpoint && *s == '.')
			{
				onedecimalpoint++;
				s++;
				continue;
