...
#ifdef ANDROID
void endpwent() { }
struct passwd *getpwent() { return NULL; }
#endif
			...
			switch ( (lastchar = mvgetch(statline, 0)) )
			{
			   ...
			   */
			   case MSAMPNEXT:
				...
				/*
				 ** Signal handler for SIGALRM is not defined for rawread.
				 */
				if (!rawreadflag)
				    getalarm(0);
			   ...
			   */
			   case MCALCPSS:
				...
				{
					calcpss    = 0;
					statmsg    = "PSIZE/USIZE gathering disabled";
				}
				else
				{
					calcpss    = 1;
					statmsg    = "PSIZE/USIZE gathering enabled";
				}
				break;
...
accumulate(struct tstat *curproc, struct tstat *curstat)
{
	...
	if (curproc->gen.state != 'E')
	{
		...
		if (curstat->mem.umem != -1)
		{
			if  (curproc->mem.umem != -1)  // no errors?
				curstat->mem.umem += curproc->mem.umem;
			else
				curstat->mem.umem  = -1;
		}
...
static struct helptext {
	...
	{"\t'%c'  - show average-per-second i.s.o. total values    (toggle)\n",
								MAVGVAL},
	{"\t'%c'  - calculate proportional and unique set size     (toggle)\n",
								MCALCPSS},
	{"\n",							' '},
	{"Raw file viewing:\n",					' '},
