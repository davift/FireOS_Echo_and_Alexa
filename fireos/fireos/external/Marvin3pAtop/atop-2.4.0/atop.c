...
#define	allflags  "ab:cde:f:ghijklmnopqrst:uvwxyz1ABCDEFGHIJKL:MNOP:QRST:UVWXYZ"
#define	MAXFL		64      /* maximum number of command-line flags  */
#define	ONE_SECOND	1 	/* constant time duration for main loop */
#define	MAX_DEBOUNCE 	5	/* number of consecutive peaks to skip logging*/
#define	MAX_DUMP_TASK	10	/* maximum number of task info to dump to syslog */
...
/*
** Declaration of static function to dump taks info to syslog
*/
static void		dumptosyslog(time_t , int , struct devtstat *);
			...
			switch (c)
			{
			   ...
			   case 'S':		/* midnight limit ?           */
				...
				break;
                           ...
                           case 'l':		/* log thread info to syslog  */
				enablelogflag++;
				break;
                           case 'L':		/* line length                */
				if ( !numeric(optarg) )
				...
				break;
                           ...
                           case 'f':        /* apply cpu filter to recorded processes */
				if ( !floating(optarg) )
					prusage(argv[0]);
				procfilterflag++;
				/* only processes above given percent */
				cpuloadfilter = atof(optarg);
				break;

                           case 't':        /* apply nprocs filter to recorded processes */
				if ( !numeric(optarg) )
					prusage(argv[0]);
				procfilterflag++;
				/* Limit number of processes */
				ntopcpuprocs = atoi(optarg);
				break;

			    case 'T':		/* Load Threshold            */
				if ( !numeric(optarg) )
					prusage(argv[0]);

				loadthreshold = atoi(optarg);
				if (loadthreshold > 100)
					prusage(argv[0]);
...
engine(void)
{
	...
	*/
	...
	static struct sstat	*syscursstat; /* system current stats */
	static struct sstat	*syspresstat; /* system previous stats */
	static struct sstat	*sysdevsstat; /* system deviation stats */
	static struct sstat	*syshlpsstat;
	...
	*/
	...
	syscursstat = calloc(1, sizeof(struct sstat));
	syspresstat = calloc(1, sizeof(struct sstat));
	sysdevsstat = calloc(1, sizeof(struct sstat));
	...
	ptrverify(syscursstat, "Malloc failed for current sysstats\n");
	ptrverify(syspresstat, "Malloc failed for temp prev sysstats\n");
	ptrverify(sysdevsstat, "Malloc failed for temp deviate sysstats\n");
	...
	*/
	for (sampcnt=0; sampcnt < nsamples; sampcnt++)
	{
		char	lastcmd = 0;
		float	systotal = 0;
		float	cpuloadpercent = 0;
		char	cpuhighloadflag = 0;
		/*
		...
		*/
		...
		syshlpsstat = syscursstat;
		syscursstat = syspresstat;
		syspresstat = syshlpsstat;
		/* obtain only sys counters to check load */
		photosyst(syscursstat);

		deviatsyst(syscursstat, syspresstat, sysdevsstat, ONE_SECOND);

		/* Calculate total system load */
		systotal = sysdevsstat->cpu.all.itime + sysdevsstat->cpu.all.stime + \
			   sysdevsstat->cpu.all.utime + sysdevsstat->cpu.all.ntime + \
			   sysdevsstat->cpu.all.wtime + sysdevsstat->cpu.all.Itime + \
			   sysdevsstat->cpu.all.Stime + sysdevsstat->cpu.all.steal + \
			   sysdevsstat->cpu.all.guest;

		/* Calculate CPU load in percentage */
		cpuloadpercent = ((systotal - (sysdevsstat->cpu.all.itime + \
				 sysdevsstat->cpu.all.wtime)) / systotal ) * 100;

		cpuhighloadflag = (cpuloadpercent > loadthreshold);

	        if (cpuhighloadflag)
	        {
		       debouncecount > MAX_DEBOUNCE ? debouncecount = 0 : debouncecount++;
	        }
	        else
	        {
		       debouncecount = 0;
	        ...
	        /*
	        ** skip collecting the thread stats if this alarm wasn't on the desired interval
	        ** unless cpu load is high and we're eligible to capture a new sample
	        */
                if (!((sampcnt % interval == 0) || (cpuhighloadflag && debouncecount == 1)))
                       continue;
		/*
		...
		*/
		...
		if (procfilterflag)
		{
			int tickfilter = interval*cpuloadfilter;
			proclimit(&devtstat, tickfilter, ntopcpuprocs);
		}

		if (enablelogflag && cpuhighloadflag && (debouncecount==1))
		{
			/*
			**log the top consuming threads when system load
			**higher than the threshold
			*/
			dumptosyslog(curtime,
				     curtime-pretime > 0 ? curtime-pretime : 1,
				     &devtstat);
		}
		if ((sampcnt % interval) == 0)
		{
			/*
...
prusage(char *myname)
{
	...
	printf("\t  -L  alternate line length (default 80) in case of "
			"non-screen output\n");
	printf("\t  -T  Enable monitoring the system load [0-100%]\n");
	printf("\t  -l  Enable logging of thread and process info\n");
	...
	printf("\n");
	printf("\tData capture options:\n");
	printf("\t  -t X  Limit data capture to top X CPU consuming processes\n");
	printf("\t  -l X  Limit data capture to processes consuming >=X%% of a CPU\n");
	printf("\n");
...
{
	awaittrigger=0;
	alarm(ONE_SECOND);	/* restart the timer */
}
...
/*
** Dump process and thread data to syslog
*/
static void
dumptosyslog(time_t curtime, int numsecs, struct devtstat *devtstat)
{
	register int    i;
	char            datestr[32], timestr[32];
	struct tstat 	*ps = devtstat->taskall;

	convdate(curtime, datestr);
	convtime(curtime, timestr);

	qsort(ps, devtstat->ntaskall , sizeof(struct tstat), comptask);

	syslog(LOG_WARNING, "# CPU load high for %d seconds at %s %s#\n",
		numsecs, datestr, timestr);
	syslog(LOG_WARNING, "PID    Name            State  User   System   Nice Pri Core TGID");
	syslog(LOG_WARNING, "Process info\n");
	/*
	**logging process info
	*/
	for (i=0; i < MAX_DUMP_TASK && ps < devtstat->taskall + devtstat->ntaskall; ps++)
	{
            if (ps->gen.isproc)
		{
			syslog(LOG_WARNING,"%-6d %-20s %c %-8lld %-8lld %-3d %-3d %-2d %-6d\n",
				ps->gen.pid,
				ps->gen.name,
				ps->gen.state,
				ps->cpu.utime / numsecs,
				ps->cpu.stime / numsecs,
				ps->cpu.nice,
				ps->cpu.prio,
				ps->cpu.curcpu,
				ps->gen.tgid);
	...
	syslog(LOG_WARNING, "Thread info\n");
	/* reinitializing the process list pointer */
	ps = devtstat->taskall;
	/*
	**logging thread info
	*/
