...
photosyst(struct sstat *si)
{
	...
	*/
	...
	/*
	** Record the system mode of operation and store it in sys stats
	*/
	if ( (fp = fopen("/data/operation_mode.txt","r")) != NULL)
	{
		si->mode.mode[0] = '\0' ; /* clear previous string value */

		fgets(si->mode.mode, sizeof(si->mode.mode), fp);
