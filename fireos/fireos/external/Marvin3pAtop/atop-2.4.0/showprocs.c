...
/***************************************************************/
char *
procprt_USIZE_a(struct tstat *curstat, int avgval, int nsecs)
{
        static char buf[10];

        if (curstat->mem.umem == (unsigned long long)-1LL)      
                return "    ?K";

        val2memstr(curstat->mem.umem*1024, buf, KBFORMAT, 0, 0);
        return buf;
}

char *
procprt_USIZE_e(struct tstat *curstat, int avgval, int nsecs)
{
        return "    0K";
}

proc_printdef procprt_USIZE = 
   { " USIZE", "USIZE", procprt_USIZE_a, procprt_USIZE_e, 6 };
/***************************************************************/
char *
