         ...
         int nexit, unsigned int noverflow, char flag)
{
	...
	unsigned long		puncomplen = sizeof(struct tstat) *
						devtstat->ntaskall;
	unsigned long		pcomplen   = (unsigned long)
						(ceil(puncomplen * 1.001) + 12);
	...
	*/
	rv = compress2(scompbuf, &scomplen,
				(Byte *)sstat, (unsigned long)sizeof *sstat, Z_BEST_SPEED);
	...
	rv = compress2(pcompbuf, &pcomplen, (Byte *)devtstat->taskall,
						(unsigned long)puncomplen, Z_BEST_SPEED);
