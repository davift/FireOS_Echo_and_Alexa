	                                      ...
	                                      char newtask, count_t totusedcpu)
{
	...
	*/
	...
	devstat->mem.umem     = curstat->mem.umem;
							...
							long interval)
{
	...
	*/
	...
	/* Copy the current device mode to deviation data */
	strcpy(dev->mode.mode, pre->mode.mode);
...
/**
  * truncate deviation to top n CPU consuming processes using more than tickfilter
  */
 void proclimit(struct devtstat* dts, int tickfilter, int nprocs)
 {
	int i,j;
	int newntaskall;
	struct tstat* thistask;
	struct tstat* newtaskall;
	int pids[nprocs];

	if ( nprocs < 1 || nprocs > dts->nprocall) nprocs = dts->nprocall;

	// First, sort existing procall based on load
	qsort(dts->procall, dts->nprocall, sizeof(struct tstat*), comptaskptr);
	for (i=0; i<nprocs; i++)
	{
		thistask = dts->procall[i];
		// break out early once we're below tickfilter
		if ((thistask->cpu.stime + thistask->cpu.utime) < tickfilter)
			break;
		pids[i] = dts->procall[i]->gen.pid;
	}
	nprocs = i;

	// Allocate a new taskall list
	// We don't know how many tasks we will need to copy
	// so rather than scanning first, allocate enough for everything
	newntaskall = 0;
	newtaskall = malloc(dts->ntaskall * sizeof(struct tstat));
	ptrverify(newtaskall, "Malloc failed for %lu filtered tasks\n", dts->ntaskall);

	// for each task in deviation
	for (i=0; i<dts->ntaskall; i++)
	{
		thistask = dts->taskall+i;
		// check the list of top PIDs
		for (j=0; j<nprocs; j++)
		{
			int pid = pids[j];
			if (thistask->gen.tgid == pid) {
				// This task is in the list. add it to new task list
				memcpy(newtaskall+newntaskall, thistask, sizeof(struct tstat));
				newntaskall++;
				break;
			}
		}
	...
	dts->ntaskall = newntaskall;
	dts->taskall = newtaskall;

	// Then re-initialize procall and procactive
	dts->procall    = malloc(dts->nprocall * sizeof(struct tstat *));
	dts->procactive = malloc(dts->nprocactive * sizeof(struct tstat *));

	ptrverify(dts->procall, "Malloc failed for %d processes\n", dts->nprocall);
	ptrverify(dts->procactive, "Malloc failed for %d active procs\n", dts->nprocactive);

	{
		int pall=0, pact=0;

		for (i=0, thistask=dts->taskall; i < dts->ntaskall; i++, thistask++)
		{
			if (thistask->gen.isproc)
			{
				dts->procall[pall++] = thistask;
				if (! thistask->gen.wasinactive)
					dts->procactive[pact++] = thistask;
			}
		}

		dts->nprocall = pall;
		dts->nprocactive = pact;
	}
}
...
{
	count_t diff = newval - oldval;

	if (diff >= 0)
		return diff;

	count_t wrapped = MAX32BITVAL + diff;

	if (wrapped > (MAX32BITVAL / 2))
	{
		// Probably not a counter wrap, but a small reverse movement. Return the negative value
		return diff;
	}
