	...
	*/
	...
	if ( (fp = fopen(smapsfile, "r")) )
	{
		...
		while (fgets(line, sizeof line, fp))
		{
			...
			else if (memcmp(line, "Private_Clean:", 14) == 0)
			{
				// Private_Clean line found to be accumulated
				sscanf(line, "Private_Clean: %llu", &ussval);
				curtask->mem.umem += ussval;
			}
			else if (memcmp(line, "Private_Dirty:", 14) == 0)
			{
				// Private_Dirty line found to be accumulated
				sscanf(line, "Private_Dirty: %llu", &ussval);
				curtask->mem.umem += ussval;
			}
		}
		...
		*/
		if (ferror(fp))
		{
			...
			curtask->mem.umem = (unsigned long long)-1LL;
		}
		fclose(fp);
	}
	else
