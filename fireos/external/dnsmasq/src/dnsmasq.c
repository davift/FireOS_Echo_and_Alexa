...
// fosmod_wifi_bug_fix begin
      ...
      if (fstat(STDIN_FILENO, &sb)) {
        my_syslog(LOG_ERR, _("android listenser, STDIN fstat returns error: %d"), errno);
        stdin_is_fifo = 1;
      } else {
        stdin_is_fifo = sb.st_mode & S_IFIFO;
      }
#endif
// fosmod_wifi_bug_fix end
...
// fosmod_wifi_bug_fix begin
#ifdef __ANDROID__
  my_syslog(LOG_INFO, _("Android listenser %s"), stdin_is_fifo? "Enabled" : "Disabled");
#endif
// fosmod_wifi_bug_fix end
