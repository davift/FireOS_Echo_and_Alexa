...
static int max98396_init_setup(struct snd_soc_component *component)
{
	...
	regmap_update_bits(max98396->regmap,
		...
		(1 << MAX98396_DSP_SPK_WBAND_FLT_EN_SHIFT) |
		(1 << MAX98396_DSP_SPK_SAFE_EN_SHIFT) |
		(1 << MAX98396_DSP_SPK_DITH_EN_SHIFT) |
		(1 << MAX98396_DSP_SPK_DCBLK_EN_SHIFT),
		(1 << MAX98396_DSP_SPK_WBAND_FLT_EN_SHIFT) | 1);

	/* Enable Wideband Filter on V sense */
	/* Enable DC blocker and Dither */
	regmap_update_bits(max98396->regmap,
			   MAX98396_R20E0_IV_SENSE_PATH_CFG,
			   (1 << MAX98396_IV_V_DCBLK_EN_SHIFT) |
			   (1 << MAX98396_IV_I_DCBLK_EN_SHIFT) |
			   (1 << MAX98396_IV_SENSE_DITH_EN_SHIFT) |
			   (1 << MAX98396_IV_SENSE_WBAND_FLT_EN_SHIFT),
			   (1 << MAX98396_IV_V_DCBLK_EN_SHIFT) |
			   (1 << MAX98396_IV_I_DCBLK_EN_SHIFT) |
			   (1 << MAX98396_IV_SENSE_DITH_EN_SHIFT) |
			   (1 << MAX98396_IV_SENSE_WBAND_FLT_EN_SHIFT));
	...
	/* Disable dmon*/
	regmap_write(max98396->regmap,
		MAX98396_R203F_ENABLE_CTRLS, 0x1);
...
static void max98396_enable_amp(struct max98396_priv *max98396, int enable)
{
	regmap_update_bits(max98396->regmap,
		MAX98396_R205E_PCM_RX_EN,
		MAX98396_PCM_RX_EN_MASK, enable);

	regmap_write(max98396->regmap,
		MAX98396_R210F_GLOBAL_EN, enable);

	regmap_write(max98396->regmap,
		MAX98396_R20AF_AMP_EN, enable);
}

static int max98396_force_unmute_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
	struct max98396_priv *max98396 = snd_soc_component_get_drvdata(component);

	mutex_lock(&max98396->lock);
	ucontrol->value.integer.value[0] = max98396->force_unmute;
	mutex_unlock(&max98396->lock);

	return 0;
}

static int max98396_force_unmute_set(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
	struct max98396_priv *max98396 = snd_soc_component_get_drvdata(component);
	...
	if (ucontrol->value.integer.value[0]) {
		max98396->force_unmute = true;
		if (!max98396->prev_amp_enable) {
			max98396_enable_amp(max98396, 1);
		}
	} else {
		max98396->force_unmute = false;
		if (!max98396->prev_amp_enable) {
			max98396_enable_amp(max98396, 0);
		}
	}
	mutex_unlock(&max98396->lock);
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		mutex_lock(&max98396->lock);
		max98396->prev_amp_enable = true;
		max98396_enable_amp(max98396, 1);
		mutex_unlock(&max98396->lock);
		break;
	case SND_SOC_DAPM_POST_PMD:
		mutex_lock(&max98396->lock);
		max98396->prev_amp_enable = false;
		if (!max98396->force_unmute) {
			max98396_enable_amp(max98396, 0);
		}
		max98396->tdm_mode = false;
		mutex_unlock(&max98396->lock);
...
static int max98396_input_voltage_set(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol) {
	return 0;
}

static int max98396_manual_mode(struct max98396_priv *max98396) {
	/* Set thermal readback to manual mode before reading the voltage */
	regmap_write(max98396->regmap,
		MAX98396_R20B4_ADC_READBACK_CTRL1, 0x05);

	/* Inititates a measurement and locks the result*/
	regmap_write(max98396->regmap,
		MAX98396_R20B5_ADC_READBACK_CTRL2, 0x05);
	return 0;
}
static int max98396_input_voltage_info(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_info *uinfo) {
	uinfo->type   = SNDRV_CTL_ELEM_TYPE_INTEGER;
	uinfo->count  = 1;
	uinfo->value.integer.min  = 0;
	uinfo->value.integer.max  = 511;
	uinfo->value.integer.step = 1;
	return 0;
}
static int max98396_input_voltage_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol) {
	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
	struct max98396_priv *max98396 = snd_soc_component_get_drvdata(component);
	int ret;
	...
	if (!max98396->prev_amp_enable) {
		dev_info(component->dev, "%s temporarily enable DAC for voltage read\n", __func__);
		max98396_enable_amp(max98396, 1);
		usleep_range(3000,6000);
		amp_enabled = true;
	}

	max98396_manual_mode(max98396);

	ret = regmap_read(max98396->regmap,
		MAX98396_R20B6_ADC_PVDD_READBACK_MSB, &msb);
	if (ret < 0) {
		dev_err(component->dev, "%s Failed to read: 0x%02X \n",
				__func__, MAX98396_R20B6_ADC_PVDD_READBACK_MSB);
		goto end;
	}

	ret = regmap_read(max98396->regmap,
		MAX98396_R20B7_ADC_PVDD_READBACK_LSB, &lsb);
	if (ret < 0) {
		dev_err(component->dev, "%s Failed to read: 0x%02X\n",
				__func__, MAX98396_R20B7_ADC_PVDD_READBACK_LSB);
		goto end;
	}

	ret = 0;
	ucontrol->value.integer.value[0] = ((msb << 1) + lsb);
	dev_dbg(component->dev, ":%s- voltage %d\n", __func__,
...
/* Irq PIN mode */
SOC_SINGLE("IRQ CTRL", MAX98396_R201F_IRQ_CTRL, 0, 0x07, 0),
/* Interrupt enable control */
SOC_SINGLE("INT EN1", MAX98396_R2010_INT_EN1, 0, 0xff, 0),
SOC_SINGLE("INT EN2", MAX98396_R2011_INT_EN2, 0, 0xff, 0),
SOC_SINGLE("INT EN3", MAX98396_R2012_INT_EN3, 0, 0x3f, 0),
SOC_SINGLE("INT EN4", MAX98396_R2013_INT_EN4, 0, 0x0f, 0),

/* Thermal Threshold */
SOC_SINGLE("THERM WARN THRESH", MAX98396_R2020_THERM_WARN_THRESH, 0, 0x7f, 0),
SOC_SINGLE("THERM WARN THRESH2", MAX98396_R2021_THERM_WARN_THRESH2, 0, 0x7f, 0),
SOC_SINGLE("THERM SHDN THRESH", MAX98396_R2022_THERM_SHDN_THRESH, 0, 0x7f, 0),

/* Force unmute */
SOC_SINGLE_BOOL_EXT("MAX98396 Force Unmute A", 0, max98396_force_unmute_get,
					max98396_force_unmute_set),
{
	.iface	= SNDRV_CTL_ELEM_IFACE_MIXER,
	.name	= "MAX98396 Get Voltage A",
	.info	= max98396_input_voltage_info,
	.get	= max98396_input_voltage_get,
	.put	= max98396_input_voltage_set,
},
...
/* Force unmute */
SOC_SINGLE_BOOL_EXT("MAX98396 Force Unmute B", 0, max98396_force_unmute_get,
					max98396_force_unmute_set),
{
	.iface	= SNDRV_CTL_ELEM_IFACE_MIXER,
	.name	= "MAX98396 Get Voltage B",
	.info	= max98396_input_voltage_info,
	.get	= max98396_input_voltage_get,
	.put	= max98396_input_voltage_set,
},
...
/* Force unmute */
SOC_SINGLE_BOOL_EXT("MAX98396 Force Unmute C", 0, max98396_force_unmute_get,
					max98396_force_unmute_set),
{
	.iface	= SNDRV_CTL_ELEM_IFACE_MIXER,
	.name	= "MAX98396 Get Voltage C",
	.info	= max98396_input_voltage_info,
	.get	= max98396_input_voltage_get,
	.put	= max98396_input_voltage_set,
},
...
unsigned int max98396_get_irq_state(struct snd_soc_component *component)
{
	int ret;
	...
	for (i = 0; i < 4; i++) {
		ret = regmap_read(max98396->regmap,
				  MAX98396_R2006_INT_STATE1 + i, &temp);
		if (ret < 0) {
			dev_err(component->dev,
				"Failed to read: 0x%02X\n", MAX98396_R2006_INT_STATE1 + i);
			temp = 0;
		}

		state = (state << 8) + (temp & 0xff);

		/* clear the flag after read */
		ret = regmap_write(max98396->regmap,
				   MAX98396_R2015_INT_FLAG_CLR1 + i, 0xff);
		if (ret < 0) {
			dev_err(component->dev,
				"Failed to write: 0x%02X\n", MAX98396_R2015_INT_FLAG_CLR1 + i);
		}
	}

	return state;
}
EXPORT_SYMBOL(max98396_get_irq_state);

int max98396_reset_platform(struct snd_soc_component *component, int enable)
{
	struct max98396_priv *max98396 = snd_soc_component_get_drvdata(component);
	// TODO add lock
...
static int max98396_suspend(struct device *dev)
{
	...
	dev_info(dev, "%s: dac %u init_done %u disable suspend %d", __func__,
		max98396->dac_num, max98396->init_done, max98396->disable_suspend);

	if (max98396->disable_suspend)
		return 0;
...
static int max98396_temp_sensor_read_temp_tz(void *data, int *temp)
{
	...
	if (!max98396->init_done) {
		dev_err(dev, "%s DAC is not initialized yet\n", __func__);
		return -EAGAIN;
	}

	mutex_lock(&max98396->lock);
	if (!max98396->prev_amp_enable) {
		dev_dbg(dev, "%s temporarily enable DAC for temperature read\n", __func__);
		max98396_enable_amp(max98396, 1);
		usleep_range(3000,6000);
		amp_enabled = true;
	}
	...
	max98396_manual_mode(max98396);
	ret = regmap_read(max98396->regmap,
		MAX98396_R20BA_TEMP_READBACK_MSB, &msb);
	if (ret < 0) {
		dev_err(dev,
			"%s Failed to read: 0x%02X\n", __func__, MAX98396_R20BA_TEMP_READBACK_MSB);
		if (amp_enabled)
			max98396_enable_amp(max98396, 0);
		mutex_unlock(&max98396->lock);
		return -EAGAIN;
	...
	ret = regmap_read(max98396->regmap,
		MAX98396_R20BB_TEMP_READBACK_LSB, &lsb);
	if (ret < 0) {
		dev_err(dev,
			"%s Failed to read: 0x%02X\n", __func__, MAX98396_R20BB_TEMP_READBACK_LSB);
		if (amp_enabled)
			max98396_enable_amp(max98396, 0);
		mutex_unlock(&max98396->lock);
		return -EAGAIN;
...
static struct thermal_zone_of_device_ops max98396_temp_sensor_tz_ops = {
	.get_temp = max98396_temp_sensor_read_temp_tz,
};
...
static const struct snd_soc_component_driver max98396_component_a = {
	.controls		= max98396_snd_controls_a,
	.num_controls		= ARRAY_SIZE(max98396_snd_controls_a),
	.dapm_routes		= max98396_audio_map_a,
	.num_dapm_routes	= ARRAY_SIZE(max98396_audio_map_a),
	.dapm_widgets		= max98396_dapm_widgets_a,
	.num_dapm_widgets	= ARRAY_SIZE(max98396_dapm_widgets_a),
	.idle_bias_on		= 1,
	.use_pmdown_time	= 1,
	.endianness		= 1,
...
static const struct snd_soc_component_driver max98396_component_b = {
	.controls		= max98396_snd_controls_b,
	.num_controls		= ARRAY_SIZE(max98396_snd_controls_b),
	.dapm_routes		= max98396_audio_map_b,
	.num_dapm_routes	= ARRAY_SIZE(max98396_audio_map_b),
	.dapm_widgets		= max98396_dapm_widgets_b,
	.num_dapm_widgets	= ARRAY_SIZE(max98396_dapm_widgets_b),
	.idle_bias_on		= 1,
	.use_pmdown_time	= 1,
	.endianness		= 1,
	.non_legacy_dai_naming	= 1,
...
static const struct snd_soc_component_driver max98396_component_c = {
	.controls		= max98396_snd_controls_c,
	.num_controls		= ARRAY_SIZE(max98396_snd_controls_c),
	.dapm_routes		= max98396_audio_map_c,
	.num_dapm_routes	= ARRAY_SIZE(max98396_audio_map_c),
	.dapm_widgets		= max98396_dapm_widgets_c,
	.num_dapm_widgets	= ARRAY_SIZE(max98396_dapm_widgets_c),
	.idle_bias_on		= 1,
	.use_pmdown_time	= 1,
	.endianness		= 1,
	.non_legacy_dai_naming	= 1,
	...
	struct max98396_priv *max98396)
{
	...
	if (device_property_read_bool(dev, "disable-suspend")) {
		max98396->disable_suspend = true;
	} else {
		max98396->disable_suspend = false;
	}
	dev_info(dev, "%s: suspend disabled %d\n", __func__, max98396->disable_suspend);
	...
	const struct i2c_device_id *id)
{
	...
	max98396->force_unmute = false;
	max98396->prev_amp_enable = false;
	mutex_init(&max98396->lock);

	tzd = thermal_zone_of_sensor_register(&i2c->dev, 0, i2c,
						&max98396_temp_sensor_tz_ops);
	if (IS_ERR(tzd)) {
		ret = PTR_ERR(tzd);
		dev_err(&i2c->dev,
			"%s: Failed to register the thermal sensor %d \n", __func__, ret);
        }
