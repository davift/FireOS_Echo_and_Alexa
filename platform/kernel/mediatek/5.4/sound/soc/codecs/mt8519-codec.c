...
/*
 * mt8519-codec.c  --  Mediatek 8519 ALSA SoC Codec driver
 *
...
#include <sound/soc.h>
#include <sound/tlv.h>
#include "mt8519-codec.h"
#include <linux/iio/consumer.h>

#define MT8519_CODEC_NAME "mt8519-codec"
#define CODEC_MT8519_NAME "mtk-codec-mt8519"
#define COMMON_CLOCK_FRAMEWORK_API
#define M_ANALOG4 153
...
{
	struct mt8519_codec_priv *mt8519_codec =
			(struct mt8519_codec_priv *) context;
	int ret = 0;

	if (!(mt8519_codec && mt8519_codec->regmap_modules[id]))
		return -EIO;

	ret = regmap_read(mt8519_codec->regmap_modules[id],
			(reg & (~offset)), val);
	...
	enum regmap_module_id id, unsigned int offset)
{
	...
	ret = regmap_write(mt8519_codec->regmap_modules[id],
			(reg & (~offset)), val);

	return ret;
}

struct mt8519_codec_module_info {
	int id;
	unsigned int offset;
};

static const struct mt8519_codec_module_info mt8519_codec_modules[] = {
	{ .id = REGMAP_AFE, .offset = AFE_OFFSET },
	{ .id = REGMAP_APMIXEDSYS, .offset = APMIXED_OFFSET },
};

static int mt8519_codec_found_module_id(unsigned int reg)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(mt8519_codec_modules); i++)
		if (reg & mt8519_codec_modules[i].offset)
			return mt8519_codec_modules[i].id;

	return -EINVAL;
}

static unsigned int mt8519_codec_found_module_offset(unsigned int reg)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(mt8519_codec_modules); i++)
		if (reg & mt8519_codec_modules[i].offset)
			return mt8519_codec_modules[i].offset;

	return 0;
}

static int mt8519_codec_reg_read(void *context,
	unsigned int reg, unsigned int *val)
{
	int id;
	unsigned int offset;

	id = mt8519_codec_found_module_id(reg);
	offset = mt8519_codec_found_module_offset(reg);
...
static int mt8519_codec_reg_write(void *context,
		unsigned int reg, unsigned int val)
{
	int id;
	unsigned int offset;
...
static void mt8519_codec_regmap_lock(void *lock_arg)
{
}

static void mt8519_codec_regmap_unlock(void *lock_arg)
{
}

static const struct regmap_config mt8519_codec_regmap = {
	.reg_bits = 32,
	.val_bits = 32,
	.reg_read = mt8519_codec_reg_read,
	.reg_write = mt8519_codec_reg_write,
	.lock = mt8519_codec_regmap_lock,
	.unlock = mt8519_codec_regmap_unlock,
...
static const struct soc_enum mt8519_codec_pga_gain_enums[] = {
...
static int mt8519_codec_get_gain_enum_id(const char *name)
{
...
static int mt8519_codec_pga_gain_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_kcontrol_chip(kcontrol);
	struct mt8519_codec_priv *codec_data =
			snd_soc_component_get_drvdata(component);
	int id = mt8519_codec_get_gain_enum_id(kcontrol->id.name);
	uint32_t value = 0;
...
static int mt8519_codec_pga_gain_put(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_kcontrol_chip(kcontrol);
	struct mt8519_codec_priv *codec_data =
...
static int mt8519_codec_uplink_sgen_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_kcontrol_chip(kcontrol);
	struct mt8519_codec_priv *codec_data =
...
static int mt8519_codec_uplink_sgen_put(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_kcontrol_chip(kcontrol);
	struct mt8519_codec_priv *codec_data =
...
static int mt8519_codec_downlink_vol_mode_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_kcontrol_chip(kcontrol);
	struct mt8519_codec_priv *codec_data =
...
static int mt8519_codec_downlink_vol_mode_put(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_kcontrol_chip(kcontrol);
	struct mt8519_codec_priv *codec_data =
...
static int mt8519_codec_downlink_vol_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_kcontrol_chip(kcontrol);
	struct mt8519_codec_priv *codec_data =
...
static int mt8519_codec_downlink_vol_put(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_kcontrol_chip(kcontrol);
	struct mt8519_codec_priv *codec_data =
	...
	dev_info(codec_data->cmpnt->dev, "%s, value1 %u, value2 %u\n",
	...
	if (value1 == DIGITAL_GAIN) { /*digtial gain*/
		...
		dev_info(codec_data->cmpnt->dev, "%s no this case!\n",
				__func__);
	}
...
static int mt8519_codec_uplink_vol_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_kcontrol_chip(kcontrol);
	struct mt8519_codec_priv *codec_data =
...
static int mt8519_codec_uplink_vol_put(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_kcontrol_chip(kcontrol);
	struct mt8519_codec_priv *codec_data =
...
static int mt8519_codec_downlink_loopback_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_kcontrol_chip(kcontrol);
	struct mt8519_codec_priv *codec_data =
...
static int mt8519_codec_downlink_loopback_put(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_kcontrol_chip(kcontrol);
	struct mt8519_codec_priv *codec_data =
...
static const struct snd_kcontrol_new mt8519_codec_snd_controls[] = {
	...
	SOC_ENUM_EXT("Audio_PGA1_Setting",
		mt8519_codec_pga_gain_enums[UL_L_PGA_GAIN],
		mt8519_codec_pga_gain_get,
		mt8519_codec_pga_gain_put),
	/* Audio_PGA2_Setting */
	SOC_ENUM_EXT("Audio_PGA2_Setting",
		mt8519_codec_pga_gain_enums[UL_R_PGA_GAIN],
		mt8519_codec_pga_gain_get,
		mt8519_codec_pga_gain_put),
	/* for uplink debug */
	SOC_SINGLE_EXT("Audio Uplink SGEN",
		SND_SOC_NOPM, 0, 1, 0,
		mt8519_codec_uplink_sgen_get,
		mt8519_codec_uplink_sgen_put),
	/* for downlink debug */
	SOC_SINGLE_EXT("Audio Downlink Loopback",
		SND_SOC_NOPM, 0, 1, 0,
		mt8519_codec_downlink_loopback_get,
		mt8519_codec_downlink_loopback_put),
	/* Audio_DAC_AMP Vol mode */
	SOC_SINGLE_EXT("Audio Downlink Vol Mode",
		SND_SOC_NOPM, 0, 1, 0,
		mt8519_codec_downlink_vol_mode_get,
		mt8519_codec_downlink_vol_mode_put),
	/* Audio_DAC_AMP Vol */
	SOC_DOUBLE_EXT("Audio Downlink Vol",
		SND_SOC_NOPM, 0, 1, 32, 0,
		mt8519_codec_downlink_vol_get,
		mt8519_codec_downlink_vol_put),
	/* Audio_ADC Vol :655360:20db*/
	SOC_DOUBLE_EXT("Audio Uplink Vol",
		SND_SOC_NOPM, 0, 1, 655360, 0,
		mt8519_codec_uplink_vol_get,
		mt8519_codec_uplink_vol_put),
};

static int mt8519_codec_enable_adda_afe_on(
	struct snd_soc_component *component)
{
	...
	spin_lock_irqsave(&mt8519_codec->adda_afe_on_lock, flags);

	mt8519_codec->adda_afe_on_ref_cnt++;
	if (mt8519_codec->adda_afe_on_ref_cnt == 1)
		snd_soc_component_update_bits(component,
			AFE_ADDA_UL_DL_CON0, 0x1, 0x1);

	spin_unlock_irqrestore(&mt8519_codec->adda_afe_on_lock, flags);

	return 0;
}

static int mt8519_codec_disable_adda_afe_on(
	struct snd_soc_component *component)
{
	struct mt8519_codec_priv *mt8519_codec =
		snd_soc_component_get_drvdata(component);
	...
	mt8519_codec->adda_afe_on_ref_cnt--;
	if (mt8519_codec->adda_afe_on_ref_cnt == 0)
		snd_soc_component_update_bits(component,
			AFE_ADDA_UL_DL_CON0, 0x1, 0x0);
	else if (mt8519_codec->adda_afe_on_ref_cnt < 0)
		mt8519_codec->adda_afe_on_ref_cnt = 0;
...
static int mt8519_codec_enable_26m_clk(
	struct snd_soc_component *component)
{
	struct mt8519_codec_priv *mt8519_codec =
		snd_soc_component_get_drvdata(component);
	...
	spin_lock_irqsave(&mt8519_codec->adda_26mclk_lock, flags);

	mt8519_codec->adda_26mclk_on_ref_cnt++;
	if (mt8519_codec->adda_26mclk_on_ref_cnt == 1) {
		snd_soc_component_update_bits(component,
			AUDUL_CON20,
	...
	spin_unlock_irqrestore(&mt8519_codec->adda_26mclk_lock, flags);

	return 0;
}

static int mt8519_codec_disable_26m_clk(
	struct snd_soc_component *component)
{
	struct mt8519_codec_priv *mt8519_codec =
		snd_soc_component_get_drvdata(component);
	...
	mt8519_codec->adda_26mclk_on_ref_cnt--;
	if (mt8519_codec->adda_26mclk_on_ref_cnt == 0) {
		snd_soc_component_update_bits(component,
			AUDUL_CON20,
			AUDUL_CON20_CLK_EN_MASK,
			0x0);
		snd_soc_component_update_bits(component,
			...
			AUDUL_CON20_CLK_SEL_MASK,
			0x0);
	} else if (mt8519_codec->adda_26mclk_on_ref_cnt < 0)
		mt8519_codec->adda_26mclk_on_ref_cnt = 0;
...
static bool get_devinfo_enable(struct snd_soc_component *component)
{
	u32 value;

	value = get_devinfo_with_index(M_ANALOG4);
	dev_info(component->dev, "%s  value 0x%x\n", __func__, value);

	/* if bit1 is 1, has downloaded efuse val */
	if ((value >> 1) & 0x1)
		return true;
	else
		return false;
...
static unsigned int get_bg_trim_val_from_devinfo(void)
{
	u32 value;

	/* bit7~bit14 is bg trim val */
	value = get_devinfo_with_index(M_ANALOG4);
	return (value >> 7) & (0xff);
}

static unsigned int get_ldo1p8_vosel_val_from_devinfo(void)
{
	u32 value;

	/* bit3~bit6 is ldol18 vosel val */
	value = get_devinfo_with_index(M_ANALOG4);
	return (value >> 3) & (0xf);
}

static int mt8519_codec_dac_depop_setup(
	struct snd_soc_component *component)
{
	unsigned int bg_trim_val = 0x1F;
	unsigned int ldo1p8_vosel_val = 0x8;

	if (get_devinfo_enable(component)) {
		bg_trim_val = get_bg_trim_val_from_devinfo();
		ldo1p8_vosel_val = get_ldo1p8_vosel_val_from_devinfo();
	}
	dev_info(component->dev, "%s (%d)-(0x1f-0x%x)-(0x8-0x%x)\n",
		__func__, get_devinfo_enable(component), bg_trim_val, ldo1p8_vosel_val);
	...
	snd_soc_component_update_bits(component,
			AUDUL_CON26,
			AUDUL_CON26_BGR_CORE_EN_MASK,
			AUDUL_CON26_BGR_CORE_EN_MASK);
	snd_soc_component_update_bits(component,
			AUDUL_CON26,
			AUDUL_CON26_BGR_VBUFFER_EN_MASK,
			AUDUL_CON26_BGR_VBUFFER_EN_MASK);
	/* change val by sw setting */
	snd_soc_component_update_bits(component,
			AUDUL_CON27,
			AUDUL_CON27_EFUSE_MUX_MASK,
			AUDUL_CON27_EFUSE_MUX_VAL(0x7));
	snd_soc_component_update_bits(component,
			AUDUL_CON26,
			AUDUL_CON26_BG_TRIM_MASK,
			AUDUL_CON26_BG_TRIM_VAL(bg_trim_val));
	usleep_range(10, 11);
	...
	snd_soc_component_update_bits(component,
		       AUDUL_CON27,
		       AUDDL_CON27_LDO_VOLSEL_ADAC_MASK,
		       AUDDL_CON27_LDO_VOLSEL_ADAC_VAL(ldo1p8_vosel_val));
	snd_soc_component_update_bits(component,
			AUDDL_CON00,
	...
	snd_soc_component_update_bits(component,
			AUDUL_CON27,
			AUDDL_CON27_I2VL_IB_SEL_MASK,
			AUDDL_CON27_I2VL_IB_SEL_VAL(0x1));
	snd_soc_component_update_bits(component,
			AUDUL_CON26,
			AUDDL_CON26_I2VR_IB_SEL_MASK,
			AUDDL_CON26_I2VR_IB_SEL_VAL(0x1));
	usleep_range(10, 11);
	...
	snd_soc_component_update_bits(component,
			AUDUL_CON27,
			AUDUL_CON27_CLK_SOURCE_SEL_MASK,
			AUDUL_CON27_CLK_SOURCE_SEL_MASK);
	snd_soc_component_update_bits(component,
			AUDDL_CON00,
	...
	snd_soc_component_update_bits(component,
			AUDDL_CON18,
			AUDDL_CON18_DEPOP_RAMPGEN_ISEL_MASK,
			AUDDL_CON18_DEPOP_RAMPGEN_ISEL_VAL(0x1));
	snd_soc_component_update_bits(component,
			AUDDL_CON18,
	...
	snd_soc_component_update_bits(component,
			AUDDL_CON02,
			AUDDL_CON02_REV_ADAC2_MASK,
			AUDDL_CON02_REV_ADAC2_VAL(0x2));
	usleep_range(10, 11);
	...
	snd_soc_component_update_bits(component,
			AUDDL_CON02,
			AUDDL_CON02_REV_ADAC2_MASK,
			AUDDL_CON02_REV_ADAC2_VAL(0x0));
	usleep_range(2000000, 2001000);

	return 0;
}

static int mt8519_codec_adc_setup(
	struct snd_soc_component *component)
{
	/*8519 must disable loopback(DAC to ADC)*/
...
static int mt8519_codec_get_channel_voltage(
	struct snd_soc_component *component,
...
static int mt8519_codec_adc_rc_calibration_val(
	struct snd_soc_component *component, struct iio_channel *ch, int chinfo)
{
	...
	for (i = 0; i < ADC_RC_TRIM_VALUE_MAX; i++) {
		...
		spin_lock_irqsave(&mt8519_codec->adda_rc_calibration_lock,
				flags);
		...
		//				&reg_val);
		//} while (reg_val != 0x2);
		spin_unlock_irqrestore(&mt8519_codec->adda_rc_calibration_lock,
					flags);

		mt8519_codec_get_channel_voltage(component, ch, &Volt_ch[i]);
...
static int mt8519_codec_adc_rc_calibration_setup(
	struct snd_soc_component *component)
{
		/*LDO*/
	snd_soc_component_update_bits(component,
	...
	snd_soc_component_update_bits(component,
			...
			AUDUL_CON09_AUDUL_CHOPPER_CLK_EN_MASK);
	mt8519_codec_enable_26m_clk(component);
...
static int mt8519_codec_adc_rc_calibration_clean(
	struct snd_soc_component *component)
{
	snd_soc_component_update_bits(component,
			AUDUL_CON20,
	...
	snd_soc_component_update_bits(component,
			...
			AUDUL_CON09_AUDUL_CHOPPER_CLK_EN_MASK,
			0x0);
	mt8519_codec_disable_26m_clk(component);
...
/*L&R ch --> ch5*/
static int mt8519_codec_adc_rc_calibration(
	struct snd_soc_component *component)
{
	...
	if (!codec_data->ul_no_rc_cali) {
		...
		mt8519_codec_adc_rc_calibration_setup(component);

		snd_soc_component_update_bits(component,
			AUDUL_CON21,
			AUDUL_CON21_C_CALB_PATH_SEL,
			AUXADC_CH_0); /*0:Lch,1:Rch*/
		trim_bestL = mt8519_codec_adc_rc_calibration_val(component,
		...
		trim_bestR = mt8519_codec_adc_rc_calibration_val(component,
		...
		mt8519_codec_adc_rc_calibration_clean(component);
...
static int mt8519_codec_ana_dac_clk_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_component *component =
		snd_soc_dapm_to_component(w->dapm);
	unsigned int low_i2v_ib = 1;
	unsigned int high_i2v_ib = 9;
	unsigned int i;

	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		/* step by step to increase i2v ib from 2 to 9, default setting is 1 */
		for (i = low_i2v_ib; i < high_i2v_ib; i++) {
			snd_soc_component_update_bits(component,
					AUDUL_CON27,
					AUDDL_CON27_I2VL_IB_SEL_MASK,
					AUDDL_CON27_I2VL_IB_SEL_VAL(i+1));
			snd_soc_component_update_bits(component,
					AUDUL_CON26,
					AUDDL_CON26_I2VR_IB_SEL_MASK,
					AUDDL_CON26_I2VR_IB_SEL_VAL(i+1));
			usleep_range(10, 11);
		}
		snd_soc_component_update_bits(component,
				AUDDL_CON18,
		...
		snd_soc_component_update_bits(component,
				AUDDL_CON18,
				AUDDL_CON18_VCMBUF_EN_MASK,
				AUDDL_CON18_VCMBUF_EN_MASK);
		snd_soc_component_update_bits(component,
				AUDDL_CON18,
				AUDDL_CON18_VCMB_SEL_MASK,
				AUDDL_CON18_VCMB_SEL_MASK);
		usleep_range(10, 11);
		snd_soc_component_update_bits(component,
			AUDDL_CON00,
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		/* step by step to decrease i2v ib from 9 to 1*/
		for (i = high_i2v_ib; i <= low_i2v_ib; i++) {
			snd_soc_component_update_bits(component,
					AUDUL_CON27,
					AUDDL_CON27_I2VL_IB_SEL_MASK,
					AUDDL_CON27_I2VL_IB_SEL_VAL(i));
			snd_soc_component_update_bits(component,
					AUDUL_CON26,
					AUDDL_CON26_I2VR_IB_SEL_MASK,
					AUDDL_CON26_I2VR_IB_SEL_VAL(i));
			usleep_range(10, 11);
		}
		snd_soc_component_update_bits(component,
			AUDDL_CON18,
...
static int mt8519_codec_ana_dac_vcm_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_component *component =
			snd_soc_dapm_to_component(w->dapm);
...
static int mt8519_codec_ana_adc_clk_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_component *component =
				snd_soc_dapm_to_component(w->dapm);
...
static int mt8519_codec_ana_adc_int1_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_component *component =
		snd_soc_dapm_to_component(w->dapm);
...
static int mt8519_codec_ana_adc_int2_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_component *component =
		snd_soc_dapm_to_component(w->dapm);
...
static int mt8519_codec_ana_adc_saradc_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_component *component =
		snd_soc_dapm_to_component(w->dapm);
...
static int mt8519_codec_ana_adc_left_pga_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_component *component =
		snd_soc_dapm_to_component(w->dapm);
...
static int mt8519_codec_ana_adc_right_pga_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_component *component =
		snd_soc_dapm_to_component(w->dapm);
...
static int mt8519_codec_ana_adc_ldo_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_component *component =
		snd_soc_dapm_to_component(w->dapm);
...
static int mt8519_codec_ana_adc_vcm_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_component *component =
		snd_soc_dapm_to_component(w->dapm);
...
static int mt8519_codec_ana_adc_vref_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_component *component =
		snd_soc_dapm_to_component(w->dapm);
...
static int mt8519_codec_dig_adda_clk_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_component *component =
		snd_soc_dapm_to_component(w->dapm);
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		mt8519_codec_enable_adda_afe_on(component);
		break;
	case SND_SOC_DAPM_POST_PMD:
		mt8519_codec_disable_adda_afe_on(component);
		break;
	default:
		return 0;
	}
...
static int mt8519_codec_micbias0_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_component *component =
		snd_soc_dapm_to_component(w->dapm);
	...
	case SND_SOC_DAPM_PRE_PMU:
		snd_soc_component_update_bits(component,
			...
			AUDUL_CON20_AUD_PWDB_MICBIAS_MASK);
		usleep_range(mt8519_codec->micbias0_setup_time_us,
			mt8519_codec->micbias0_setup_time_us + 1);
		break;
...
static const struct snd_soc_dapm_widget mt8519_codec_dapm_widgets[] = {
...
static const struct snd_soc_dapm_route mt8519_codec_intercon[] = {
...
struct mt8519_codec_rate {
	unsigned int rate;
	unsigned int regvalue;
};

static const struct mt8519_codec_rate mt8519_codec_ul_voice_modes[] = {
...
static const struct mt8519_codec_rate mt8519_codec_ul_rates[] = {
	{ .rate =  8000, .regvalue = 0, },
...
static const struct mt8519_codec_rate mt8519_codec_dl_voice_modes[] = {
...
static const struct mt8519_codec_rate mt8519_codec_dl_input_modes[] = {
	{ .rate =   8000, .regvalue = 0, },
...
static const struct mt8519_codec_rate mt8519_codec_dl_rates[] = {
	{ .rate =   8000, .regvalue = 0, },
	{ .rate =  11025, .regvalue = 1, },
	{ .rate =  12000, .regvalue = 2, },
	{ .rate =  16000, .regvalue = 3, },
...
static int mt8519_codec_rate_to_val(const struct mt8519_codec_rate *table,
...
struct mt8519_codec_rate_ctrl {
	unsigned int top_ctrl_reg;
	unsigned int top_ctrl_w_val;
	unsigned int top_ctrl_r_val;
};

static const struct mt8519_codec_rate_ctrl top_ctrl_regs[] = {
...
static int mt8519_codec_valid_new_rate(struct snd_soc_component *component,
	int stream)
{
...
static int mt8519_codec_setup_ul_rate(struct snd_soc_component *component,
	int rate)
{
	uint32_t val = 0;

	val = mt8519_codec_rate_to_val(mt8519_codec_ul_voice_modes,
		ARRAY_SIZE(mt8519_codec_ul_voice_modes), rate);
	if (val < 0)
		goto err;
...
static int mt8519_codec_setup_dl_rate(struct snd_soc_component *component,
	int rate)
{
	int32_t val = 0;

	val = mt8519_codec_rate_to_val(mt8519_codec_dl_input_modes,
		ARRAY_SIZE(mt8519_codec_dl_input_modes), rate);
	...
	val = mt8519_codec_rate_to_val(mt8519_codec_dl_rates,
		ARRAY_SIZE(mt8519_codec_dl_rates), rate);
	if (val < 0)
		goto err;
	...
	mt8519_codec_valid_new_rate(component, SNDRV_PCM_STREAM_PLAYBACK);
...
static int mt8519_codec_configure_ul(struct snd_soc_component *component)
{
	return 0;
}

static int mt8519_codec_configure_dl_nle(struct snd_soc_component *component,
	int rate)
{
	static struct {
		unsigned int reg;
...
static int mt8519_codec_configure_dl(struct snd_soc_component *component,
	int rate)
{
	struct mt8519_codec_priv *mt8519_codec =
		snd_soc_component_get_drvdata(component);
	...
	snd_soc_component_update_bits(component, ABB_AFE_CON5,
		...
		ABB_AFE_CON5_SDM_GAIN_VAL(0x3c));

	if (mt8519_codec->dl_nle_support)
		mt8519_codec_configure_dl_nle(component, rate);
	else {
		/* need to set ag to manual mode and */
		/* tune to max to make sure output swing when no nle */
		snd_soc_component_update_bits(component,
			AFE_NLE_GAIN_IMP_LCH_CFG0,
			AFE_NLE_ANALOG_GAIN_FIX_MANUAL_MODE,
			AFE_NLE_ANALOG_GAIN_FIX_MANUAL_VAL(0x1));
		snd_soc_component_update_bits(component,
			AFE_NLE_GAIN_IMP_RCH_CFG0,
			AFE_NLE_ANALOG_GAIN_FIX_MANUAL_MODE,
			AFE_NLE_ANALOG_GAIN_FIX_MANUAL_VAL(0x1));
		snd_soc_component_update_bits(component,
			AFE_NLE_GAIN_IMP_LCH_CFG0,
			AFE_NLE_ANALOG_GAIN_MANUAL_VAL_MASK,
			AFE_NLE_ANALOG_GAIN_MANUAL_VAL(0x16));
		snd_soc_component_update_bits(component,
			AFE_NLE_GAIN_IMP_RCH_CFG0,
			AFE_NLE_ANALOG_GAIN_MANUAL_VAL_MASK,
			AFE_NLE_ANALOG_GAIN_MANUAL_VAL(0x16));
...
static int mt8519_codec_hw_params(struct snd_pcm_substream *substream,
	...
	struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_CAPTURE)
		ret = mt8519_codec_setup_ul_rate(component, rate);
	else if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
		ret = mt8519_codec_setup_dl_rate(component, rate);
	...
	if (substream->stream == SNDRV_PCM_STREAM_CAPTURE)
		mt8519_codec_configure_ul(component);
	else if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
		mt8519_codec_configure_dl(component, rate);

	return ret;
}

static int mt8519_codec_enable_ul(struct snd_soc_component *component)
{
	snd_soc_component_update_bits(component, ABB_ULAFE_CON0,
...
static int mt8519_codec_enable_dl_nle(struct snd_soc_component *component)
{
...
static int mt8519_codec_enable_dl(struct snd_soc_component *component)
{
	struct mt8519_codec_priv *mt8519_codec =
		snd_soc_component_get_drvdata(component);

	if (mt8519_codec->dl_nle_support)
		mt8519_codec_enable_dl_nle(component);
...
static int mt8519_codec_disable_ul(struct snd_soc_component *component)
{
	snd_soc_component_update_bits(component, ABB_ULAFE_CON0,
		ABB_ULAFE_CON0_UL_FIFO_ON_MASK,
		0x0);
...
static int mt8519_codec_disable_dl_nle(struct snd_soc_component *component)
{
	snd_soc_component_update_bits(component, AFE_NLE_CFG,
		AFE_NLE_CFG_AFE_NLE_ON_MASK,
		0x0);
...
static int mt8519_codec_disable_dl(struct snd_soc_component *component)
{
	struct mt8519_codec_priv *mt8519_codec =
		snd_soc_component_get_drvdata(component);
	...
	if (mt8519_codec->dl_nle_support)
		mt8519_codec_disable_dl_nle(component);
...
static int mt8519_codec_trigger(struct snd_pcm_substream *substream,
	int cmd, struct snd_soc_dai *dai)
{
	...
	case SNDRV_PCM_TRIGGER_PAUSE_PUSH:
		if (substream->stream == SNDRV_PCM_STREAM_CAPTURE)
			mt8519_codec_disable_ul(component);
		else if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
			mt8519_codec_disable_dl(component);
		break;
	case SNDRV_PCM_TRIGGER_START:
	case SNDRV_PCM_TRIGGER_RESUME:
	case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		if (substream->stream == SNDRV_PCM_STREAM_CAPTURE)
			mt8519_codec_enable_ul(component);
		else if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
			mt8519_codec_enable_dl(component);
		break;
	default:
		return -EINVAL;
	}
...
#define MT8519_CODEC_DL_RATES SNDRV_PCM_RATE_8000_48000
#define MT8519_CODEC_UL_RATES (SNDRV_PCM_RATE_8000 | SNDRV_PCM_RATE_16000 | \
	SNDRV_PCM_RATE_32000 | SNDRV_PCM_RATE_48000)

#define MT8519_CODEC_FORMATS (SNDRV_PCM_FMTBIT_S16_LE | \
	SNDRV_PCM_FMTBIT_S32_LE)

static const struct snd_soc_dai_ops mt8519_codec_dai_ops = {
	.hw_params	= mt8519_codec_hw_params,
	.trigger	= mt8519_codec_trigger,
};

static struct snd_soc_dai_driver mt8519_codec_dai[] = {
	{
		.name = "mt8519-codec-dai",
		.playback = {
			.stream_name = "AIF Playback",
			.channels_min = 1,
			.channels_max = 2,
			.rates = MT8519_CODEC_DL_RATES,
			.formats = MT8519_CODEC_FORMATS,
		},
		.capture = {
			.stream_name = "AIF Capture",
			.channels_min = 1,
			.channels_max = 2,
			.rates = MT8519_CODEC_UL_RATES,
			.formats = MT8519_CODEC_FORMATS,
		},
		.ops = &mt8519_codec_dai_ops,
	},
};

#ifdef CONFIG_DEBUG_FS
struct mt8519_codec_reg_attr {
	uint32_t offset;
	char *name;
};
...
static const struct mt8519_codec_reg_attr mt8519_codec_dump_reg_list[] = {
	/* audio_top_sys */
	...
	DUMP_REG_ENTRY(AUDUL_CON26),
	DUMP_REG_ENTRY(AUDUL_CON27),
};

static ssize_t mt8519_codec_debug_read(struct file *file,
	char __user *user_buf,
	size_t count, loff_t *pos)
{
	struct mt8519_codec_priv *mt8519_codec = file->private_data;
	struct snd_soc_component *component = mt8519_codec->cmpnt;
	ssize_t ret, i;
	char *buf;
	int n = 0;
	unsigned int val;
	...
	for (i = 0; i < ARRAY_SIZE(mt8519_codec_dump_reg_list); i++) {
		if (snd_soc_component_read(component,
			mt8519_codec_dump_reg_list[i].offset, &val) < 0)
			val = 0;
		n += scnprintf(buf + n, count - n, "%s = 0x%x\n",
			mt8519_codec_dump_reg_list[i].name, val);
	}
...
static ssize_t mt8519_codec_debug_write(struct file *file,
...
static const struct file_operations mt8519_codec_debug_ops = {
	.open = simple_open,
	.read = mt8519_codec_debug_read,
	.write = mt8519_codec_debug_write,
	.llseek = default_llseek,
};
#endif

static void mt8519_codec_init_regs(struct snd_soc_component *component)
{
	static struct {
		unsigned int reg;
		unsigned int mask;
	...
	mt8519_codec_dac_depop_setup(component);

	mt8519_codec_adc_setup(component);

	mt8519_codec_adc_rc_calibration(component);
}


static struct regmap *mt8519_codec_get_regmap_of(const char *phandle_name,
		struct device *dev)
{
...
static const char * const mt8519_codec_regmap_of_str[REGMAP_NUMS] = {
	"mediatek,afe-regmap",
	"mediatek,apmixedsys-regmap",
};

static int mt8519_codec_parse_of(struct snd_soc_component *cmpnt)
{
	...
	for (i = 0; i < REGMAP_NUMS; i++) {
		mt8519_codec->regmap_modules[i] = mt8519_codec_get_regmap_of(
				mt8519_codec_regmap_of_str[i], dev);
		if (!mt8519_codec->regmap_modules[i]) {
			dev_info(dev, "%s failed to get %s\n", __func__,
				mt8519_codec_regmap_of_str[i]);
			ret = -EPROBE_DEFER;
			break;
		}
	}
	...
	of_property_read_u32(dev->of_node,
		...
		&mt8519_codec->micbias0_setup_time_us);

	of_property_read_u32(dev->of_node,
		"mediatek,downlink-nle-support",
		&mt8519_codec->dl_nle_support);

	if (device_property_read_bool(dev, "dac-disable")){
		mt8519_codec->dac_disable = true;
	}

	return ret;
}

static int mt8519_codec_clk_probe(struct snd_soc_component *cmpnt)
{
#ifdef COMMON_CLOCK_FRAMEWORK_API
	struct mt8519_codec_priv *mt8519_codec =
		snd_soc_component_get_drvdata(cmpnt);

	mt8519_codec->clk = devm_clk_get(cmpnt->dev, "bus");
	if (IS_ERR(mt8519_codec->clk)) {
		dev_dbg(cmpnt->dev, "%s devm_clk_get %s fail\n",
			__func__, "bus");
		return PTR_ERR(mt8519_codec->clk);
	}

	return clk_prepare_enable(mt8519_codec->clk);
#else
	return 0;
#endif
}

static int mt8519_codec_probe(struct snd_soc_component *cmpnt)
{
	struct mt8519_codec_priv *mt8519_codec =
		snd_soc_component_get_drvdata(cmpnt);
	int ret = 0;

	dev_info(cmpnt->dev, "+%s\n", __func__);

	mt8519_codec->cmpnt = cmpnt;

	ret = mt8519_codec_parse_of(cmpnt);
	if (ret < 0)
		return ret;

	spin_lock_init(&mt8519_codec->adda_afe_on_lock);
	spin_lock_init(&mt8519_codec->adda_26mclk_lock);
	spin_lock_init(&mt8519_codec->adda_rc_calibration_lock);

	ret = mt8519_codec_clk_probe(cmpnt);
	if (ret < 0)
		return ret;

	if(mt8519_codec->dac_disable == false)
		mt8519_codec_init_regs(cmpnt);

#ifdef CONFIG_DEBUG_FS
	mt8519_codec->debugfs = debugfs_create_file("mt8519_codec_regs",
			0644, NULL, mt8519_codec, &mt8519_codec_debug_ops);
#endif

	dev_info(cmpnt->dev, "-%s ret = %d\n", __func__, ret);

	return ret;
}

static void mt8519_codec_remove(struct snd_soc_component *cmpnt)
{
	struct mt8519_codec_priv *mt8519_codec =
		snd_soc_component_get_drvdata(cmpnt);

#ifdef CONFIG_DEBUG_FS
	debugfs_remove(mt8519_codec->debugfs);
#endif
#ifdef COMMON_CLOCK_FRAMEWORK_API
	clk_disable_unprepare(mt8519_codec->clk);
#endif
	mt8519_codec->cmpnt = NULL;
}

static int mt8519_codec_suspend(struct snd_soc_component *cmpnt)
{
#ifdef COMMON_CLOCK_FRAMEWORK_API
	struct mt8519_codec_priv *mt8519_codec =
		snd_soc_component_get_drvdata(cmpnt);
...
static int mt8519_codec_resume(struct snd_soc_component *cmpnt)
{
#ifdef COMMON_CLOCK_FRAMEWORK_API
	struct mt8519_codec_priv *mt8519_codec =
		snd_soc_component_get_drvdata(cmpnt);
...
static const struct snd_soc_component_driver soc_codec_dev_mt8519_codec = {
	.name = CODEC_MT8519_NAME,
	.probe = mt8519_codec_probe,
	.remove = mt8519_codec_remove,
	.suspend = mt8519_codec_suspend,
	.resume = mt8519_codec_resume,
	.controls		= mt8519_codec_snd_controls,
	.num_controls		= ARRAY_SIZE(mt8519_codec_snd_controls),
	.dapm_widgets		= mt8519_codec_dapm_widgets,
	.num_dapm_widgets	= ARRAY_SIZE(mt8519_codec_dapm_widgets),
	.dapm_routes		= mt8519_codec_intercon,
	.num_dapm_routes	= ARRAY_SIZE(mt8519_codec_intercon),
};

static int mt8519_codec_dev_probe(struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;
	struct mt8519_codec_priv *mt8519_codec = NULL;
	int ret;

	dev_info(dev, "%s dev name %s\n", __func__, dev_name(dev));

	mt8519_codec = devm_kzalloc(dev,
			sizeof(struct mt8519_codec_priv), GFP_KERNEL);
	if (!mt8519_codec)
		return -ENOMEM;

	/* get regmap of codec */
	mt8519_codec->regmap = devm_regmap_init(dev, NULL, mt8519_codec,
		&mt8519_codec_regmap);
	if (IS_ERR(mt8519_codec->regmap)) {
		dev_info(dev, "%s failed to get regmap of codec\n", __func__);
		devm_kfree(dev, mt8519_codec);
		mt8519_codec->regmap = NULL;
		return -EINVAL;
	}

	dev_set_drvdata(dev, mt8519_codec);

	mt8519_codec->adc_ch5 = iio_channel_get(dev, "adc-ch5");
	pr_err("get iio adc-ch5 = %d\n", IS_ERR(mt8519_codec->adc_ch5));

	if (IS_ERR(mt8519_codec->adc_ch5)) {
		ret = PTR_ERR(mt8519_codec->adc_ch5);
		mt8519_codec->ul_no_rc_cali = 1;
		dev_err(dev, "IIO channel not found:%d\n", ret);
	}

	return devm_snd_soc_register_component(dev,
			&soc_codec_dev_mt8519_codec,
			mt8519_codec_dai, ARRAY_SIZE(mt8519_codec_dai));
}

static const struct of_device_id mt8519_codec_of_match[] = {
	{.compatible = "mediatek," MT8519_CODEC_NAME,},
	{}
};

MODULE_DEVICE_TABLE(of, mt8519_codec_of_match);

static struct platform_driver mt8519_codec_driver = {
	.probe = mt8519_codec_dev_probe,
	.driver = {
		.name = MT8519_CODEC_NAME,
		.owner = THIS_MODULE,
		.of_match_table = mt8519_codec_of_match,
	},
};

module_platform_driver(mt8519_codec_driver);

MODULE_DESCRIPTION("ASoC MT8519 codec driver");
MODULE_LICENSE("GPL");
