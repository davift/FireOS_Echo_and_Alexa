...
/*
 * Mediatek 8519 ALSA SoC AFE platform driver
 *
...
#include "mt8519-afe-common.h"
#include "mt8519-afe-utils.h"
#include "mt8519-reg.h"
#include "mt8519-afe-debug.h"
#include "mt8519-afe-controls.h"
#include "../common/mtk-base-afe.h"
#include "../common/mtk-afe-platform-driver.h"
#include "../common/mtk-afe-fe-dai.h"
#include "mtk_hifixdsp_common.h"

#define MT8519_ETDM1_IN_MCLK_MULTIPLIER 256
#define MT8519_ETDM2_OUT_MCLK_MULTIPLIER 256
#define MT8519_ETDM2_IN_MCLK_MULTIPLIER 256
#define MT8519_ETDM_NORMAL_MAX_BCK_RATE 24576000
...
static const unsigned int mt8519_afe_backup_list[] = {
	AUDIO_TOP_CON0,
	AUDIO_TOP_CON1,
	AUDIO_TOP_CON2,
	AUDIO_TOP_CON3,
...
static const struct snd_pcm_hardware mt8519_afe_hardware = {
...
struct mt8519_afe_rate {
	unsigned int rate;
	unsigned int reg_val;
};

static const struct mt8519_afe_rate mt8519_afe_fs_rates[] = {
	{ .rate = 8000, .reg_val = MT8519_FS_8K },
	{ .rate = 12000, .reg_val = MT8519_FS_12K },
	{ .rate = 16000, .reg_val = MT8519_FS_16K },
	{ .rate = 24000, .reg_val = MT8519_FS_24K },
	{ .rate = 32000, .reg_val = MT8519_FS_32K },
	{ .rate = 48000, .reg_val = MT8519_FS_48K },
	{ .rate = 96000, .reg_val = MT8519_FS_96K },
	{ .rate = 192000, .reg_val = MT8519_FS_192K },
	{ .rate = 384000, .reg_val = MT8519_FS_384K },
	{ .rate = 7350, .reg_val = MT8519_FS_7P35K },
	{ .rate = 11025, .reg_val = MT8519_FS_11P025K },
	{ .rate = 14700, .reg_val = MT8519_FS_14P7K },
	{ .rate = 22050, .reg_val = MT8519_FS_22P05K },
	{ .rate = 29400, .reg_val = MT8519_FS_29P4K },
	{ .rate = 44100, .reg_val = MT8519_FS_44P1K },
	{ .rate = 88200, .reg_val = MT8519_FS_88P2K },
	{ .rate = 176400, .reg_val = MT8519_FS_176P4K },
	{ .rate = 352800, .reg_val = MT8519_FS_352P8K },
};

static int mt8519_afe_fs_timing(unsigned int rate)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(mt8519_afe_fs_rates); i++)
		if (mt8519_afe_fs_rates[i].rate == rate)
			return mt8519_afe_fs_rates[i].reg_val;

	return -EINVAL;
}

static int mt8519_afe_irq_direction_enable(struct mtk_base_afe *afe,
	int irq_id,
	int direction)
{
	struct mtk_base_afe_irq *irq;

	if (irq_id >= MT8519_AFE_IRQ_NUM)
		return -1;

	irq = &afe->irqs[irq_id];

	if (direction == MT8519_AFE_IRQ_DIR_MCU) {
		...
		regmap_update_bits(afe->regmap, AFE_IRQ_MASK,
		       (1 << irq->irq_data->irq_clr_shift),
		       (1 << irq->irq_data->irq_clr_shift));
	} else if (direction == MT8519_AFE_IRQ_DIR_DSP) {
		regmap_update_bits(afe->regmap, ASYS_IRQ_MASK,
		       (1 << irq->irq_data->irq_clr_shift),
		       (1 << irq->irq_data->irq_clr_shift));
		regmap_update_bits(afe->regmap, AFE_IRQ_MASK,
...
bool mt8519_afe_rate_supported(unsigned int rate, unsigned int id)
{
	switch (id) {
	case MT8519_AFE_IO_ETDM1_IN:
	case MT8519_AFE_IO_ETDM2_OUT:
	case MT8519_AFE_IO_ETDM2_IN:
		if (rate >= 8000 && rate <= 384000)
			return true;
		break;
	case MT8519_AFE_IO_DMIC:
	case MT8519_AFE_IO_PCM1:
	case MT8519_AFE_IO_INT_ADDA:
...
bool mt8519_afe_channel_supported(unsigned int channel, unsigned int id)
{
	switch (id) {
	case MT8519_AFE_IO_ETDM1_IN:
...
static int mt8519_afe_gasrc_get_period_val(unsigned int rate,
	bool op_freq_45m, unsigned int cali_cycles);

static void mt8519_afe_inc_etdm_occupy(struct mtk_base_afe *afe,
	unsigned int etdm_set, unsigned int stream)
{
	struct mt8519_afe_private *afe_priv = afe->platform_priv;
	struct mt8519_etdm_data *etdm_data = &afe_priv->etdm_data[etdm_set];
	unsigned long flags;
...
static void mt8519_afe_dec_etdm_occupy(struct mtk_base_afe *afe,
	unsigned int etdm_set, unsigned int stream)
{
	struct mt8519_afe_private *afe_priv = afe->platform_priv;
	struct mt8519_etdm_data *etdm_data = &afe_priv->etdm_data[etdm_set];
...
static unsigned int mt8519_afe_tdm_ch_fixup(unsigned int channels,
...
static const struct mt8519_etdm_ctrl_reg etdm_ctrl_reg[MT8519_ETDM_SETS][2] = {
	{
		{
			.con0 = -1,
			.con1 = -1,
...
static void mt8519_afe_enable_etdm(struct mtk_base_afe *afe,
	unsigned int etdm_set, unsigned int stream)
{
	struct mt8519_afe_private *afe_priv = afe->platform_priv;
	struct mt8519_etdm_data *etdm_data =
		&afe_priv->etdm_data[etdm_set];
	unsigned int en_reg = etdm_ctrl_reg[etdm_set][stream].con0;
	unsigned long flags;
	bool need_update = false;

	dev_info(afe->dev, "%s, etdm_set %u active %d \n", __func__, etdm_set, etdm_data->active[stream]);
...
static void mt8519_afe_disable_etdm(struct mtk_base_afe *afe,
	unsigned int etdm_set, unsigned int stream)
{
	struct mt8519_afe_private *afe_priv = afe->platform_priv;
	struct mt8519_etdm_data *etdm_data =
...
static bool mt8519_afe_is_etdm_low_power(struct mtk_base_afe *afe,
	unsigned int etdm_set, unsigned int stream)
{
...
static bool mt8519_afe_is_int_1x_en_low_power(struct mtk_base_afe *afe)
{
	unsigned int val;
	bool lp_mode;
...
static int mt8519_afe_configure_etdm_out(struct mtk_base_afe *afe,
	...
	unsigned int channels, unsigned int bit_width)
{
	...
	tdm_channels = (data_mode == MT8519_ETDM_DATA_ONE_PIN) ?
		mt8519_afe_tdm_ch_fixup(channels, etdm_set) : 2;
	...
	if (clk_mode == MT8519_ETDM_SHARED_CLOCK)
	...
	if (!slave_mode) {
		...
		if (bck > MT8519_ETDM_NORMAL_MAX_BCK_RATE)
	...
	val = ETDM_OUT_CON4_FS(mt8519_afe_fs_timing(rate));
	ctrl_mask = ETDM_OUT_CON4_CTRL_MASK;

	if (etdm_set == MT8519_ETDM2) {
		if (slave_mode)
			val |= ETDM_OUT_CON4_CONN_FS(MT8519_FS_ETDMOUT2_1X_EN);
		else
			val |= ETDM_OUT_CON4_CONN_FS(
				mt8519_afe_fs_timing(rate));
...
static int mt8519_afe_configure_etdm_in(struct mtk_base_afe *afe,
	unsigned int etdm_set, unsigned int rate,
	unsigned int channels, unsigned int bit_width)
{
	struct mt8519_afe_private *afe_priv = afe->platform_priv;
	...
	val |= ETDM_IN_CON1_LRCK_RESET_MASK;
	regmap_update_bits(afe->regmap, ctrl_reg, ctrl_mask, val);

	val = ETDM_IN_CON3_FS(mt8519_afe_fs_timing(rate));
	...
	if (slave_mode) {
		if (etdm_set == MT8519_ETDM1)
			val = ETDM_IN_CON4_CONN_FS(MT8519_FS_ETDMIN1_1X_EN);
		else
			val = ETDM_IN_CON4_CONN_FS(MT8519_FS_ETDMIN2_1X_EN);
	} else
	...
	val = 0;
	if (data_mode == MT8519_ETDM_DATA_MULTI_PIN) {
...
static void mt8519_afe_etdm1_in_force_on(struct mtk_base_afe *afe)
{
	struct mt8519_afe_private *priv = afe->platform_priv;
	struct mt8519_etdm_data *etdm1 = &priv->etdm_data[MT8519_ETDM1];
	...
	slave_mode = etdm1->slave_mode[SNDRV_PCM_STREAM_CAPTURE];

	if (rate % 8000)
		mt8519_afe_enable_clk(afe, priv->clocks[MT8519_CLK_FA2SYS]);

	if (mclk == 0)
		mclk = MT8519_ETDM1_IN_MCLK_MULTIPLIER * rate;

	mt8519_afe_enable_clk(afe, priv->clocks[MT8519_CLK_TDMIN_MCK]);
	mt8519_afe_enable_top_cg(afe, MT8519_TOP_CG_TDM_IN);

	mt8519_afe_configure_etdm_in(afe,
		MT8519_ETDM1,
		rate,
		etdm1->force_channels[SNDRV_PCM_STREAM_CAPTURE],
		etdm1->force_bit_width[SNDRV_PCM_STREAM_CAPTURE]);

	need_tuner = (mt8519_afe_is_int_1x_en_low_power(afe)
		  && !mt8519_afe_is_etdm_low_power(afe, MT8519_ETDM1,
				SNDRV_PCM_STREAM_CAPTURE))
		 || (!mt8519_afe_is_int_1x_en_low_power(afe)
		   && mt8519_afe_is_etdm_low_power(afe, MT8519_ETDM1,
				SNDRV_PCM_STREAM_CAPTURE));

	if (need_tuner) {
		if (rate % 8000) {
			mt8519_afe_enable_apll_associated_cfg(afe,
				MT8519_AFE_APLL1);
		} else {
			mt8519_afe_enable_apll_associated_cfg(afe,
				MT8519_AFE_APLL2);
	...
	mt8519_afe_set_clk_parent(afe,
		priv->clocks[MT8519_CLK_TDMIN_M_SEL],
		(rate % 8000) ? priv->clocks[MT8519_CLK_AUD1] :
		priv->clocks[MT8519_CLK_AUD2]);
	mt8519_afe_set_clk_rate(afe, priv->clocks[MT8519_CLK_APLL12_DIV1],
		mclk);

	if (!slave_mode)
		mt8519_afe_enable_etdm(afe, MT8519_ETDM1,
			SNDRV_PCM_STREAM_CAPTURE);

	etdm1->force_on_status[SNDRV_PCM_STREAM_CAPTURE] = true;
}

static void mt8519_afe_etdm2_out_force_on(struct mtk_base_afe *afe)
{
	struct mt8519_afe_private *priv = afe->platform_priv;
	struct mt8519_etdm_data *etdm2 = &priv->etdm_data[MT8519_ETDM2];
	unsigned int mclk, rate;
	bool need_tuner = false;
	bool slave_mode = false;
	...
	slave_mode = etdm2->slave_mode[SNDRV_PCM_STREAM_PLAYBACK];

	if (rate % 8000)
		mt8519_afe_enable_clk(afe, priv->clocks[MT8519_CLK_FA2SYS]);

	if (mclk == 0)
		mclk = MT8519_ETDM2_OUT_MCLK_MULTIPLIER * rate;

	mt8519_afe_enable_clk(afe, priv->clocks[MT8519_CLK_I2SOUT_MCK]);
	mt8519_afe_enable_top_cg(afe, MT8519_TOP_CG_I2S_OUT);

	mt8519_afe_configure_etdm_out(afe,
		MT8519_ETDM2,
		rate,
	...
	need_tuner = (mt8519_afe_is_int_1x_en_low_power(afe)
		  && !mt8519_afe_is_etdm_low_power(afe, MT8519_ETDM2,
	...
	mt8519_afe_set_clk_parent(afe,
		priv->clocks[MT8519_CLK_I2SOUT_M_SEL],
		(rate % 8000) ?
		priv->clocks[MT8519_CLK_AUD1] :
		priv->clocks[MT8519_CLK_AUD2]);
	mt8519_afe_set_clk_rate(afe,
		priv->clocks[MT8519_CLK_APLL12_DIV2],
		mclk);

	if (!slave_mode)
		mt8519_afe_enable_etdm(afe, MT8519_ETDM2,
			SNDRV_PCM_STREAM_PLAYBACK);

	etdm2->force_on_status[SNDRV_PCM_STREAM_PLAYBACK] = true;
}

static void mt8519_afe_etdm2_in_force_on(struct mtk_base_afe *afe)
{
	struct mt8519_afe_private *priv = afe->platform_priv;
	struct mt8519_etdm_data *etdm2 = &priv->etdm_data[MT8519_ETDM2];
	unsigned int mclk, rate;
	...
	slave_mode = etdm2->slave_mode[SNDRV_PCM_STREAM_CAPTURE];

	if (rate % 8000)
		mt8519_afe_enable_clk(afe, priv->clocks[MT8519_CLK_FA2SYS]);

	if (mclk == 0)
		mclk = MT8519_ETDM2_IN_MCLK_MULTIPLIER * rate;

	mt8519_afe_enable_clk(afe, priv->clocks[MT8519_CLK_I2SIN_MCK]);
	mt8519_afe_enable_top_cg(afe, MT8519_TOP_CG_I2S_IN);
	...
	mt8519_afe_set_clk_parent(afe,
		priv->clocks[MT8519_CLK_I2SIN_M_SEL],
		(rate % 8000) ?
		priv->clocks[MT8519_CLK_AUD1] :
		priv->clocks[MT8519_CLK_AUD2]);
	mt8519_afe_set_clk_rate(afe,
		priv->clocks[MT8519_CLK_APLL12_DIV0],
		mclk);
...
static int mt8519_afe_prepare_etdm_out(struct mtk_base_afe *afe,
	...
	unsigned int etdm_set)
{
	...
	mt8519_afe_configure_etdm_out(afe, etdm_set, rate,
		channels, bit_width);

	return 0;
}

static int mt8519_afe_prepare_etdm_in(struct mtk_base_afe *afe,
	struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai,
	unsigned int etdm_set)
{
	...
	mt8519_afe_configure_etdm_in(afe, etdm_set, rate,
		channels, bit_width);

	return 0;
}

static int mt8519_afe_etdm1_startup(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
	struct mt8519_afe_private *afe_priv = afe->platform_priv;
	struct mt8519_etdm_data *etdm_data =
		&afe_priv->etdm_data[MT8519_ETDM1];
	unsigned int clk_mode = etdm_data->clock_mode;
	...
	if (etdm_data->force_on[stream]) {
		bool force_apply_in_change =
			...
			(etdm_data->force_on_policy[SNDRV_PCM_STREAM_CAPTURE]
			 == MT8519_ETDM_FORCE_ON_1ST_TRIGGER);

		if (force_apply_in_change) {
			mt8519_afe_enable_main_clk(afe);

			if (force_apply_in_change)
				mt8519_afe_etdm1_in_force_on(afe);
		}
	...
	if (stream == SNDRV_PCM_STREAM_CAPTURE ||
	    clk_mode == MT8519_ETDM_SHARED_CLOCK) {
		mt8519_afe_enable_clk(afe,
			afe_priv->clocks[MT8519_CLK_TDMIN_MCK]);
		mt8519_afe_enable_top_cg(afe, MT8519_TOP_CG_TDM_IN);
	}
...
static void mt8519_afe_etdm1_shutdown(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
	struct mt8519_afe_private *afe_priv = afe->platform_priv;
	struct mt8519_be_dai_data *be =
		&afe_priv->be_data[dai->id - MT8519_AFE_BACKEND_BASE];
	unsigned int etdm_set = MT8519_ETDM1;
	struct mt8519_etdm_data *etdm_data = &afe_priv->etdm_data[etdm_set];
	unsigned int clk_mode = etdm_data->clock_mode;
	unsigned int stream = substream->stream;
	...
	if (reset_in_change) {
		if ((etdm_data->occupied[stream] == 1)
			...
			need_tuner = (mt8519_afe_is_int_1x_en_low_power(afe)
				&& !mt8519_afe_is_etdm_low_power(afe, etdm_set,
					SNDRV_PCM_STREAM_CAPTURE))
				|| (!mt8519_afe_is_int_1x_en_low_power(afe)
				&& mt8519_afe_is_etdm_low_power(afe, etdm_set,
					SNDRV_PCM_STREAM_CAPTURE));
			...
			if (need_tuner) {
				if (rate % 8000) {
					mt8519_afe_disable_apll_associated_cfg(
						afe, MT8519_AFE_APLL1);
				} else {
					mt8519_afe_disable_apll_associated_cfg(
						afe, MT8519_AFE_APLL2);
				}
			}
		}
		mt8519_afe_dec_etdm_occupy(afe, etdm_set,
			SNDRV_PCM_STREAM_CAPTURE);
	}

	if (etdm_data->force_on[stream] && !slave_mode)
		return;

	if (etdm_data->occupied[stream] == 0) {
		if (reset_in_change)
			mt8519_afe_disable_etdm(afe, etdm_set,
				SNDRV_PCM_STREAM_CAPTURE);
	}
	if (etdm_data->force_on[stream] && slave_mode)
		return;
	...
	if (reset_in_change) {
		mt8519_afe_disable_top_cg(afe, MT8519_TOP_CG_TDM_IN);
		mt8519_afe_disable_clk(afe,
			afe_priv->clocks[MT8519_CLK_TDMIN_MCK]);
	}
	...
	if (be->prepared[stream]) {
		...
		if (rate % 8000)
			mt8519_afe_disable_clk(afe,
					afe_priv->clocks[MT8519_CLK_FA2SYS]);
	}

	mt8519_afe_disable_main_clk(afe);
}

static int mt8519_afe_etdm1_prepare(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
	struct mt8519_afe_private *afe_priv = afe->platform_priv;
	...
	if (apply_in_change)
		mt8519_afe_inc_etdm_occupy(afe, etdm_set,
			SNDRV_PCM_STREAM_CAPTURE);

	if (etdm_data->force_on[stream]) {
		if (apply_in_change && slave_mode)
			mt8519_afe_enable_etdm(afe, etdm_set,
				SNDRV_PCM_STREAM_CAPTURE);
		return 0;
	}

	if (apply_in_change) {
		ret = mt8519_afe_prepare_etdm_in(afe, substream, dai,
						 etdm_set);
		if (ret)
			return ret;
	}
	...
	if (rate % 8000)
		mt8519_afe_enable_clk(afe, afe_priv->clocks[MT8519_CLK_FA2SYS]);
	...
	if (apply_in_change) {
		...
		mt8519_afe_set_clk_parent(afe,
			afe_priv->clocks[MT8519_CLK_TDMIN_M_SEL],
			(rate % 8000) ?
			afe_priv->clocks[MT8519_CLK_AUD1] :
			afe_priv->clocks[MT8519_CLK_AUD2]);
		mt8519_afe_set_clk_rate(afe,
			afe_priv->clocks[MT8519_CLK_APLL12_DIV1],
			mclk);
		mt8519_afe_enable_etdm(afe, etdm_set,
			SNDRV_PCM_STREAM_CAPTURE);
	}
...
static int mt8519_afe_etdm2_startup(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
	struct mt8519_afe_private *afe_priv = afe->platform_priv;
	struct mt8519_etdm_data *etdm_data =
		&afe_priv->etdm_data[MT8519_ETDM2];
	unsigned int clk_mode = etdm_data->clock_mode;
	unsigned int stream = substream->stream;
	...
	if (etdm_data->force_on[stream]) {
		...
		if (force_apply_out_change || force_apply_in_change) {
			...
			if (force_apply_out_change)
				mt8519_afe_etdm2_out_force_on(afe);

			if (force_apply_in_change)
				mt8519_afe_etdm2_in_force_on(afe);
		}
	...
	if (stream == SNDRV_PCM_STREAM_PLAYBACK ||
	    clk_mode == MT8519_ETDM_SHARED_CLOCK) {
		mt8519_afe_enable_clk(afe,
			afe_priv->clocks[MT8519_CLK_I2SOUT_MCK]);
		mt8519_afe_enable_top_cg(afe, MT8519_TOP_CG_I2S_OUT);
	}
	...
	if (stream == SNDRV_PCM_STREAM_CAPTURE ||
	    clk_mode == MT8519_ETDM_SHARED_CLOCK) {
		mt8519_afe_enable_clk(afe,
			afe_priv->clocks[MT8519_CLK_I2SIN_MCK]);
		mt8519_afe_enable_top_cg(afe, MT8519_TOP_CG_I2S_IN);
	}
...
static void mt8519_afe_etdm2_shutdown(struct snd_pcm_substream *substream,
				      struct snd_soc_dai *dai)
{
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
	struct mt8519_afe_private *afe_priv = afe->platform_priv;
	struct mt8519_be_dai_data *be =
		&afe_priv->be_data[dai->id - MT8519_AFE_BACKEND_BASE];
	unsigned int etdm_set = MT8519_ETDM2;
	struct mt8519_etdm_data *etdm_data = &afe_priv->etdm_data[etdm_set];
	unsigned int clk_mode = etdm_data->clock_mode;
	unsigned int stream = substream->stream;
	...
	if (reset_out_change) {
		mt8519_afe_disable_top_cg(afe, MT8519_TOP_CG_I2S_OUT);
		mt8519_afe_disable_clk(afe,
			afe_priv->clocks[MT8519_CLK_I2SOUT_MCK]);
	}

	if (reset_in_change) {
		mt8519_afe_disable_top_cg(afe, MT8519_TOP_CG_I2S_IN);
		mt8519_afe_disable_clk(afe,
			afe_priv->clocks[MT8519_CLK_I2SIN_MCK]);
	}
	if (be->prepared[stream]) {
...
static int mt8519_afe_etdm2_prepare(struct snd_pcm_substream *substream,
				    struct snd_soc_dai *dai)
{
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
	struct mt8519_afe_private *afe_priv = afe->platform_priv;
	...
	if (etdm_data->force_on[stream]) {
		if (apply_out_change && slave_mode)
			mt8519_afe_enable_etdm(afe, etdm_set,
				SNDRV_PCM_STREAM_PLAYBACK);
		if (apply_in_change && slave_mode)
			mt8519_afe_enable_etdm(afe, etdm_set,
	...
	if (apply_out_change) {
		ret = mt8519_afe_prepare_etdm_out(afe, substream, dai,
						  etdm_set);
		if (ret)
			return ret;
	}
	...
	if (apply_out_change) {
		...
		need_tuner = (mt8519_afe_is_int_1x_en_low_power(afe)
			  && !mt8519_afe_is_etdm_low_power(afe,
		...
		mt8519_afe_set_clk_parent(afe,
			afe_priv->clocks[MT8519_CLK_I2SOUT_M_SEL],
			(rate % 8000) ?
			afe_priv->clocks[MT8519_CLK_AUD1] :
			afe_priv->clocks[MT8519_CLK_AUD2]);
		mt8519_afe_set_clk_rate(afe,
			afe_priv->clocks[MT8519_CLK_APLL12_DIV2],
			mclk);
		mt8519_afe_enable_etdm(afe, etdm_set,
			SNDRV_PCM_STREAM_PLAYBACK);
	}
	...
	if (apply_in_change) {
		...
		mt8519_afe_set_clk_parent(afe,
			afe_priv->clocks[MT8519_CLK_I2SIN_M_SEL],
			(rate % 8000) ?
			afe_priv->clocks[MT8519_CLK_AUD1] :
			afe_priv->clocks[MT8519_CLK_AUD2]);
		mt8519_afe_set_clk_rate(afe,
			afe_priv->clocks[MT8519_CLK_APLL12_DIV0],
			mclk);
		mt8519_afe_enable_etdm(afe, etdm_set,
			SNDRV_PCM_STREAM_CAPTURE);
	}
...
static bool mt8519_match_1st_be_cpu_dai(struct snd_pcm_substream *substream,
	char *dai_name)
{
...
static int mt8519_memif_fs(struct snd_pcm_substream *substream,
	unsigned int rate)
{
	...
	struct mt8519_fe_dai_data *fe_data = &afe_priv->fe_data[dai_id];
	int fs;

	if (dai_id == MT8519_AFE_MEMIF_UL8)
		return MT8519_FS_ETDMIN1_NX_EN;

	if (dai_id == MT8519_AFE_MEMIF_UL3 && is_ul3_in_direct_mode(afe))
		return MT8519_FS_ETDMIN2_NX_EN;

	fs = mt8519_afe_fs_timing(rate);
	...
	// slave mode
	switch (dai_id) {
	case MT8519_AFE_MEMIF_UL2:
	case MT8519_AFE_MEMIF_UL3:
	case MT8519_AFE_MEMIF_UL4:
	case MT8519_AFE_MEMIF_UL9:
	case MT8519_AFE_MEMIF_UL10:
		if (mt8519_match_1st_be_cpu_dai(substream, "ETDM1_IN"))
			fs = MT8519_FS_ETDMIN1_1X_EN;
		else if (mt8519_match_1st_be_cpu_dai(substream, "ETDM2_IN"))
			fs = MT8519_FS_ETDMIN2_1X_EN;
		break;
	case MT8519_AFE_MEMIF_DLM:
	case MT8519_AFE_MEMIF_DL2:
	case MT8519_AFE_MEMIF_DL3:
		if (mt8519_match_1st_be_cpu_dai(substream, "ETDM2_OUT"))
			fs = MT8519_FS_ETDMOUT2_1X_EN;
		break;
	default:
		break;
	}
...
static int mt8519_irq_fs(struct snd_pcm_substream *substream,
	unsigned int rate)
{
	int irq_fs = mt8519_memif_fs(substream, rate);

	if (irq_fs == MT8519_FS_ETDMIN1_NX_EN)
		irq_fs = MT8519_FS_ETDMIN1_1X_EN;
	else if (irq_fs == MT8519_FS_ETDMIN2_NX_EN)
		irq_fs = MT8519_FS_ETDMIN2_1X_EN;

	return irq_fs;
}

static int mt8519_alloc_dmabuf(struct snd_pcm_substream *substream,
...
static int mt8519_free_dmabuf(struct snd_pcm_substream *substream,
			      struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
	...
	if (fe_data->use_sram) {
		...
		mt8519_afe_unblock_dpidle(afe);
	}

	return ret;
}

int mt8519_afe_fe_startup(struct snd_pcm_substream *substream,
			  struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
	...
	mt8519_afe_irq_direction_enable(afe,
		memif->irq_usage,
		MT8519_AFE_IRQ_DIR_MCU);
...
static void mt8519_afe_fe_shutdown(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
...
static int mt8519_afe_fe_hw_params(struct snd_pcm_substream *substream,
				   struct snd_pcm_hw_params *params,
				   struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	...
	struct mtk_base_afe_memif *memif = &afe->memif[dai_id];
	const struct mtk_base_memif_data *data = memif->data;
	struct mt8519_control_data *ctrl_data = &afe_priv->ctrl_data;
	...
	if (dai_id == MT8519_AFE_MEMIF_UL2) {
		unsigned int val = 0;
		...
		if (!ctrl_data->bypass_cm1) {
			...
			val |= UL_REORDER_START_DATA(8 + ctrl_data->cm1_start_ch) |
	...
	} else if (dai_id == MT8519_AFE_MEMIF_UL9) {
		unsigned int val = 0;
	...
	/* DMA allocate */
	ret = mt8519_alloc_dmabuf(substream, params, dai);
	...
	msb2_at_bit33 =
		upper_32_bits(substream->runtime->dma_addr + memif->buffer_size - 1) ? 1 : 0;

#if IS_ENABLED(CONFIG_REMAP_ON_DSP_BUS)
	memif->phys_buf_addr = afe_hal_phys_addr_cpu2dsp(memif->phys_buf_addr);
#endif

	/* start */
	if (memif->data->reg_ofs_base >= 0)
		regmap_write(afe->regmap, memif->data->reg_ofs_base,
			 memif->phys_buf_addr);
	/* end */
	if ((memif->data->reg_ofs_base + AFE_BASE_END_OFFSET) >= 0)
		regmap_write(afe->regmap,
				 memif->data->reg_ofs_base + AFE_BASE_END_OFFSET,
				 memif->phys_buf_addr + memif->buffer_size - 1);

	/* set MSB to 33-bit */
	if ((memif->data->msb_reg) >= 0 && (memif->data->msb_shift) >= 0)
		regmap_update_bits(afe->regmap, memif->data->msb_reg,
			       1 << memif->data->msb_shift,
			       msb_at_bit33 << memif->data->msb_shift);

	/* set buf end MSB to 33-bit */
	if ((memif->data->msb2_reg) >= 0 && (memif->data->msb2_shift) >= 0)
	...
	if ((memif->data->fs_reg) >= 0 && (memif->data->fs_shift) >= 0)
...
static int mt8519_afe_fe_hw_free(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
	...
	return mt8519_free_dmabuf(substream, dai);
}


static int mt8519_afe_fe_prepare(struct snd_pcm_substream *substream,
				 struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
...
int mt8519_afe_fe_trigger(struct snd_pcm_substream *substream, int cmd,
			  struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
...
static int mt8519_afe_fe_set_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
	struct mt8519_afe_private *afe_priv = afe->platform_priv;
	struct mt8519_fe_dai_data *fe_data = &afe_priv->fe_data[dai->id];
...
static int mt8519_afe_etdm_set_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
	struct mt8519_afe_private *afe_priv = afe->platform_priv;
	unsigned int etdm_set = (dai->id == MT8519_AFE_IO_ETDM1_IN) ?
				 MT8519_ETDM1 : MT8519_ETDM2;
	unsigned int stream = (dai->id == MT8519_AFE_IO_ETDM2_OUT) ?
			       SNDRV_PCM_STREAM_PLAYBACK :
			       SNDRV_PCM_STREAM_CAPTURE;
	...
	case SND_SOC_DAIFMT_I2S:
		afe_priv->etdm_data[etdm_set].format[stream] =
			MT8519_ETDM_FORMAT_I2S;
		break;
	case SND_SOC_DAIFMT_DSP_A:
		afe_priv->etdm_data[etdm_set].format[stream] =
			MT8519_ETDM_FORMAT_DSPA;
		break;
	case SND_SOC_DAIFMT_DSP_B:
		afe_priv->etdm_data[etdm_set].format[stream] =
			MT8519_ETDM_FORMAT_DSPB;
		break;
	case SND_SOC_DAIFMT_LEFT_J:
		afe_priv->etdm_data[etdm_set].format[stream] =
			MT8519_ETDM_FORMAT_LJ;
		break;
	case SND_SOC_DAIFMT_RIGHT_J:
		afe_priv->etdm_data[etdm_set].format[stream] =
			MT8519_ETDM_FORMAT_RJ;
		break;
	default:
		return -EINVAL;
	}
...
static int mt8519_afe_etdm_set_tdm_slot(struct snd_soc_dai *dai,
...
static int mt8519_afe_etdm_set_sysclk(struct snd_soc_dai *dai,
				      int clk_id,
				      unsigned int freq,
				      int dir)
{
...
static void mt8519_afe_enable_pcm1(struct mtk_base_afe *afe)
{
	regmap_update_bits(afe->regmap, PCM_INTF_CON1,
			   PCM_INTF_CON1_EN, PCM_INTF_CON1_EN);
}

static void mt8519_afe_disable_pcm1(struct mtk_base_afe *afe)
{
	regmap_update_bits(afe->regmap, PCM_INTF_CON1,
...
static int mt8519_afe_configure_pcm1(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	...
	if (!slave_mode) {
		...
		fs = mt8519_afe_fs_timing(runtime->rate);
		regmap_update_bits(afe->regmap, AFE_PCM_SRC_FS_CON0,
	...
	if (fmt == MT8519_PCM_FORMAT_PCMA ||
	    fmt == MT8519_PCM_FORMAT_PCMB)
...
static int mt8519_afe_pcm1_startup(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
	...
	mt8519_afe_enable_top_cg(afe, MT8519_TOP_CG_PCMIF);

	return 0;
}

static void mt8519_afe_pcm1_shutdown(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
	...
	mt8519_afe_disable_pcm1(afe);

	mt8519_afe_disable_top_cg(afe, MT8519_TOP_CG_PCMIF);
}

static int mt8519_afe_pcm1_prepare(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
	int ret;
	...
	ret = mt8519_afe_configure_pcm1(substream, dai);
	if (ret)
		return ret;

	mt8519_afe_enable_pcm1(afe);

	return 0;
}

static int mt8519_afe_pcm1_set_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
	struct mt8519_afe_private *afe_priv = afe->platform_priv;
	...
	switch (fmt & SND_SOC_DAIFMT_FORMAT_MASK) {
	case SND_SOC_DAIFMT_I2S:
		afe_priv->pcm_intf_data.format = MT8519_PCM_FORMAT_I2S;
		break;
	default:
		return -EINVAL;
	}
...
static void mt8519_afe_enable_dmic(struct mtk_base_afe *afe,
					struct snd_soc_dai *dai)
{
	unsigned int channels = dai->channels;
	unsigned int val = 0;
...
static void mt8519_afe_disable_dmic(struct mtk_base_afe *afe)
{
	unsigned int msk = 0;
...
static const struct reg_sequence mt8519_afe_dmic_iir_coeff_reg_defaults[] = {
...
static int mt8519_afe_load_dmic_iir_coeff_table(struct mtk_base_afe *afe)
{
	return regmap_multi_reg_write(afe->regmap,
			mt8519_afe_dmic_iir_coeff_reg_defaults,
			ARRAY_SIZE(mt8519_afe_dmic_iir_coeff_reg_defaults));
}

static int mt8519_afe_configure_dmic_array(struct mtk_base_afe *afe)
{
	struct mt8519_afe_private *afe_priv = afe->platform_priv;
	struct mt8519_dmic_data *dmic_data = &afe_priv->dmic_data;
...
static int mt8519_afe_configure_dmic(struct mtk_base_afe *afe,
	struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct mt8519_afe_private *afe_priv = afe->platform_priv;
	...
	mt8519_afe_configure_dmic_array(afe);
	...
	if (iir_on) {
		mt8519_afe_load_dmic_iir_coeff_table(afe);
...
static int mt8519_afe_dmic_startup(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
	...
	mt8519_afe_enable_top_cg(afe, MT8519_TOP_CG_DMIC0);
	mt8519_afe_enable_top_cg(afe, MT8519_TOP_CG_DMIC1);
	mt8519_afe_enable_top_cg(afe, MT8519_TOP_CG_DMIC2);
	mt8519_afe_enable_top_cg(afe, MT8519_TOP_CG_DMIC3);

	return 0;
}

static void mt8519_afe_dmic_shutdown(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
	...
	mt8519_afe_disable_dmic(afe);

	mt8519_afe_disable_top_cg(afe, MT8519_TOP_CG_DMIC3);
	mt8519_afe_disable_top_cg(afe, MT8519_TOP_CG_DMIC2);
	mt8519_afe_disable_top_cg(afe, MT8519_TOP_CG_DMIC1);
	mt8519_afe_disable_top_cg(afe, MT8519_TOP_CG_DMIC0);

	mt8519_afe_disable_main_clk(afe);
}

static int mt8519_afe_dmic_prepare(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
	...
	mt8519_afe_configure_dmic(afe, substream, dai);
	mt8519_afe_enable_dmic(afe, dai);

	return 0;
}

static int mt8519_afe_int_adda_startup(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
	struct mt8519_afe_private *afe_priv = afe->platform_priv;
	...
	mt8519_afe_enable_main_clk(afe);
	mt8519_afe_enable_clk(afe, afe_priv->clocks[MT8519_CLK_TOP_AUD_26M]);

	if (stream == SNDRV_PCM_STREAM_PLAYBACK) {
		mt8519_afe_enable_top_cg(afe, MT8519_TOP_CG_DAC);
		mt8519_afe_enable_top_cg(afe, MT8519_TOP_CG_DAC_PREDIS);
		mt8519_afe_enable_top_cg(afe, MT8519_TOP_CG_DL_ASRC);
	} else if (stream == SNDRV_PCM_STREAM_CAPTURE) {
		mt8519_afe_enable_top_cg(afe, MT8519_TOP_CG_ADC);
	}

	return 0;
}

static void mt8519_afe_int_adda_shutdown(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
	struct mt8519_afe_private *afe_priv = afe->platform_priv;
	...
	if (stream == SNDRV_PCM_STREAM_PLAYBACK) {
		mt8519_afe_disable_top_cg(afe, MT8519_TOP_CG_DL_ASRC);
		mt8519_afe_disable_top_cg(afe, MT8519_TOP_CG_DAC_PREDIS);
		mt8519_afe_disable_top_cg(afe, MT8519_TOP_CG_DAC);
	} else if (stream == SNDRV_PCM_STREAM_CAPTURE) {
		mt8519_afe_disable_top_cg(afe, MT8519_TOP_CG_ADC);
	}

	if (!mt8519_afe_is_int_1x_en_low_power(afe)) {
		if (rate % 8000)
			mt8519_afe_disable_apll_associated_cfg(afe,
				MT8519_AFE_APLL1);
		else
			mt8519_afe_disable_apll_associated_cfg(afe,
				MT8519_AFE_APLL2);
	...
	mt8519_afe_disable_clk(afe, afe_priv->clocks[MT8519_CLK_TOP_AUD_26M]);
	mt8519_afe_disable_main_clk(afe);
}

static int mt8519_afe_int_adda_prepare(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
	unsigned int stream = substream->stream;
	...
	regmap_update_bits(afe->regmap, ASMO_TIMING_CON0,
		...
		ASMO_TIMING_CON0_ASMO0_MODE_VAL(mt8519_afe_fs_timing(rate)));

	return 0;
}

static const struct mt8519_gasrc_ctrl_reg gasrc_ctrl_reg[MT8519_GASRC_NUM] = {
	[MT8519_GASRC0] = {
...
static const struct mt8519_afe_gasrc_mux_map gasrc_mux_maps[] = {
	{ MT8519_GASRC0, 0, MUX_GASRC_8CH, },
	{ MT8519_GASRC0, 1, MUX_GASRC_6CH, },
	{ MT8519_GASRC0, 2, MUX_GASRC_4CH, },
	{ MT8519_GASRC0, 3, MUX_GASRC_2CH, },
	{ MT8519_GASRC1, 0, MUX_GASRC_8CH, },
	{ MT8519_GASRC1, 1, MUX_GASRC_6CH, },
	{ MT8519_GASRC1, 2, MUX_GASRC_4CH, },
	{ MT8519_GASRC1, 3, MUX_GASRC_2CH, },
	{ MT8519_GASRC2, 0, MUX_GASRC_8CH, },
	{ MT8519_GASRC2, 1, MUX_GASRC_6CH, },
	{ MT8519_GASRC2, 2, MUX_GASRC_2CH, },
	{ MT8519_GASRC3, 0, MUX_GASRC_8CH, },
	{ MT8519_GASRC3, 1, MUX_GASRC_2CH, },
};

static int mt8519_afe_query_gasrc_mux(int gasrc_id, int idx)
{
	int i;
...
static int mt8519_afe_get_gasrc_mux_config(struct mtk_base_afe *afe,
	int gasrc_id, unsigned int stream)
{
	...
	if (stream == SNDRV_PCM_STREAM_PLAYBACK) {
		input_mux = mt8519_afe_query_gasrc_mux(gasrc_id,
		...
		output_mux = mt8519_afe_query_gasrc_mux(gasrc_id,
...
static int mt8519_dai_num_to_gasrc(int num)
{
	int val = num - MT8519_AFE_IO_GASRC0;

	if (val < 0 || val >= MT8519_GASRC_NUM)
		return -EINVAL;

	return val;
}

static void mt8519_afe_reset_gasrc(struct mtk_base_afe *afe,
	struct snd_soc_dai *dai)
{
	const int gasrc_id = mt8519_dai_num_to_gasrc(dai->id);
	unsigned int val = 0;
...
static void mt8519_afe_clear_gasrc(struct mtk_base_afe *afe,
	struct snd_soc_dai *dai)
{
	const int gasrc_id = mt8519_dai_num_to_gasrc(dai->id);
...
static struct snd_soc_dai *mt8519_get_be_cpu_dai(
...
static bool mt8519_be_cpu_dai_matched(struct snd_soc_dapm_widget *w,
	...
	if (playback) {
		snd_soc_dapm_widget_for_each_source_path(w, path) {
			if (path && path->source) {
				...
				ret = mt8519_be_cpu_dai_matched(path->source,
						dai_id, stream);
				if (ret)
					return ret;
			}
		...
		snd_soc_dapm_widget_for_each_sink_path(w, path) {
			if (path && path->sink) {
				...
				ret = mt8519_be_cpu_dai_matched(path->sink,
						dai_id, stream);
				if (ret)
					return ret;
			}
...
static bool mt8519_be_cpu_dai_connected(
	struct snd_pcm_substream *be_substream,
	char *dai_name, int stream, int dai_id)
{
	struct snd_soc_dai *dai =
		mt8519_get_be_cpu_dai(be_substream, dai_name);
	...
	return mt8519_be_cpu_dai_matched(w, dai_id, stream);
}

static bool mt8519_get_be_cpu_dai_rate(struct snd_pcm_substream *be_substream,
	char *dai_name, int stream, int dai_id, unsigned int *rate)
{
	struct snd_soc_dai *dai = mt8519_get_be_cpu_dai(be_substream, dai_name);
	bool ret = false;

	if (!rate)
		return ret;

	ret = mt8519_be_cpu_dai_connected(be_substream,
...
static int mt8519_afe_gasrc_get_input_rate(
	struct snd_pcm_substream *substream, int dai_id, unsigned int rate)
{
	unsigned int input_rate;

	if (mt8519_get_be_cpu_dai_rate(substream, "INT ADDA",
			SNDRV_PCM_STREAM_CAPTURE, dai_id, &input_rate))
		return input_rate;
	else if (mt8519_get_be_cpu_dai_rate(substream, "ETDM1_IN",
			SNDRV_PCM_STREAM_CAPTURE, dai_id, &input_rate))
		return input_rate;
	else if (mt8519_get_be_cpu_dai_rate(substream, "ETDM2_IN",
			SNDRV_PCM_STREAM_CAPTURE, dai_id, &input_rate))
		return input_rate;
	else if (mt8519_get_be_cpu_dai_rate(substream, "DMIC",
			SNDRV_PCM_STREAM_CAPTURE, dai_id, &input_rate))
		return input_rate;
...
static int mt8519_afe_gasrc_get_output_rate(
	struct snd_pcm_substream *substream, int dai_id, unsigned int rate)
{
	unsigned int output_rate;

	if (mt8519_get_be_cpu_dai_rate(substream, "ETDM2_OUT",
...
static int mt8519_afe_gasrc_get_input_fs(struct snd_pcm_substream *substream,
	int dai_id, unsigned int rate)
{
	...
	if (mt8519_be_cpu_dai_connected(substream, "INT ADDA",
			SNDRV_PCM_STREAM_CAPTURE, dai_id))
		fs = MT8519_FS_AMIC_1X_EN_ASYNC;

	if (mt8519_be_cpu_dai_connected(substream, "ETDM1_IN",
			SNDRV_PCM_STREAM_CAPTURE, dai_id)) {
		etdm_data = &afe_priv->etdm_data[MT8519_ETDM1];
		if (etdm_data->slave_mode[substream->stream]) {
			fs = MT8519_FS_ETDMIN1_1X_EN;
		} else {
			be_rate = mt8519_afe_gasrc_get_input_rate(substream,
				dai_id, rate);
			fs = mt8519_afe_fs_timing(be_rate);
		}
	}

	if (mt8519_be_cpu_dai_connected(substream, "ETDM2_IN",
			SNDRV_PCM_STREAM_CAPTURE, dai_id)) {
		etdm_data = &afe_priv->etdm_data[MT8519_ETDM2];
		if (etdm_data->slave_mode[substream->stream]) {
			fs = MT8519_FS_ETDMIN2_1X_EN;
		} else {
			be_rate = mt8519_afe_gasrc_get_input_rate(substream,
	...
	if (mt8519_be_cpu_dai_connected(substream, "DMIC",
			SNDRV_PCM_STREAM_CAPTURE, dai_id)) {
		be_rate = mt8519_afe_gasrc_get_input_rate(substream,
			dai_id, rate);
		fs = mt8519_afe_fs_timing(be_rate);
...
static int mt8519_afe_gasrc_get_output_fs(struct snd_pcm_substream *substream,
	int dai_id, unsigned int rate)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_component *component =
	...
	if (mt8519_be_cpu_dai_connected(substream, "ETDM2_OUT",
		SNDRV_PCM_STREAM_PLAYBACK, dai_id)) {
		etdm_data = &afe_priv->etdm_data[MT8519_ETDM2];
		if (etdm_data->slave_mode[substream->stream]) {
			fs = MT8519_FS_ETDMOUT2_1X_EN;
		} else {
			be_rate = mt8519_afe_gasrc_get_output_rate(substream,
				dai_id, rate);
			fs = mt8519_afe_fs_timing(be_rate);
		}
	}
...
static void mt8519_afe_gasrc_set_input_fs(struct mtk_base_afe *afe,
...
static void mt8519_afe_gasrc_set_output_fs(struct mtk_base_afe *afe,
	struct snd_soc_dai *dai, int fs_timing)
{
	const int gasrc_id = mt8519_dai_num_to_gasrc(dai->id);
	unsigned int val = 0;
...
static const
	struct mt8519_afe_gasrc_freq
		mt8519_afe_gasrc_freq_palette_49m_45m_64_cycles[] = {
...
static const
	struct mt8519_afe_gasrc_freq
		mt8519_afe_gasrc_period_palette_49m_64_cycles[] = {
...
static const
	struct mt8519_afe_gasrc_freq
		mt8519_afe_gasrc_period_palette_45m_64_cycles[] = {
...
static const
	struct mt8519_afe_gasrc_freq
		mt8519_afe_gasrc_freq_palette_49m_45m_48_cycles[] = {
...
static const
	struct mt8519_afe_gasrc_freq
		mt8519_afe_gasrc_period_palette_49m_48_cycles[] = {
...
static const
	struct mt8519_afe_gasrc_freq
		mt8519_afe_gasrc_period_palette_45m_441_cycles[] = {
...
static int mt8519_afe_gasrc_get_freq_val(unsigned int rate,
	unsigned int cali_cycles)
{
	int i;
	const struct mt8519_afe_gasrc_freq *freq_palette = NULL;
	int tbl_size = 0;
...
{
	int i;
	const struct mt8519_afe_gasrc_freq *period_palette = NULL;
	int tbl_size = 0;
...
static void mt8519_afe_gasrc_set_rx_mode_fs(struct mtk_base_afe *afe,
	struct snd_soc_dai *dai, int input_rate, int output_rate)
{
	struct mt8519_afe_private *afe_priv = afe->platform_priv;
	const int gasrc_id = mt8519_dai_num_to_gasrc(dai->id);
	unsigned int cali_cycles = 0;
	unsigned int ctrl_reg = 0;
	int val = 0;
	...
	val = mt8519_afe_gasrc_get_freq_val(output_rate, cali_cycles);
	if (val > 0)
		regmap_write(afe->regmap, ctrl_reg, val);

	ctrl_reg = gasrc_ctrl_reg[gasrc_id].con3;
	val = mt8519_afe_gasrc_get_freq_val(input_rate, cali_cycles);
	if (val > 0)
		regmap_write(afe->regmap, ctrl_reg, val);
}

static void mt8519_afe_gasrc_set_tx_mode_fs(struct mtk_base_afe *afe,
	struct snd_soc_dai *dai, int input_rate, int output_rate)
{
	struct mt8519_afe_private *afe_priv = afe->platform_priv;
	const int gasrc_id = mt8519_dai_num_to_gasrc(dai->id);
	...
	val = mt8519_afe_gasrc_get_period_val(output_rate,
		gasrc_op_freq_45m, cali_cycles);
	if (val > 0)
		regmap_write(afe->regmap, ctrl_reg, val);

	ctrl_reg = gasrc_ctrl_reg[gasrc_id].con1;
	val = mt8519_afe_gasrc_get_period_val(input_rate,
		gasrc_op_freq_45m, cali_cycles);
	if (val > 0)
		regmap_write(afe->regmap, ctrl_reg, val);
}

static void mt8519_afe_gasrc_use_sel(struct mtk_base_afe *afe,
...
struct mt8519_afe_gasrc_lrck_sel {
	int fs_timing;
	unsigned int lrck_sel_val;
};

static const
	struct mt8519_afe_gasrc_lrck_sel mt8519_afe_gasrc_lrck_sels[] = {
	{
		.fs_timing = MT8519_FS_ETDMIN2_1X_EN,
		.lrck_sel_val = MT8519_AFE_GASRC_LRCK_SEL_ETDM_IN2,
	},
	{
		.fs_timing = MT8519_FS_ETDMIN1_1X_EN,
		.lrck_sel_val = MT8519_AFE_GASRC_LRCK_SEL_ETDM_IN1,
	},
	{
		.fs_timing = MT8519_FS_ETDMOUT2_1X_EN,
		.lrck_sel_val = MT8519_AFE_GASRC_LRCK_SEL_ETDM_OUT2,
	},
	{
		.fs_timing = MT8519_FS_AMIC_1X_EN_ASYNC,
		.lrck_sel_val = MT8519_AFE_GASRC_LRCK_SEL_UL_VIRTUAL,
	},
};

static int mt8519_afe_gasrc_get_lrck_sel_val(int fs_timing)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(mt8519_afe_gasrc_lrck_sels); i++)
		if (mt8519_afe_gasrc_lrck_sels[i].fs_timing == fs_timing)
			return mt8519_afe_gasrc_lrck_sels[i].lrck_sel_val;

	return -EINVAL;
}

static void mt8519_afe_gasrc_sel_lrck(
	...
	struct snd_soc_dai *dai, int fs_timing)
{
	...
	val = mt8519_afe_gasrc_get_lrck_sel_val(fs_timing);
	if (val < 0)
		return;
...
static void mt8519_afe_gasrc_sel_cali_clk(
	struct mtk_base_afe *afe,
...
static bool mt8519_afe_gasrc_is_tx_tracking(int fs_timing)
{
	if (fs_timing == MT8519_FS_ETDMOUT2_1X_EN)
		return true;
	else
		return false;
}

static bool mt8519_afe_gasrc_is_rx_tracking(int fs_timing)
{
	if ((fs_timing == MT8519_FS_ETDMIN1_1X_EN) ||
		(fs_timing == MT8519_FS_ETDMIN2_1X_EN) ||
		(fs_timing == MT8519_FS_AMIC_1X_EN_ASYNC))
		return true;
	else
...
static void mt8519_afe_gasrc_enable_iir(struct mtk_base_afe *afe,
	int gasrc_id)
{
	unsigned int ctrl_reg = 0;
...
static void mt8519_afe_gasrc_disable_iir(struct mtk_base_afe *afe,
	int gasrc_id)
{
	unsigned int ctrl_reg = 0;
...
enum afe_gasrc_iir_coeff_table_id {
	MT8519_AFE_GASRC_IIR_COEFF_384_to_352 = 0,
	MT8519_AFE_GASRC_IIR_COEFF_256_to_192,
	MT8519_AFE_GASRC_IIR_COEFF_352_to_256,
	MT8519_AFE_GASRC_IIR_COEFF_384_to_256,
	MT8519_AFE_GASRC_IIR_COEFF_352_to_192,
	MT8519_AFE_GASRC_IIR_COEFF_384_to_192,
	MT8519_AFE_GASRC_IIR_COEFF_384_to_176,
	MT8519_AFE_GASRC_IIR_COEFF_256_to_96,
	MT8519_AFE_GASRC_IIR_COEFF_352_to_128,
	MT8519_AFE_GASRC_IIR_COEFF_384_to_128,
	MT8519_AFE_GASRC_IIR_COEFF_352_to_96,
	MT8519_AFE_GASRC_IIR_COEFF_384_to_96,
	MT8519_AFE_GASRC_IIR_COEFF_384_to_88,
	MT8519_AFE_GASRC_IIR_COEFF_256_to_48,
	MT8519_AFE_GASRC_IIR_COEFF_352_to_64,
	MT8519_AFE_GASRC_IIR_COEFF_384_to_64,
	MT8519_AFE_GASRC_IIR_COEFF_352_to_48,
	MT8519_AFE_GASRC_IIR_COEFF_384_to_48,
	MT8519_AFE_GASRC_IIR_COEFF_384_to_44,
	MT8519_AFE_GASRC_IIR_COEFF_352_to_32,
	MT8519_AFE_GASRC_IIR_COEFF_384_to_32,
	MT8519_AFE_GASRC_IIR_COEFF_352_to_24,
	MT8519_AFE_GASRC_IIR_COEFF_384_to_24,
	MT8519_AFE_GASRC_IIR_TABLES,
};

struct mt8519_afe_gasrc_iir_coeff_table_id {
	int input_rate;
	int output_rate;
	int table_id;
};

static const struct mt8519_afe_gasrc_iir_coeff_table_id
	mt8519_afe_gasrc_iir_coeff_table_ids[] = {
	{
		.input_rate = 8000,
		.output_rate = 7350,
		.table_id = MT8519_AFE_GASRC_IIR_COEFF_384_to_352,
	},
	{
		.input_rate = 12000,
		.output_rate = 8000,
		.table_id = MT8519_AFE_GASRC_IIR_COEFF_384_to_256,
	},
	{
		.input_rate = 12000,
		.output_rate = 11025,
	...
	},
	{
		.input_rate = 16000,
		.output_rate = 8000,
		.table_id = MT8519_AFE_GASRC_IIR_COEFF_384_to_192,
	},
	{
		.input_rate = 16000,
		.output_rate = 12000,
		.table_id = MT8519_AFE_GASRC_IIR_COEFF_256_to_192,
	},
	{
		.input_rate = 16000,
		.output_rate = 7350,
		.table_id = MT8519_AFE_GASRC_IIR_COEFF_384_to_176,
	},
	{
		.input_rate = 16000,
		.output_rate = 14700,
	...
	},
	{
		.input_rate = 24000,
		.output_rate = 8000,
		.table_id = MT8519_AFE_GASRC_IIR_COEFF_384_to_128,
	},
	{
		.input_rate = 24000,
		.output_rate = 12000,
	...
	},
	{
		.input_rate = 32000,
		.output_rate = 8000,
		.table_id = MT8519_AFE_GASRC_IIR_COEFF_384_to_96,
	},
	{
		.input_rate = 32000,
		.output_rate = 12000,
		.table_id = MT8519_AFE_GASRC_IIR_COEFF_256_to_96,
	},
	{
		.input_rate = 32000,
		.output_rate = 16000,
	...
	},
	{
		.input_rate = 32000,
		.output_rate = 7350,
		.table_id = MT8519_AFE_GASRC_IIR_COEFF_384_to_88,
	},
	{
		.input_rate = 32000,
		.output_rate = 14700,
	...
	},
	{
		.input_rate = 48000,
		.output_rate = 8000,
		.table_id = MT8519_AFE_GASRC_IIR_COEFF_384_to_64,
	},
	{
		.input_rate = 48000,
		.output_rate = 12000,
	...
	},
	{
		.input_rate = 96000,
		.output_rate = 8000,
		.table_id = MT8519_AFE_GASRC_IIR_COEFF_384_to_32,
	},
	{
		.input_rate = 96000,
		.output_rate = 12000,
		.table_id = MT8519_AFE_GASRC_IIR_COEFF_384_to_48,
	},
	{
		.input_rate = 96000,
		.output_rate = 16000,
	...
	},
	{
		.input_rate = 96000,
		.output_rate = 11025,
		.table_id = MT8519_AFE_GASRC_IIR_COEFF_384_to_44,
	},
	{
		.input_rate = 96000,
		.output_rate = 22050,
	...
	},
	{
		.input_rate = 192000,
		.output_rate = 12000,
		.table_id = MT8519_AFE_GASRC_IIR_COEFF_384_to_24,
	},
	{
		.input_rate = 192000,
		.output_rate = 16000,
	...
	},
	{
		.input_rate = 11025,
		.output_rate = 8000,
		.table_id = MT8519_AFE_GASRC_IIR_COEFF_352_to_256,
	},
	{
		.input_rate = 11025,
		.output_rate = 7350,
	...
	},
	{
		.input_rate = 14700,
		.output_rate = 8000,
		.table_id = MT8519_AFE_GASRC_IIR_COEFF_352_to_192,
	},
	{
		.input_rate = 14700,
		.output_rate = 7350,
	...
	},
	{
		.input_rate = 22050,
		.output_rate = 8000,
		.table_id = MT8519_AFE_GASRC_IIR_COEFF_352_to_128,
	},
	{
		.input_rate = 22050,
		.output_rate = 12000,
	...
	},
	{
		.input_rate = 29400,
		.output_rate = 8000,
		.table_id = MT8519_AFE_GASRC_IIR_COEFF_352_to_96,
	},
	{
		.input_rate = 29400,
		.output_rate = 16000,
	...
	},
	{
		.input_rate = 44100,
		.output_rate = 8000,
		.table_id = MT8519_AFE_GASRC_IIR_COEFF_352_to_64,
	},
	{
		.input_rate = 44100,
		.output_rate = 12000,
	...
	},
	{
		.input_rate = 88200,
		.output_rate = 8000,
		.table_id = MT8519_AFE_GASRC_IIR_COEFF_352_to_32,
	},
	{
		.input_rate = 88200,
		.output_rate = 12000,
		.table_id = MT8519_AFE_GASRC_IIR_COEFF_352_to_48,
	},
	{
		.input_rate = 88200,
		.output_rate = 16000,
	...
	},
	{
		.input_rate = 176400,
		.output_rate = 12000,
		.table_id = MT8519_AFE_GASRC_IIR_COEFF_352_to_24,
	},
	{
		.input_rate = 176400,
		.output_rate = 16000,
...
static const unsigned int
	mt8519_afe_gasrc_iir_coeffs[MT8519_AFE_GASRC_IIR_TABLES][IIR_NUMS] = {
	[MT8519_AFE_GASRC_IIR_COEFF_384_to_352] = {
...
static bool mt8519_afe_gasrc_found_iir_coeff_table_id(int input_rate,
	int output_rate, int *table_id)
{
	int i;
	const struct mt8519_afe_gasrc_iir_coeff_table_id *table =
		mt8519_afe_gasrc_iir_coeff_table_ids;
	...
	for (i = 0; i < ARRAY_SIZE(mt8519_afe_gasrc_iir_coeff_table_ids); i++) {
...
static bool mt8519_afe_gasrc_fill_iir_coeff_table(struct mtk_base_afe *afe,
	int gasrc_id, int table_id)
{
	...
	if ((table_id < 0) ||
		(table_id >= MT8519_AFE_GASRC_IIR_TABLES))
		return false;
	...
	table = &mt8519_afe_gasrc_iir_coeffs[table_id][0];
...
static bool mt8519_afe_load_gasrc_iir_coeff_table(struct mtk_base_afe *afe,
	int gasrc_id, int input_rate, int output_rate)
{
	int table_id;

	if (mt8519_afe_gasrc_found_iir_coeff_table_id(input_rate,
			output_rate, &table_id)) {
		return mt8519_afe_gasrc_fill_iir_coeff_table(afe,
			gasrc_id, table_id);
	}
...
static void mt8519_afe_adjust_gasrc_cali_cycles(struct mtk_base_afe *afe,
	struct snd_soc_dai *dai, int fs_timing, unsigned int rate)
{
	...
	if (fs_timing == MT8519_FS_AMIC_1X_EN_ASYNC) {
		switch (rate) {
		case 8000:
		case 16000:
		case 32000:
...
static int mt8519_afe_configure_gasrc(struct mtk_base_afe *afe,
	struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct mt8519_afe_private *afe_priv = afe->platform_priv;
	...
	unsigned int stream = substream->stream;
	const int gasrc_id = mt8519_dai_num_to_gasrc(dai->id);
	struct mt8519_gasrc_data *gasrc_data;
	int mux;
	int input_fs = 0, output_fs = 0;
	int input_rate = 0, output_rate = 0;
	int fix_input_rate = 0, fix_output_rate = 0;
	unsigned int val = 0;
	unsigned int mask = 0;
	...
	mux = mt8519_afe_get_gasrc_mux_config(afe, gasrc_id, stream);
	duplex = gasrc_data->duplex;
	gasrc_op_freq_45m = &(gasrc_data->op_freq_45m);
	cali_cycles = &(gasrc_data->cali_cycles);
	fix_input_rate = gasrc_data->fix_input_rate;
	fix_output_rate = gasrc_data->fix_output_rate;

	dev_dbg(dai->dev, "%s [%d] %s fix_input_rate %d, fix_output_rate %d\n",
		__func__, gasrc_id, PCM_STREAM_STR(substream->stream),
		fix_input_rate, fix_output_rate);

	if (stream == SNDRV_PCM_STREAM_PLAYBACK) {
		if (fix_input_rate) {
			input_fs = mt8519_afe_fs_timing(fix_input_rate);
			input_rate = fix_input_rate;
		} else {
			input_fs = mt8519_afe_fs_timing(runtime->rate);
			input_rate = runtime->rate;
		}
		*gasrc_op_freq_45m = (input_rate % 8000);

		if (fix_output_rate) {
			output_fs = mt8519_afe_fs_timing(fix_output_rate);
			output_rate = fix_output_rate;
		} else {
			if (duplex) {
				paired_substream =
					...
					SNDRV_PCM_STREAM_CAPTURE);
				output_fs =
					mt8519_afe_gasrc_get_output_fs(
					paired_substream, dai->id,
					paired_substream->runtime->rate);
				output_rate =
					mt8519_afe_gasrc_get_output_rate(
					paired_substream, dai->id,
					paired_substream->runtime->rate);
			} else {
				output_fs =
		...
		if (fix_output_rate) {
			...
			output_fs = mt8519_afe_fs_timing(runtime->rate);
		...
		if (fix_input_rate) {
			...
			if (duplex) {
				paired_substream =
					...
					SNDRV_PCM_STREAM_PLAYBACK);
				input_fs =
					mt8519_afe_gasrc_get_input_fs(
					paired_substream, dai->id,
					paired_substream->runtime->rate);
				input_rate =
					mt8519_afe_gasrc_get_input_rate(
					paired_substream, dai->id,
					paired_substream->runtime->rate);
			} else {
				input_fs =
	...
	if (mt8519_afe_load_gasrc_iir_coeff_table(afe, gasrc_id,
			input_rate, output_rate)) {
		mt8519_afe_gasrc_enable_iir(afe, gasrc_id);
		gasrc_data->iir_on = true;
	} else {
		mt8519_afe_gasrc_disable_iir(afe, gasrc_id);
		gasrc_data->iir_on = false;
	}

	/* INT_ADDA ADC (RX Tracking of 26m) */
	if (stream == SNDRV_PCM_STREAM_CAPTURE)
		mt8519_afe_adjust_gasrc_cali_cycles(afe, dai,
			input_fs, output_rate);

	mt8519_afe_gasrc_set_input_fs(afe, dai, input_fs);
	mt8519_afe_gasrc_set_output_fs(afe, dai, output_fs);

	gasrc_data->cali_tx = false;
	gasrc_data->cali_rx = false;
	if (stream == SNDRV_PCM_STREAM_PLAYBACK) {
		if (mt8519_afe_gasrc_is_tx_tracking(output_fs))
			gasrc_data->cali_tx = true;
		else
			gasrc_data->cali_tx = false;
	} else if (stream == SNDRV_PCM_STREAM_CAPTURE) {
		if (mt8519_afe_gasrc_is_rx_tracking(input_fs))
	...
	if (gasrc_data->one_heart && (gasrc_id != MT8519_GASRC0))
		regmap_update_bits(afe->regmap, gasrc_ctrl_reg[gasrc_id].con0,
			GASRC_NEW_CON0_ONE_HEART, GASRC_NEW_CON0_ONE_HEART);

	if (stream == SNDRV_PCM_STREAM_PLAYBACK) {
		mt8519_afe_gasrc_set_tx_mode_fs(afe,
			dai, input_rate, output_rate);
		if (gasrc_data->cali_tx) {
			mt8519_afe_gasrc_sel_cali_clk(afe,
				dai, *gasrc_op_freq_45m);
			mt8519_afe_gasrc_sel_lrck(afe, dai, output_fs);
		}
	} else if (stream == SNDRV_PCM_STREAM_CAPTURE) {
		mt8519_afe_gasrc_set_rx_mode_fs(afe,
			dai, input_rate, output_rate);
		if (gasrc_data->cali_rx) {
			mt8519_afe_gasrc_sel_cali_clk(afe,
				dai, *gasrc_op_freq_45m);
			mt8519_afe_gasrc_sel_lrck(afe, dai, input_fs);
		}
	}
...
static int mt8519_afe_enable_gasrc(struct snd_soc_dai *dai, int stream)
{
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
	struct mt8519_afe_private *afe_priv = afe->platform_priv;
	const int gasrc_id = mt8519_dai_num_to_gasrc(dai->id);
	...
	if (gasrc_data->cali_tx || gasrc_data->cali_rx) {
		if (gasrc_data->one_heart)
			ctrl_reg = gasrc_ctrl_reg[MT8519_GASRC0].con6;
		else
	...
	if (gasrc_data->one_heart)
		ctrl_reg = gasrc_ctrl_reg[MT8519_GASRC0].con0;
	else
		ctrl_reg = gasrc_ctrl_reg[gasrc_id].con0;
...
static int mt8519_afe_disable_gasrc(struct snd_soc_dai *dai, bool directly)
{
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
	struct mt8519_afe_private *afe_priv = afe->platform_priv;
	const int gasrc_id = mt8519_dai_num_to_gasrc(dai->id);
...
static int mt8519_afe_gasrc_startup(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
	struct mt8519_afe_private *afe_priv = afe->platform_priv;
	...
	mt8519_afe_enable_clk(afe, afe_priv->clocks[MT8519_CLK_FASM_L]);
	mt8519_afe_enable_clk(afe, afe_priv->clocks[MT8519_CLK_FASM_M]);
	mt8519_afe_enable_clk(afe, afe_priv->clocks[MT8519_CLK_FASM_H]);

	switch (gasrc_id) {
	case MT8519_GASRC0:
		mt8519_afe_enable_top_cg(afe, MT8519_TOP_CG_GASRC0);
		break;
	case MT8519_GASRC1:
		mt8519_afe_enable_top_cg(afe, MT8519_TOP_CG_GASRC1);
		break;
	case MT8519_GASRC2:
		mt8519_afe_enable_top_cg(afe, MT8519_TOP_CG_GASRC2);
		break;
	case MT8519_GASRC3:
		mt8519_afe_enable_top_cg(afe, MT8519_TOP_CG_GASRC3);
		break;
	default:
		break;
	}
...
static void mt8519_afe_gasrc_shutdown(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
	struct mt8519_afe_private *afe_priv = afe->platform_priv;
	...
	switch (gasrc_id) {
	case MT8519_GASRC0:
		mt8519_afe_disable_top_cg(afe, MT8519_TOP_CG_GASRC0);
		break;
	case MT8519_GASRC1:
		mt8519_afe_disable_top_cg(afe, MT8519_TOP_CG_GASRC1);
		break;
	case MT8519_GASRC2:
		mt8519_afe_disable_top_cg(afe, MT8519_TOP_CG_GASRC2);
		break;
	case MT8519_GASRC3:
		mt8519_afe_disable_top_cg(afe, MT8519_TOP_CG_GASRC3);
		break;
	default:
		break;
	}

	mt8519_afe_disable_clk(afe, afe_priv->clocks[MT8519_CLK_FASM_H]);
	mt8519_afe_disable_clk(afe, afe_priv->clocks[MT8519_CLK_FASM_M]);
	mt8519_afe_disable_clk(afe, afe_priv->clocks[MT8519_CLK_FASM_L]);

	mt8519_afe_disable_main_clk(afe);
}

static int mt8519_afe_gasrc_prepare(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
	struct mt8519_afe_private *afe_priv = afe->platform_priv;
	...
	if (gasrc_data->duplex && counter > 0) {
		mt8519_afe_disable_gasrc(dai, true);
		gasrc_data->re_enable[substream->stream] = true;
	}

	mt8519_afe_reset_gasrc(afe, dai);
	mt8519_afe_clear_gasrc(afe, dai);
	mt8519_afe_gasrc_use_sel(afe, dai, true);
	mt8519_afe_configure_gasrc(afe, substream, dai);

	return 0;
}

static int mt8519_afe_gasrc_trigger(struct snd_pcm_substream *substream,
	int cmd, struct snd_soc_dai *dai)
{
	int ret = 0;

	dev_dbg(dai->dev, "%s [%d] %s cmd %d\n", __func__,
		mt8519_dai_num_to_gasrc(dai->id),
		PCM_STREAM_STR(substream->stream), cmd);

	switch (cmd) {
	case SNDRV_PCM_TRIGGER_START:
	case SNDRV_PCM_TRIGGER_RESUME:
		ret = mt8519_afe_enable_gasrc(dai, substream->stream);
		break;
	case SNDRV_PCM_TRIGGER_STOP:
	case SNDRV_PCM_TRIGGER_SUSPEND:
		ret = mt8519_afe_disable_gasrc(dai, false);
		break;
	default:
		break;
	}
...
static int mt8519_afe_configure_multi_in(struct mtk_base_afe *afe,
...
{
	struct mt8519_afe_private *afe_priv = afe->platform_priv;
	struct mt8519_multi_in_data *multi_in = &afe_priv->multi_in_data;
	unsigned int val;
	...
	if (spdif_input) {
		...
		if (multi_in->format == MT8519_MULTI_IN_FORMAT_I2S) {
			val |= AFE_MPHONE_MULTI_CON1_DELAY_DATA |
			       AFE_MPHONE_MULTI_CON1_LEFT_ALIGN;
		} else if (multi_in->format == MT8519_MULTI_IN_FORMAT_LJ) {
			val |= AFE_MPHONE_MULTI_CON1_LEFT_ALIGN |
			       AFE_MPHONE_MULTI_CON1_LRCK_INV;
		} else if (multi_in->format == MT8519_MULTI_IN_FORMAT_RJ) {
...
static int mt8519_afe_enable_multi_in(struct mtk_base_afe *afe)
{
	regmap_update_bits(afe->regmap, AFE_MPHONE_MULTI_CON0,
...
static int mt8519_afe_disable_multi_in(struct mtk_base_afe *afe)
{
	struct mt8519_afe_private *afe_priv = afe->platform_priv;
...
static int mt8519_afe_spdif_in_startup(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
...
static void mt8519_afe_spdif_in_shutdown(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
...
static int mt8519_afe_spdif_in_prepare(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct snd_pcm_runtime * const runtime = substream->runtime;
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
	...
	ret = mt8519_afe_configure_multi_in(afe,
					    dai->channels,
					    dai->sample_bits,
					    runtime->period_size,
					    true);
...
static int mt8519_afe_spdif_in_trigger(struct snd_pcm_substream *substream,
	int cmd, struct snd_soc_dai *dai)
{
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
	...
	case SNDRV_PCM_TRIGGER_START:
	case SNDRV_PCM_TRIGGER_RESUME:
		mt8519_afe_enable_multi_in(afe);
		break;
	case SNDRV_PCM_TRIGGER_STOP:
	case SNDRV_PCM_TRIGGER_SUSPEND:
		mt8519_afe_disable_multi_in(afe);
		break;
	default:
		return -EINVAL;
	}
...
static int mt8519_afe_multi_in_startup(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
	...
	mt8519_afe_enable_top_cg(afe, MT8519_TOP_CG_MULTI_IN);

	return 0;
}

static void mt8519_afe_multi_in_shutdown(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
	...
	mt8519_afe_disable_top_cg(afe, MT8519_TOP_CG_MULTI_IN);
	mt8519_afe_disable_main_clk(afe);
}

static int mt8519_afe_multi_in_prepare(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct snd_pcm_runtime * const runtime = substream->runtime;
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
...
static int mt8519_afe_multi_in_set_fmt(struct snd_soc_dai *dai,
	unsigned int fmt)
{
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
	struct mt8519_afe_private *afe_priv = afe->platform_priv;
	...
	switch (fmt & SND_SOC_DAIFMT_FORMAT_MASK) {
	case SND_SOC_DAIFMT_I2S:
		afe_priv->multi_in_data.format = MT8519_MULTI_IN_FORMAT_I2S;
		break;
	case SND_SOC_DAIFMT_LEFT_J:
		afe_priv->multi_in_data.format = MT8519_MULTI_IN_FORMAT_LJ;
		break;
	case SND_SOC_DAIFMT_RIGHT_J:
		afe_priv->multi_in_data.format = MT8519_MULTI_IN_FORMAT_RJ;
		break;
	default:
		return -EINVAL;
	}
...
static int mt8519_afe_multi_in_trigger(struct snd_pcm_substream *substream,
	int cmd, struct snd_soc_dai *dai)
{
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
...
static int mt8519_afe_handle_etdm_force_on(struct mtk_base_afe *afe,
	bool probe);

static int mt8519_afe_handle_etdm_force_off(struct mtk_base_afe *afe);

static int mt8519_afe_dai_suspend(struct snd_soc_dai *dai)
{
	struct mtk_base_afe *afe = dev_get_drvdata(dai->dev);
	struct mt8519_afe_private *afe_priv = afe->platform_priv;
	int i;
#ifdef CONFIG_MTK_HIFIXDSP_SUPPORT
	struct mt8519_adsp_data *adsp_data = &(afe_priv->adsp_data);
#endif

	dev_info(afe->dev, "%s, dai %d suspend %d , c_active:%d, p_active:%d\n",
	...
	mt8519_afe_handle_etdm_force_off(afe);
	...
	mt8519_afe_disable_top_cg(afe, MT8519_TOP_CG_AFE_CONN);
	mt8519_afe_disable_top_cg(afe, MT8519_TOP_CG_AFE);
	mt8519_afe_disable_top_cg(afe, MT8519_TOP_CG_A1SYS_HOPPING);
	mt8519_afe_disable_top_cg(afe, MT8519_TOP_CG_A1SYS);
	mt8519_afe_disable_clk(afe, afe_priv->clocks[MT8519_CLK_TOP_AUD_BUS]);
...
static int mt8519_afe_dai_resume(struct snd_soc_dai *dai)
{
	struct mtk_base_afe *afe = dev_get_drvdata(dai->dev);
	struct mt8519_afe_private *afe_priv = afe->platform_priv;
	int i;
	...
	mt8519_afe_enable_clk(afe, afe_priv->clocks[MT8519_CLK_TOP_AUD_BUS]);
	mt8519_afe_enable_top_cg(afe, MT8519_TOP_CG_A1SYS);
	mt8519_afe_enable_top_cg(afe, MT8519_TOP_CG_A1SYS_HOPPING);
	mt8519_afe_enable_top_cg(afe, MT8519_TOP_CG_AFE);
	mt8519_afe_enable_top_cg(afe, MT8519_TOP_CG_AFE_CONN);
	...
	mt8519_afe_handle_etdm_force_on(afe, false);
...
/* FE DAIs */
static const struct snd_soc_dai_ops mt8519_afe_fe_dai_ops = {
	.startup	= mt8519_afe_fe_startup,
	.shutdown	= mt8519_afe_fe_shutdown,
	.hw_params	= mt8519_afe_fe_hw_params,
	.hw_free	= mt8519_afe_fe_hw_free,
	.prepare	= mt8519_afe_fe_prepare,
	.trigger	= mt8519_afe_fe_trigger,
	.set_fmt	= mt8519_afe_fe_set_fmt,
};

/* BE DAIs */
static const struct snd_soc_dai_ops mt8519_afe_etdm1_ops = {
	.startup	= mt8519_afe_etdm1_startup,
	.shutdown	= mt8519_afe_etdm1_shutdown,
	.prepare	= mt8519_afe_etdm1_prepare,
	.set_fmt	= mt8519_afe_etdm_set_fmt,
	.set_tdm_slot	= mt8519_afe_etdm_set_tdm_slot,
	.set_sysclk	= mt8519_afe_etdm_set_sysclk,
};

static const struct snd_soc_dai_ops mt8519_afe_etdm2_ops = {
	.startup	= mt8519_afe_etdm2_startup,
	.shutdown	= mt8519_afe_etdm2_shutdown,
	.prepare	= mt8519_afe_etdm2_prepare,
	.set_fmt	= mt8519_afe_etdm_set_fmt,
	.set_tdm_slot	= mt8519_afe_etdm_set_tdm_slot,
	.set_sysclk	= mt8519_afe_etdm_set_sysclk,
};

static const struct snd_soc_dai_ops mt8519_afe_pcm1_ops = {
	.startup	= mt8519_afe_pcm1_startup,
	.shutdown	= mt8519_afe_pcm1_shutdown,
	.prepare	= mt8519_afe_pcm1_prepare,
	.set_fmt	= mt8519_afe_pcm1_set_fmt,
};

static const struct snd_soc_dai_ops mt8519_afe_dmic_ops = {
	.startup	= mt8519_afe_dmic_startup,
	.shutdown	= mt8519_afe_dmic_shutdown,
	.prepare	= mt8519_afe_dmic_prepare,
};

static const struct snd_soc_dai_ops mt8519_afe_int_adda_ops = {
	.startup	= mt8519_afe_int_adda_startup,
	.shutdown	= mt8519_afe_int_adda_shutdown,
	.prepare	= mt8519_afe_int_adda_prepare,
};

static const struct snd_soc_dai_ops mt8519_afe_gasrc_ops = {
	.startup	= mt8519_afe_gasrc_startup,
	.shutdown	= mt8519_afe_gasrc_shutdown,
	.prepare	= mt8519_afe_gasrc_prepare,
	.trigger	= mt8519_afe_gasrc_trigger,
};

static const struct snd_soc_dai_ops mt8519_afe_spdif_in_ops = {
	.startup	= mt8519_afe_spdif_in_startup,
	.shutdown	= mt8519_afe_spdif_in_shutdown,
	.prepare	= mt8519_afe_spdif_in_prepare,
	.trigger	= mt8519_afe_spdif_in_trigger,
};

static const struct snd_soc_dai_ops mt8519_afe_multi_in_ops = {
	.startup	= mt8519_afe_multi_in_startup,
	.shutdown	= mt8519_afe_multi_in_shutdown,
	.prepare	= mt8519_afe_multi_in_prepare,
	.set_fmt	= mt8519_afe_multi_in_set_fmt,
	.trigger	= mt8519_afe_multi_in_trigger,
};

static struct snd_soc_dai_driver mt8519_afe_pcm_dais[] = {
	/* FE DAIs: memory intefaces to CPU */
	{
		.name = "DLM",
		.id = MT8519_AFE_MEMIF_DLM,
		.suspend = mt8519_afe_dai_suspend,
		.resume = mt8519_afe_dai_resume,
		.playback = {
			.stream_name = "DLM",
			.channels_min = 1,
			.channels_max = 8,
		...
		.ops = &mt8519_afe_fe_dai_ops,
	}, {
		.name = "DL2",
		.id = MT8519_AFE_MEMIF_DL2,
		.suspend = mt8519_afe_dai_suspend,
		.resume = mt8519_afe_dai_resume,
		.playback = {
			.stream_name = "DL2",
	...
	/* BE DAIs */
		...
		.ops = &mt8519_afe_etdm1_ops,
	}, {
	/* BE DAIs */
		.name = "ETDM2_OUT",
		.id = MT8519_AFE_IO_ETDM2_OUT,
		.playback = {
		...
		.ops = &mt8519_afe_etdm2_ops,
	}, {
	/* BE DAIs */
		.name = "ETDM2_IN",
		.id = MT8519_AFE_IO_ETDM2_IN,
		.capture = {
	...
	/* BE DAIs */
		...
		.ops = &mt8519_afe_pcm1_ops,
		.symmetric_rates = 1,
		.symmetric_samplebits = 1,
	}, {
	/* BE DAIs */
		.name = "VIRTUAL_DL_SRC",
		.id = MT8519_AFE_IO_VIRTUAL_DL_SRC,
		.capture = {
	...
	/* BE DAIs */
		...
		.ops = &mt8519_afe_dmic_ops,
	}, {
	/* BE DAIs */
		.name = "INT ADDA",
		.id = MT8519_AFE_IO_INT_ADDA,
		.playback = {
		...
		.ops = &mt8519_afe_int_adda_ops,
	}, {
	/* BE DAIs */
		.name = "GASRC0",
		.id = MT8519_AFE_IO_GASRC0,
		.playback = {
		...
		.ops = &mt8519_afe_gasrc_ops,
	}, {
	/* BE DAIs */
		.name = "GASRC1",
		.id = MT8519_AFE_IO_GASRC1,
		.playback = {
	...
	/* BE DAIs */
		...
		.ops = &mt8519_afe_spdif_in_ops,
	}, {
	/* BE DAIs */
		.name = "MULTI_IN",
		.id = MT8519_AFE_IO_MULTI_IN,
		.capture = {
		...
		.ops = &mt8519_afe_multi_in_ops,
	}, {
	/* BE DAIs */
		.name = "GASRC2_VIRTUAL",
		.id = MT8519_AFE_IO_VIRTUAL_GASRC2,
		.capture = {
			.stream_name = "GASRC2_VIRTUAL",
			.channels_min = 1,
			.channels_max = 2,
			.rates = SNDRV_PCM_RATE_8000_192000,
			.formats = SNDRV_PCM_FMTBIT_S16_LE |
...
static int mt8519_snd_soc_dapm_get_enum_double(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	...
	mt8519_afe_enable_reg_rw_clk(afe);

	ret = snd_soc_dapm_get_enum_double(kcontrol, ucontrol);

	mt8519_afe_disable_reg_rw_clk(afe);

	return ret;
}

static int mt8519_snd_soc_dapm_put_enum_double(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_dapm_context *dapm =
		snd_soc_dapm_kcontrol_dapm(kcontrol);
...
static const struct snd_kcontrol_new mt8519_afe_o00_mix[] = {
	SOC_DAPM_SINGLE_AUTODISABLE("I00 Switch", AFE_CONN0, 0, 1, 0),
};

static const struct snd_kcontrol_new mt8519_afe_o01_mix[] = {
	SOC_DAPM_SINGLE_AUTODISABLE("I01 Switch", AFE_CONN1, 1, 1, 0),
};

static const struct snd_kcontrol_new mt8519_afe_o02_mix[] = {
	SOC_DAPM_SINGLE_AUTODISABLE("I02 Switch", AFE_CONN2, 2, 1, 0),
};

static const struct snd_kcontrol_new mt8519_afe_o03_mix[] = {
	SOC_DAPM_SINGLE_AUTODISABLE("I03 Switch", AFE_CONN3, 3, 1, 0),
};

static const struct snd_kcontrol_new mt8519_afe_o04_mix[] = {
	...
	SOC_DAPM_SINGLE_AUTODISABLE("I36 Switch", AFE_CONN4_1, 4, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I40 Switch", AFE_CONN4_1, 8, 1, 0),
};

static const struct snd_kcontrol_new mt8519_afe_o05_mix[] = {
...
static const struct snd_kcontrol_new mt8519_afe_o14_mix[] = {
...
static const struct snd_kcontrol_new mt8519_afe_o15_mix[] = {
...
static const struct snd_kcontrol_new mt8519_afe_o16_mix[] = {
...
static const struct snd_kcontrol_new mt8519_afe_o17_mix[] = {
...
static const struct snd_kcontrol_new mt8519_afe_o20_mix[] = {
	...
	SOC_DAPM_SINGLE_AUTODISABLE("I36 Switch", AFE_CONN20_1, 4, 1, 0),
...
static const struct snd_kcontrol_new mt8519_afe_o21_mix[] = {
	...
	SOC_DAPM_SINGLE_AUTODISABLE("I37 Switch", AFE_CONN21_1, 5, 1, 0),
...
static const struct snd_kcontrol_new mt8519_afe_o26_mix[] = {
	SOC_DAPM_SINGLE_AUTODISABLE("I04 Switch", AFE_CONN26, 4, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I05 Switch", AFE_CONN26, 5, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I06 Switch", AFE_CONN26, 6, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I07 Switch", AFE_CONN26, 7, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I12 Switch", AFE_CONN26, 12, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I32 Switch", AFE_CONN26_1, 0, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I33 Switch", AFE_CONN26_1, 1, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I34 Switch", AFE_CONN26_1, 2, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I35 Switch", AFE_CONN26_1, 3, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I40 Switch", AFE_CONN26_1, 8, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I42 Switch", AFE_CONN26_1, 10, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I43 Switch", AFE_CONN26_1, 11, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I44 Switch", AFE_CONN26_1, 12, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I45 Switch", AFE_CONN26_1, 13, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I60 Switch", AFE_CONN26_1, 28, 1, 0),
};

static const struct snd_kcontrol_new mt8519_afe_o27_mix[] = {
	SOC_DAPM_SINGLE_AUTODISABLE("I04 Switch", AFE_CONN27, 4, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I05 Switch", AFE_CONN27, 5, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I06 Switch", AFE_CONN27, 6, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I07 Switch", AFE_CONN27, 7, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I13 Switch", AFE_CONN27, 13, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I32 Switch", AFE_CONN27_1, 0, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I33 Switch", AFE_CONN27_1, 1, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I34 Switch", AFE_CONN27_1, 2, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I35 Switch", AFE_CONN27_1, 3, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I41 Switch", AFE_CONN27_1, 9, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I42 Switch", AFE_CONN27_1, 10, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I43 Switch", AFE_CONN27_1, 11, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I44 Switch", AFE_CONN27_1, 12, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I45 Switch", AFE_CONN27_1, 13, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I61 Switch", AFE_CONN27_1, 29, 1, 0),
};

static const struct snd_kcontrol_new mt8519_afe_o28_mix[] = {
	SOC_DAPM_SINGLE_AUTODISABLE("I04 Switch", AFE_CONN28, 4, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I05 Switch", AFE_CONN28, 5, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I06 Switch", AFE_CONN28, 6, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I07 Switch", AFE_CONN28, 7, 1, 0),
	...
	SOC_DAPM_SINGLE_AUTODISABLE("I33 Switch", AFE_CONN28_1, 1, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I34 Switch", AFE_CONN28_1, 2, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I35 Switch", AFE_CONN28_1, 3, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I38 Switch", AFE_CONN28_1, 6, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I40 Switch", AFE_CONN28_1, 8, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I42 Switch", AFE_CONN28_1, 10, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I43 Switch", AFE_CONN28_1, 11, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I44 Switch", AFE_CONN28_1, 12, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I45 Switch", AFE_CONN28_1, 13, 1, 0),
};

static const struct snd_kcontrol_new mt8519_afe_o29_mix[] = {
	SOC_DAPM_SINGLE_AUTODISABLE("I04 Switch", AFE_CONN29, 4, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I05 Switch", AFE_CONN29, 5, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I06 Switch", AFE_CONN29, 6, 1, 0),
	...
	SOC_DAPM_SINGLE_AUTODISABLE("I32 Switch", AFE_CONN29_1, 0, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I33 Switch", AFE_CONN29_1, 1, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I34 Switch", AFE_CONN29_1, 2, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I35 Switch", AFE_CONN29_1, 3, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I36 Switch", AFE_CONN29_1, 4, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I39 Switch", AFE_CONN29_1, 7, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I40 Switch", AFE_CONN29_1, 8, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I42 Switch", AFE_CONN29_1, 10, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I43 Switch", AFE_CONN29_1, 11, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I44 Switch", AFE_CONN29_1, 12, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I45 Switch", AFE_CONN29_1, 13, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I41 Switch", AFE_CONN29_1, 9, 1, 0),
};

static const struct snd_kcontrol_new mt8519_afe_o30_mix[] = {
	...
	SOC_DAPM_SINGLE_AUTODISABLE("I26 Switch", AFE_CONN30, 26, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I27 Switch", AFE_CONN30, 27, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I32 Switch", AFE_CONN30_1, 0, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I33 Switch", AFE_CONN30_1, 1, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I36 Switch", AFE_CONN30_1, 4, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I37 Switch", AFE_CONN30_1, 5, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I38 Switch", AFE_CONN30_1, 6, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I40 Switch", AFE_CONN30_1, 8, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I41 Switch", AFE_CONN30_1, 9, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I46 Switch", AFE_CONN30_1, 14, 1, 0),
};

static const struct snd_kcontrol_new mt8519_afe_o31_mix[] = {
	...
	SOC_DAPM_SINGLE_AUTODISABLE("I27 Switch", AFE_CONN31, 27, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I28 Switch", AFE_CONN31, 28, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I33 Switch", AFE_CONN31_1, 1, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I34 Switch", AFE_CONN31_1, 2, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I37 Switch", AFE_CONN31_1, 5, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I38 Switch", AFE_CONN31_1, 6, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I39 Switch", AFE_CONN31_1, 7, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I41 Switch", AFE_CONN31_1, 9, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I47 Switch", AFE_CONN31_1, 15, 1, 0),
};

static const struct snd_kcontrol_new mt8519_afe_o32_mix[] = {
	...
	SOC_DAPM_SINGLE_AUTODISABLE("I26 Switch", AFE_CONN32, 26, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I28 Switch", AFE_CONN32, 28, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I29 Switch", AFE_CONN32, 29, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I32 Switch", AFE_CONN32_1, 0, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I34 Switch", AFE_CONN32_1, 2, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I35 Switch", AFE_CONN32_1, 3, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I38 Switch", AFE_CONN32_1, 6, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I39 Switch", AFE_CONN32_1, 7, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I40 Switch", AFE_CONN32_1, 8, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I42 Switch", AFE_CONN32_1, 10, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I48 Switch", AFE_CONN32_1, 16, 1, 0),
};

static const struct snd_kcontrol_new mt8519_afe_o33_mix[] = {
	...
	SOC_DAPM_SINGLE_AUTODISABLE("I27 Switch", AFE_CONN33, 27, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I29 Switch", AFE_CONN33, 29, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I33 Switch", AFE_CONN33_1, 1, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I35 Switch", AFE_CONN33_1, 3, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I36 Switch", AFE_CONN33_1, 4, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I39 Switch", AFE_CONN33_1, 7, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I41 Switch", AFE_CONN33_1, 9, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I43 Switch", AFE_CONN33_1, 11, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I49 Switch", AFE_CONN33_1, 17, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I58 Switch", AFE_CONN33_1, 26, 1, 0),
};

static const struct snd_kcontrol_new mt8519_afe_o34_mix[] = {
	...
	SOC_DAPM_SINGLE_AUTODISABLE("I28 Switch", AFE_CONN34, 28, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I32 Switch", AFE_CONN34_1, 0, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I36 Switch", AFE_CONN34_1, 4, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I37 Switch", AFE_CONN34_1, 5, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I38 Switch", AFE_CONN34_1, 6, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I44 Switch", AFE_CONN34_1, 12, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I50 Switch", AFE_CONN34_1, 18, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I59 Switch", AFE_CONN34_1, 27, 1, 0),
};

static const struct snd_kcontrol_new mt8519_afe_o35_mix[] = {
	SOC_DAPM_SINGLE_AUTODISABLE("I05 Switch", AFE_CONN35, 5, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I12 Switch", AFE_CONN35, 12, 1, 0),
	...
	SOC_DAPM_SINGLE_AUTODISABLE("I29 Switch", AFE_CONN35, 29, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I33 Switch", AFE_CONN35_1, 1, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I37 Switch", AFE_CONN35_1, 5, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I39 Switch", AFE_CONN35_1, 7, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I45 Switch", AFE_CONN35_1, 13, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I51 Switch", AFE_CONN35_1, 19, 1, 0),
};

static const struct snd_kcontrol_new mt8519_afe_o36_mix[] = {
	SOC_DAPM_SINGLE_AUTODISABLE("I06 Switch", AFE_CONN36, 6, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I12 Switch", AFE_CONN36, 12, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I13 Switch", AFE_CONN36, 13, 1, 0),
	...
	SOC_DAPM_SINGLE_AUTODISABLE("I38 Switch", AFE_CONN36_1, 6, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I52 Switch", AFE_CONN36_1, 20, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I58 Switch", AFE_CONN36_1, 26, 1, 0),
};

static const struct snd_kcontrol_new mt8519_afe_o37_mix[] = {
	SOC_DAPM_SINGLE_AUTODISABLE("I07 Switch", AFE_CONN37, 7, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I13 Switch", AFE_CONN37, 13, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I14 Switch", AFE_CONN37, 14, 1, 0),
	...
	SOC_DAPM_SINGLE_AUTODISABLE("I39 Switch", AFE_CONN37_1, 7, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I53 Switch", AFE_CONN37_1, 21, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I59 Switch", AFE_CONN37_1, 27, 1, 0),
};

static const struct snd_kcontrol_new mt8519_afe_o38_mix[] = {
	SOC_DAPM_SINGLE_AUTODISABLE("I08 Switch", AFE_CONN38, 8, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I12 Switch", AFE_CONN38, 12, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I14 Switch", AFE_CONN38, 14, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I15 Switch", AFE_CONN38, 15, 1, 0),
...
static const struct snd_kcontrol_new mt8519_afe_o39_mix[] = {
	SOC_DAPM_SINGLE_AUTODISABLE("I09 Switch", AFE_CONN39, 9, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I13 Switch", AFE_CONN39, 13, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I15 Switch", AFE_CONN39, 15, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I16 Switch", AFE_CONN39, 16, 1, 0),
...
static const struct snd_kcontrol_new mt8519_afe_o40_mix[] = {
	SOC_DAPM_SINGLE_AUTODISABLE("I10 Switch", AFE_CONN40, 10, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I14 Switch", AFE_CONN40, 14, 1, 0),
...
static const struct snd_kcontrol_new mt8519_afe_o41_mix[] = {
	SOC_DAPM_SINGLE_AUTODISABLE("I11 Switch", AFE_CONN41, 11, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I15 Switch", AFE_CONN41, 15, 1, 0),
...
static const struct snd_kcontrol_new mt8519_afe_o42_mix[] = {
	...
	SOC_DAPM_SINGLE_AUTODISABLE("I43 Switch", AFE_CONN42_1, 11, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I44 Switch", AFE_CONN42_1, 12, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I45 Switch", AFE_CONN42_1, 13, 1, 0),
};

static const struct snd_kcontrol_new mt8519_afe_o43_mix[] = {
	...
	SOC_DAPM_SINGLE_AUTODISABLE("I42 Switch", AFE_CONN43_1, 10, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I43 Switch", AFE_CONN43_1, 11, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I44 Switch", AFE_CONN43_1, 12, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I45 Switch", AFE_CONN43_1, 13, 1, 0),
};

static const struct snd_kcontrol_new mt8519_afe_o44_mix[] = {
	...
	SOC_DAPM_SINGLE_AUTODISABLE("I42 Switch", AFE_CONN44_1, 10, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I43 Switch", AFE_CONN44_1, 11, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I44 Switch", AFE_CONN44_1, 12, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I45 Switch", AFE_CONN44_1, 13, 1, 0),
};

static const struct snd_kcontrol_new mt8519_afe_o45_mix[] = {
...
static const struct snd_kcontrol_new mt8519_afe_o46_mix[] = {
	...
	SOC_DAPM_SINGLE_AUTODISABLE("I20 Switch", AFE_CONN46, 20, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I22 Switch", AFE_CONN46, 22, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I26 Switch", AFE_CONN46, 26, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I46 Switch", AFE_CONN46_1, 14, 1, 0),
};

static const struct snd_kcontrol_new mt8519_afe_o47_mix[] = {
	...
	SOC_DAPM_SINGLE_AUTODISABLE("I21 Switch", AFE_CONN47, 21, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I23 Switch", AFE_CONN47, 23, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I27 Switch", AFE_CONN47, 27, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I47 Switch", AFE_CONN47_1, 15, 1, 0),
};

static const struct snd_kcontrol_new mt8519_afe_o48_mix[] = {
	SOC_DAPM_SINGLE_AUTODISABLE("I10 Switch", AFE_CONN48, 10, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I12 Switch", AFE_CONN48, 12, 1, 0),
...
static const struct snd_kcontrol_new mt8519_afe_o49_mix[] = {
	SOC_DAPM_SINGLE_AUTODISABLE("I11 Switch", AFE_CONN49, 11, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I13 Switch", AFE_CONN49, 13, 1, 0),
...
static const struct snd_kcontrol_new ul3_mux =
	SOC_DAPM_ENUM_EXT("UL3 Source", ul3_mux_enum,
		mt8519_snd_soc_dapm_get_enum_double,
		mt8519_snd_soc_dapm_put_enum_double);
...
static const char * const mt8519_afe_gasrc_text_1[] = {
	"Gasrc_8ch", "Gasrc_6ch", "Gasrc_4ch", "Gasrc_2ch",
};

static const char * const mt8519_afe_gasrc_text_2[] = {
	"Gasrc_8ch", "Gasrc_6ch", "Gasrc_2ch",
};

static const char * const mt8519_afe_gasrc_text_3[] = {
	"Gasrc_8ch", "Gasrc_2ch",
};

static SOC_ENUM_SINGLE_VIRT_DECL(mt8519_afe_gasrc0_input_enum,
	mt8519_afe_gasrc_text_1);
static SOC_ENUM_SINGLE_VIRT_DECL(mt8519_afe_gasrc1_input_enum,
	mt8519_afe_gasrc_text_1);
static SOC_ENUM_SINGLE_VIRT_DECL(mt8519_afe_gasrc2_input_enum,
	mt8519_afe_gasrc_text_2);
static SOC_ENUM_SINGLE_VIRT_DECL(mt8519_afe_gasrc3_input_enum,
	mt8519_afe_gasrc_text_3);

static SOC_ENUM_SINGLE_VIRT_DECL(mt8519_afe_gasrc0_output_enum,
	mt8519_afe_gasrc_text_1);
static SOC_ENUM_SINGLE_VIRT_DECL(mt8519_afe_gasrc1_output_enum,
	mt8519_afe_gasrc_text_1);
static SOC_ENUM_SINGLE_VIRT_DECL(mt8519_afe_gasrc2_output_enum,
	mt8519_afe_gasrc_text_2);
static SOC_ENUM_SINGLE_VIRT_DECL(mt8519_afe_gasrc3_output_enum,
	mt8519_afe_gasrc_text_3);

static int mt8519_afe_gasrc_string_to_id(const char *name)
{
	int gasrc_id = -1;

	if (strstr(name, "GASRC0"))
		gasrc_id = MT8519_GASRC0;
	else if (strstr(name, "GASRC1"))
		gasrc_id = MT8519_GASRC1;
	else if (strstr(name, "GASRC2"))
		gasrc_id = MT8519_GASRC2;
	else if (strstr(name, "GASRC3"))
		gasrc_id = MT8519_GASRC3;

	return gasrc_id;
}

static int mt8519_afe_gasrc_source_mux_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_dapm_context *dapm =
		snd_soc_dapm_kcontrol_dapm(kcontrol);
	...
	int gasrc_id = mt8519_afe_gasrc_string_to_id(name);
...
static int mt8519_afe_gasrc_source_mux_put(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_dapm_context *dapm =
		snd_soc_dapm_kcontrol_dapm(kcontrol);
...
static int mt8519_afe_gasrc_sink_mux_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_dapm_context *dapm =
		snd_soc_dapm_kcontrol_dapm(kcontrol);
...
static int mt8519_afe_gasrc_sink_mux_put(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_dapm_context *dapm =
		snd_soc_dapm_kcontrol_dapm(kcontrol);
...
#ifdef CONFIG_MTK_HIFIXDSP_SUPPORT
struct mtk_base_afe *mt8519_afe_pcm_get_info(void)
{
	return g_priv;
}
EXPORT_SYMBOL_GPL(mt8519_afe_pcm_get_info);

static int mt8519_adsp_afe_memif_set(struct mtk_base_afe *afe,
				       int memif_id,
				       unsigned int rate,
				       unsigned int channels,
				       snd_pcm_format_t format)
{
	...
	if (memif->data->ch_num_shift >= 0)
		regmap_update_bits(afe->regmap,
			memif->data->ch_num_reg,
			memif->data->ch_num_maskbit << memif->data->ch_num_shift,
			channels << memif->data->ch_num_shift);

	if (memif->data->int_odd_shift >= 0) {
		unsigned int odd_en = (channels == 1) ? 1 : 0;

		regmap_update_bits(afe->regmap, memif->data->int_odd_reg,
				   1 << memif->data->int_odd_shift,
				   odd_en << memif->data->int_odd_shift);
	}

	if (memif_id == MT8519_AFE_MEMIF_UL2) {
		unsigned int val = 0;
		...
		regmap_update_bits(afe->regmap, AFE_I2S_UL2_REORDER,
				   UL_REORDER_CTRL_MASK, val);
	} else if (memif_id == MT8519_AFE_MEMIF_UL9) {
		unsigned int val = 0;
	...
	if (memif->data->id == MT8519_AFE_MEMIF_UL8)
		fs = MT8519_FS_ETDMIN1_NX_EN;
	...
	mt8519_afe_irq_direction_enable(afe,
		memif->irq_usage,
		MT8519_AFE_IRQ_DIR_DSP);

	return 0;
}

static int mt8519_adsp_afe_memif_enable(struct mtk_base_afe *afe,
				       int memif_id,
				       unsigned int rate,
				       unsigned int period_size,
				       int enable)
...
static int mt8519_afe_memif_init(struct mtk_base_afe *afe, int memif_id)
{
	struct mtk_base_afe_memif *memif = &afe->memif[memif_id];
...
static int mt8519_afe_memif_uninit(struct mtk_base_afe *afe, int memif_id)
{
	struct mtk_base_afe_memif *memif = &afe->memif[memif_id];
	...
	mt8519_adsp_afe_memif_enable(afe, memif_id, 0, 0, 0);
...
static int mt8519_afe_sram_get(struct mtk_base_afe *afe,
			int memif_id,
			dma_addr_t *paddr,
			unsigned char **vaddr,
			unsigned int *size,
			void *user)
{
	...
	*paddr = afe_priv->afe_sram_phy_addr;
	*size = afe_priv->afe_sram_size;
	return ret;
}

static int mt8519_afe_sram_put(struct mtk_base_afe *afe,
			       dma_addr_t *paddr,
			       void *user)
{
	return 0;//mtk_audio_sram_free(afe->sram, user);
}

static void mt8519_adsp_afe_init(struct mtk_base_afe *afe)
{
	dev_info(afe->dev, "%s\n", __func__);
...
static void mt8519_adsp_afe_uninit(struct mtk_base_afe *afe)
{
	dev_info(afe->dev, "%s\n", __func__);
...
static const struct snd_kcontrol_new mt8519_afe_gasrc0_input_mux =
	SOC_DAPM_ENUM_EXT("GASRC0 Source", mt8519_afe_gasrc0_input_enum,
		mt8519_afe_gasrc_source_mux_get,
		mt8519_afe_gasrc_source_mux_put);
static const struct snd_kcontrol_new mt8519_afe_gasrc1_input_mux =
	SOC_DAPM_ENUM_EXT("GASRC1 Source", mt8519_afe_gasrc1_input_enum,
		mt8519_afe_gasrc_source_mux_get,
		mt8519_afe_gasrc_source_mux_put);
static const struct snd_kcontrol_new mt8519_afe_gasrc2_input_mux =
	SOC_DAPM_ENUM_EXT("GASRC2 Source", mt8519_afe_gasrc2_input_enum,
		mt8519_afe_gasrc_source_mux_get,
		mt8519_afe_gasrc_source_mux_put);
static const struct snd_kcontrol_new mt8519_afe_gasrc3_input_mux =
	SOC_DAPM_ENUM_EXT("GASRC3 Source", mt8519_afe_gasrc3_input_enum,
		mt8519_afe_gasrc_source_mux_get,
		mt8519_afe_gasrc_source_mux_put);

static const struct snd_kcontrol_new mt8519_afe_gasrc0_output_mux =
	SOC_DAPM_ENUM_EXT("GASRC0 Sink", mt8519_afe_gasrc0_output_enum,
		mt8519_afe_gasrc_sink_mux_get,
		mt8519_afe_gasrc_sink_mux_put);
static const struct snd_kcontrol_new mt8519_afe_gasrc1_output_mux =
	SOC_DAPM_ENUM_EXT("GASRC1 Sink", mt8519_afe_gasrc1_output_enum,
		mt8519_afe_gasrc_sink_mux_get,
		mt8519_afe_gasrc_sink_mux_put);
static const struct snd_kcontrol_new mt8519_afe_gasrc2_output_mux =
	SOC_DAPM_ENUM_EXT("GASRC2 Sink", mt8519_afe_gasrc2_output_enum,
		mt8519_afe_gasrc_sink_mux_get,
		mt8519_afe_gasrc_sink_mux_put);
static const struct snd_kcontrol_new mt8519_afe_gasrc3_output_mux =
	SOC_DAPM_ENUM_EXT("GASRC3 Sink", mt8519_afe_gasrc3_output_enum,
		mt8519_afe_gasrc_sink_mux_get,
		mt8519_afe_gasrc_sink_mux_put);

/* AFE_DAC_CLK */
static int mt8519_afe_dac_clk_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		mt8519_afe_enable_main_clk(afe);
		mt8519_afe_enable_clk(afe,
			afe_priv->clocks[MT8519_CLK_TOP_AUD_26M]);
		mt8519_afe_enable_top_cg(afe, MT8519_TOP_CG_DAC);
		mt8519_afe_enable_top_cg(afe, MT8519_TOP_CG_DAC_PREDIS);
		break;
...
/* AFE_ADC_CLK */
static int mt8519_afe_adc_clk_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_component *component =
		snd_soc_dapm_to_component(w->dapm);
...
static const struct snd_soc_dapm_widget mt8519_afe_pcm_widgets[] = {
	/* inter-connections */
	...
	// virtual mixer
	SND_SOC_DAPM_MIXER("I20V", SND_SOC_NOPM, 0, 0, NULL, 0),
	SND_SOC_DAPM_MIXER("I21V", SND_SOC_NOPM, 0, 0, NULL, 0),
	...
	SND_SOC_DAPM_MIXER("I58V", SND_SOC_NOPM, 0, 0, NULL, 0),
	SND_SOC_DAPM_MIXER("I59V", SND_SOC_NOPM, 0, 0, NULL, 0),
	SND_SOC_DAPM_MIXER("I40V", SND_SOC_NOPM, 0, 0, NULL, 0),
	SND_SOC_DAPM_MIXER("I41V", SND_SOC_NOPM, 0, 0, NULL, 0),
	SND_SOC_DAPM_MIXER("I36V", SND_SOC_NOPM, 0, 0, NULL, 0),
	SND_SOC_DAPM_MIXER("I37V", SND_SOC_NOPM, 0, 0, NULL, 0),

	SND_SOC_DAPM_MIXER("O14_O15", SND_SOC_NOPM, 0, 0, NULL, 0),

	SND_SOC_DAPM_MIXER("O00", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o00_mix, ARRAY_SIZE(mt8519_afe_o00_mix)),
	SND_SOC_DAPM_MIXER("O01", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o01_mix, ARRAY_SIZE(mt8519_afe_o01_mix)),
	SND_SOC_DAPM_MIXER("O02", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o02_mix, ARRAY_SIZE(mt8519_afe_o02_mix)),
	SND_SOC_DAPM_MIXER("O03", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o03_mix, ARRAY_SIZE(mt8519_afe_o03_mix)),
	SND_SOC_DAPM_MIXER("O04", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o04_mix, ARRAY_SIZE(mt8519_afe_o04_mix)),
	SND_SOC_DAPM_MIXER("O05", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o05_mix, ARRAY_SIZE(mt8519_afe_o05_mix)),
	SND_SOC_DAPM_MIXER("O06", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o06_mix, ARRAY_SIZE(mt8519_afe_o06_mix)),
	SND_SOC_DAPM_MIXER("O07", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o07_mix, ARRAY_SIZE(mt8519_afe_o07_mix)),
	SND_SOC_DAPM_MIXER("O08", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o08_mix, ARRAY_SIZE(mt8519_afe_o08_mix)),
	SND_SOC_DAPM_MIXER("O09", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o09_mix, ARRAY_SIZE(mt8519_afe_o09_mix)),
	SND_SOC_DAPM_MIXER("O10", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o10_mix, ARRAY_SIZE(mt8519_afe_o10_mix)),
	SND_SOC_DAPM_MIXER("O11", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o11_mix, ARRAY_SIZE(mt8519_afe_o11_mix)),
	SND_SOC_DAPM_MIXER("O12", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o12_mix, ARRAY_SIZE(mt8519_afe_o12_mix)),
	SND_SOC_DAPM_MIXER("O13", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o13_mix, ARRAY_SIZE(mt8519_afe_o13_mix)),
	SND_SOC_DAPM_MIXER("O14", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o14_mix, ARRAY_SIZE(mt8519_afe_o14_mix)),
	SND_SOC_DAPM_MIXER("O15", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o15_mix, ARRAY_SIZE(mt8519_afe_o15_mix)),
	SND_SOC_DAPM_MIXER("O16", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o16_mix, ARRAY_SIZE(mt8519_afe_o16_mix)),
	SND_SOC_DAPM_MIXER("O17", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o17_mix, ARRAY_SIZE(mt8519_afe_o17_mix)),
	SND_SOC_DAPM_MIXER("O18", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o18_mix, ARRAY_SIZE(mt8519_afe_o18_mix)),
	SND_SOC_DAPM_MIXER("O19", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o19_mix, ARRAY_SIZE(mt8519_afe_o19_mix)),
	SND_SOC_DAPM_MIXER("O20", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o20_mix, ARRAY_SIZE(mt8519_afe_o20_mix)),
	SND_SOC_DAPM_MIXER("O21", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o21_mix, ARRAY_SIZE(mt8519_afe_o21_mix)),
	SND_SOC_DAPM_MIXER("O26", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o26_mix, ARRAY_SIZE(mt8519_afe_o26_mix)),
	SND_SOC_DAPM_MIXER("O27", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o27_mix, ARRAY_SIZE(mt8519_afe_o27_mix)),
	SND_SOC_DAPM_MIXER("O28", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o28_mix, ARRAY_SIZE(mt8519_afe_o28_mix)),
	SND_SOC_DAPM_MIXER("O29", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o29_mix, ARRAY_SIZE(mt8519_afe_o29_mix)),
	SND_SOC_DAPM_MIXER("O30", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o30_mix, ARRAY_SIZE(mt8519_afe_o30_mix)),
	SND_SOC_DAPM_MIXER("O31", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o31_mix, ARRAY_SIZE(mt8519_afe_o31_mix)),
	SND_SOC_DAPM_MIXER("O32", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o32_mix, ARRAY_SIZE(mt8519_afe_o32_mix)),
	SND_SOC_DAPM_MIXER("O33", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o33_mix, ARRAY_SIZE(mt8519_afe_o33_mix)),
	SND_SOC_DAPM_MIXER("O34", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o34_mix, ARRAY_SIZE(mt8519_afe_o34_mix)),
	SND_SOC_DAPM_MIXER("O35", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o35_mix, ARRAY_SIZE(mt8519_afe_o35_mix)),
	SND_SOC_DAPM_MIXER("O36", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o36_mix, ARRAY_SIZE(mt8519_afe_o36_mix)),
	SND_SOC_DAPM_MIXER("O37", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o37_mix, ARRAY_SIZE(mt8519_afe_o37_mix)),
	SND_SOC_DAPM_MIXER("O38", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o38_mix, ARRAY_SIZE(mt8519_afe_o38_mix)),
	SND_SOC_DAPM_MIXER("O39", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o39_mix, ARRAY_SIZE(mt8519_afe_o39_mix)),
	SND_SOC_DAPM_MIXER("O40", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o40_mix, ARRAY_SIZE(mt8519_afe_o40_mix)),
	SND_SOC_DAPM_MIXER("O41", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o41_mix, ARRAY_SIZE(mt8519_afe_o41_mix)),
	SND_SOC_DAPM_MIXER("O42", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o42_mix, ARRAY_SIZE(mt8519_afe_o42_mix)),
	SND_SOC_DAPM_MIXER("O43", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o43_mix, ARRAY_SIZE(mt8519_afe_o43_mix)),
	SND_SOC_DAPM_MIXER("O44", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o44_mix, ARRAY_SIZE(mt8519_afe_o44_mix)),
	SND_SOC_DAPM_MIXER("O45", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o45_mix, ARRAY_SIZE(mt8519_afe_o45_mix)),
	SND_SOC_DAPM_MIXER("O46", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o46_mix, ARRAY_SIZE(mt8519_afe_o46_mix)),
	SND_SOC_DAPM_MIXER("O47", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o47_mix, ARRAY_SIZE(mt8519_afe_o47_mix)),
	SND_SOC_DAPM_MIXER("O48", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o48_mix, ARRAY_SIZE(mt8519_afe_o48_mix)),
	SND_SOC_DAPM_MIXER("O49", SND_SOC_NOPM, 0, 0,
			   mt8519_afe_o49_mix, ARRAY_SIZE(mt8519_afe_o49_mix)),
	...
	SND_SOC_DAPM_SUPPLY_S("AFE_DAC_CLK", 2, SND_SOC_NOPM, 0, 0,
			mt8519_afe_dac_clk_event,
			SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_SUPPLY_S("AFE_ADC_CLK", 2, SND_SOC_NOPM, 0, 0,
			mt8519_afe_adc_clk_event,
			SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
};

static const struct snd_soc_dapm_route mt8519_afe_pcm_routes[] = {
	{"I22", NULL, "DLM"},
	{"I23", NULL, "DLM"},
	{"I24", NULL, "DLM"},
	{"I25", NULL, "DLM"},
	...
	{"I20V", NULL, "VIRTUAL_DL_SRC"},
	{"I21V", NULL, "VIRTUAL_DL_SRC"},
	...
	{"I58V", NULL, "VIRTUAL_DL_SRC"},
	{"I59V", NULL, "VIRTUAL_DL_SRC"},

	{"I40V", NULL, "VIRTUAL_DL_SRC"},
	{"I41V", NULL, "VIRTUAL_DL_SRC"},
	...
	{"GASRC2_VIRTUAL", NULL, "Virtual_GASRC2_Input"},
	{"I36V", NULL, "GASRC2_VIRTUAL"},
	{"I37V", NULL, "GASRC2_VIRTUAL"},
	{"O29", "I36 Switch", "I36V"},
	{"O30", "I37 Switch", "I37V"},
...
static const struct mtk_base_memif_data memif_data[] = {
	{
		.name = "DLM",
		.id = MT8519_AFE_MEMIF_DLM,
...
static int spdif_in_irq_handler(int irq, void *dev_id)
{
	...
	struct mt8519_afe_private *afe_priv = afe->platform_priv;
	struct mtk_base_afe_memif *memif = &afe->memif[MT8519_AFE_MEMIF_UL1];
	struct mt8519_multi_in_data *multi_in = &afe_priv->multi_in_data;
...
static int spdif_in_detect_irq_handler(int irq, void *dev_id)
{
	struct mtk_base_afe *afe = dev_id;
	struct mt8519_afe_private *afe_priv = afe->platform_priv;
	struct mt8519_spdif_in_data *spdif_in = &afe_priv->spdif_in_data;
...
static const struct mtk_base_irq_data irq_data[MT8519_AFE_IRQ_NUM] = {
	{
		.id = MT8519_AFE_IRQ1,
		.irq_cnt_reg = -1,
		.irq_cnt_shift = -1,
...
static const int memif_specified_irqs[MT8519_AFE_MEMIF_NUM] = {
	[MT8519_AFE_MEMIF_DLM] = MT8519_AFE_IRQ10,
	[MT8519_AFE_MEMIF_DL2] = MT8519_AFE_IRQ11,
	[MT8519_AFE_MEMIF_DL3] = MT8519_AFE_IRQ12,
	[MT8519_AFE_MEMIF_DL6] = MT8519_AFE_IRQ13,
	[MT8519_AFE_MEMIF_UL1] = MT8519_AFE_IRQ3,
	[MT8519_AFE_MEMIF_UL2] = MT8519_AFE_IRQ15,
	[MT8519_AFE_MEMIF_UL3] = MT8519_AFE_IRQ16,
	[MT8519_AFE_MEMIF_UL4] = MT8519_AFE_IRQ17,
	[MT8519_AFE_MEMIF_UL5] = MT8519_AFE_IRQ18,
	[MT8519_AFE_MEMIF_UL8] = MT8519_AFE_IRQ19,
	[MT8519_AFE_MEMIF_UL9] = MT8519_AFE_IRQ20,
	[MT8519_AFE_MEMIF_UL10] = MT8519_AFE_IRQ21,
};
...
int mt8519_reg_read(void *context, unsigned int reg, unsigned int *val)
{
...
int mt8519_reg_write(void *context, unsigned int reg, unsigned int val)
{
	struct mtk_base_afe *afe = context;
...
static const struct regmap_bus mt8519_afe_regmap_bus = {
	.fast_io = true,
	.reg_write = mt8519_reg_write,
	.reg_read = mt8519_reg_read,
	.val_format_endian_default = REGMAP_ENDIAN_LITTLE,
};
#endif

static const struct regmap_config mt8519_afe_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,
...
static irqreturn_t mt8519_afe_irq_handler(int irq, void *dev_id)
{
	struct mtk_base_afe *afe = dev_id;
	unsigned int val;
	...
	for (i = 0; i < MT8519_AFE_MEMIF_NUM; i++) {
	...
	force_apply_etdm2_out =
		...
		(etdm2->force_on[SNDRV_PCM_STREAM_CAPTURE] &&
		 (!probe ||
		  ...
		  MT8519_ETDM_FORCE_ON_DEFAULT) &&
		 (etdm2->clock_mode == MT8519_ETDM_SHARED_CLOCK));
...
static int mt8519_afe_handle_etdm_force_off(struct mtk_base_afe *afe)
{
	...
	if (force_apply_etdm1_in) {
		...
		mt8519_afe_disable_etdm(afe, MT8519_ETDM1,
			SNDRV_PCM_STREAM_CAPTURE);
		...
		mt8519_afe_disable_top_cg(afe, MT8519_TOP_CG_TDM_IN);
		mt8519_afe_disable_clk(afe, priv->clocks[MT8519_CLK_TDMIN_MCK]);
	...
	if (force_apply_etdm2_out) {
		...
		mt8519_afe_disable_etdm(afe, MT8519_ETDM2,
			SNDRV_PCM_STREAM_PLAYBACK);
		...
		mt8519_afe_disable_top_cg(afe, MT8519_TOP_CG_I2S_OUT);
		mt8519_afe_disable_clk(afe,
			priv->clocks[MT8519_CLK_I2SOUT_MCK]);
	...
	if (force_apply_etdm2_in) {
		...
		mt8519_afe_disable_top_cg(afe, MT8519_TOP_CG_I2S_IN);
		mt8519_afe_disable_clk(afe, priv->clocks[MT8519_CLK_I2SIN_MCK]);

		etdm2->force_on_status[SNDRV_PCM_STREAM_CAPTURE] = false;
	}

	if (rate % 8000)
		mt8519_afe_disable_clk(afe, priv->clocks[MT8519_CLK_FA2SYS]);
...
static int mt8519_afe_runtime_suspend(struct device *dev)
{
	return 0;
}

static int mt8519_afe_runtime_resume(struct device *dev)
{
	return 0;
}

static int mt8519_afe_dev_runtime_suspend(struct device *dev)
{
...
static int mt8519_afe_dev_runtime_resume(struct device *dev)
{
	struct mtk_base_afe *afe = dev_get_drvdata(dev);
...
static int mt8519_afe_pcm_copy(struct snd_pcm_substream *substream,
			       ...
			       mtk_sp_copy_f sp_copy)
{
	...
	mt8519_afe_set_clk_parent(afe,
		afe_priv->clocks[MT8519_CLK_TOP_AUD_BUS],
		afe_priv->clocks[MT8519_CLK_AUD_UNIVPLL_D6]);

	ret = sp_copy(substream, channel, hwoff, buf, bytes);

	mt8519_afe_set_clk_parent(afe,
		afe_priv->clocks[MT8519_CLK_TOP_AUD_BUS],
		afe_priv->clocks[MT8519_CLK_CLK26M]);

	return ret;
}


static int mt8519_afe_init_registers(struct mtk_base_afe *afe)
{
	size_t i;
	struct mt8519_afe_private *afe_priv = afe->platform_priv;
...
static bool mt8519_afe_validate_sram(struct mtk_base_afe *afe,
...
static void __iomem *mt8519_afe_sram_pa_to_va(struct mtk_base_afe *afe,
	u32 phy_addr)
{
	struct mt8519_afe_private *afe_priv = afe->platform_priv;
...
static void mt8519_afe_parse_of(struct mtk_base_afe *afe,
				struct device_node *np)
{
	...
	struct {
		...
		{ "dlm",	MT8519_AFE_MEMIF_DLM },
		{ "dl2",	MT8519_AFE_MEMIF_DL2 },
		{ "dl3",	MT8519_AFE_MEMIF_DL3 },
		{ "dl6",	MT8519_AFE_MEMIF_DL6 },
		{ "ul1",	MT8519_AFE_MEMIF_UL1 },
		{ "ul2",	MT8519_AFE_MEMIF_UL2 },
		{ "ul3",	MT8519_AFE_MEMIF_UL3 },
		{ "ul4",	MT8519_AFE_MEMIF_UL4 },
		{ "ul5",	MT8519_AFE_MEMIF_UL5 },
		{ "ul8",	MT8519_AFE_MEMIF_UL8 },
		{ "ul9",	MT8519_AFE_MEMIF_UL9 },
		{ "ul10",	MT8519_AFE_MEMIF_UL10 },
	};
	struct {
		char *name;
		unsigned int set;
		...
		{"etdm1-in", MT8519_ETDM1, SNDRV_PCM_STREAM_CAPTURE},
		{"etdm2-out", MT8519_ETDM2, SNDRV_PCM_STREAM_PLAYBACK},
		{"etdm2-in", MT8519_ETDM2, SNDRV_PCM_STREAM_CAPTURE},
	};
	struct {
		char *name;
		unsigned int set;
	} of_afe_gasrcs[] = {
		{"gasrc0", MT8519_GASRC0},
		{"gasrc1", MT8519_GASRC1},
		{"gasrc2", MT8519_GASRC2},
		{"gasrc3", MT8519_GASRC3},
	};
	...
	for (i = 0; i < ARRAY_SIZE(of_afe_etdms); i++) {
		...
		snprintf(prop, sizeof(prop),
			"mediatek,%s-force-master-clock",
			 of_afe_etdms[i].name);
		ret = of_property_read_u32(np, prop, &temps[0]);
		if (ret == 0) {
			if (temps[0])
				etdm_data->slave_mode[stream] = false;
			else
				etdm_data->slave_mode[stream] = true;
		}
	}
	...
	for (i = 0; i < ARRAY_SIZE(of_fe_table); i++) {
		bool valid_sram;
		struct mt8519_fe_dai_data *fe_data;
		...
		valid_sram = mt8519_afe_validate_sram(afe, val[0], val[1]);
		if (!valid_sram) {
		...
		fe_data->sram_vir_addr = mt8519_afe_sram_pa_to_va(afe,
	...
	for (i = 0; i < ARRAY_SIZE(of_afe_gasrcs); i++) {
		memset(val, 0, sizeof(val));
		gasrc_data = &afe_priv->gasrc_data[of_afe_gasrcs[i].set];

		snprintf(prop, sizeof(prop), "mediatek,%s-fix-rate",
			of_afe_gasrcs[i].name);
		ret = of_property_read_u32_array(np, prop, &val[0], 2);
		if (ret)
			continue;
		gasrc_data->fix_input_rate = val[0];
		gasrc_data->fix_output_rate = val[1];
	}
}

static int mt8519_afe_component_probe(struct snd_soc_component *component)
{
	int ret;
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(component);

	ret = mt8519_afe_add_controls(component);
	if (ret)
		return ret;

	ret = mt8519_afe_handle_etdm_force_on(afe, true);
	if (ret)
		return ret;
...
static snd_pcm_uframes_t mt8519_afe_pcm_pointer(struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	if (rtd->cpu_dai->id >= MT8519_AFE_MEMIF_NUM)
		return 0;
	return mtk_afe_pcm_pointer(substream);
}

const struct snd_pcm_ops mt8519_afe_pcm_ops = {
	.ioctl = snd_pcm_lib_ioctl,
	.pointer = mt8519_afe_pcm_pointer,
	.ack = mtk_afe_pcm_ack,
	.copy_user = mtk_afe_pcm_copy_user,
};

const struct snd_soc_component_driver mt8519_afe_component = {
	.name = AFE_PCM_NAME,
	.probe = mt8519_afe_component_probe,
	.pcm_new = mtk_afe_pcm_new,
	.pcm_free = mtk_afe_pcm_free,
	.ops = &mt8519_afe_pcm_ops,
	.dapm_widgets = mt8519_afe_pcm_widgets,
	.num_dapm_widgets = ARRAY_SIZE(mt8519_afe_pcm_widgets),
	.dapm_routes = mt8519_afe_pcm_routes,
	.num_dapm_routes = ARRAY_SIZE(mt8519_afe_pcm_routes),
};

static int mt8519_afe_pcm_dev_probe(struct platform_device *pdev)
{
	int ret, i, sel_irq;
	unsigned int irq_id;
	struct mtk_base_afe *afe;
	struct mt8519_afe_private *afe_priv;
	struct resource *res;
	...
	ret = devm_request_irq(afe->dev, irq_id, mt8519_afe_irq_handler,
	...
	ret = mt8519_afe_init_audio_clk(afe);
	if (ret) {
		dev_info(afe->dev, "mt8519_afe_init_audio_clk fail\n");
		return ret;
	}

#ifdef DEBUG_AFE_REGISTER_RW
	afe->regmap = devm_regmap_init(&pdev->dev,
		&mt8519_afe_regmap_bus, afe,
		&mt8519_afe_regmap_config);
#else
	...
	afe->memif_size = MT8519_AFE_MEMIF_NUM;
	afe->memif = devm_kcalloc(afe->dev, afe->memif_size,
				  sizeof(*afe->memif), GFP_KERNEL);
	if (!afe->memif)
		return -ENOMEM;

	afe->irqs_size = MT8519_AFE_IRQ_NUM;
	...
	afe->mtk_afe_hardware = &mt8519_afe_hardware;
	afe->memif_fs = mt8519_memif_fs;
	afe->irq_fs = mt8519_irq_fs;

	afe->copy = mt8519_afe_pcm_copy;
	...
	if (!pm_runtime_enabled(&pdev->dev)) {
		...
		ret = mt8519_afe_runtime_resume(&pdev->dev);
		if (ret)
			goto err_pm_disable;
	}
	...
	afe->reg_back_up_list = mt8519_afe_backup_list;
	afe->reg_back_up_list_num = ARRAY_SIZE(mt8519_afe_backup_list);
	afe->runtime_resume = mt8519_afe_runtime_resume;
	afe->runtime_suspend = mt8519_afe_runtime_suspend;
#ifdef CONFIG_MTK_HIFIXDSP_SUPPORT
	afe_priv->adsp_data.afe_memif_set = mt8519_adsp_afe_memif_set;
	afe_priv->adsp_data.afe_memif_enable = mt8519_adsp_afe_memif_enable;
	afe_priv->adsp_data.afe_init = mt8519_adsp_afe_init;
	afe_priv->adsp_data.afe_uninit = mt8519_adsp_afe_uninit;
	afe_priv->adsp_data.afe_memif_init = mt8519_afe_memif_init;
	afe_priv->adsp_data.afe_memif_uninit = mt8519_afe_memif_uninit;
	afe_priv->adsp_data.afe_sram_get = mt8519_afe_sram_get;
	afe_priv->adsp_data.afe_sram_put = mt8519_afe_sram_put;
#endif
	...
	mt8519_afe_parse_of(afe, pdev->dev.of_node);

	ret = snd_soc_register_component(&pdev->dev,
					 &mt8519_afe_component,
					 mt8519_afe_pcm_dais,
					 ARRAY_SIZE(mt8519_afe_pcm_dais));
	if (ret)
		goto err_component;

	mt8519_afe_init_registers(afe);

	mt8519_afe_init_debugfs(afe);

	dev_info(&pdev->dev, "MT8519 AFE driver initialized.\n");
	return 0;
...
static int mt8519_afe_pcm_dev_remove(struct platform_device *pdev)
{
	struct mtk_base_afe *afe = platform_get_drvdata(pdev);
	struct mt8519_afe_private *afe_priv = afe->platform_priv;

	mt8519_afe_cleanup_debugfs(afe);
	...
	if (!pm_runtime_status_suspended(&pdev->dev))
		mt8519_afe_runtime_suspend(&pdev->dev);

	snd_soc_unregister_component(&pdev->dev);
	return 0;
}

static const struct of_device_id mt8519_afe_pcm_dt_match[] = {
	{ .compatible = "mediatek,mt8519-afe-pcm", },
	{ }
};
MODULE_DEVICE_TABLE(of, mt8519_afe_pcm_dt_match);

static const struct dev_pm_ops mt8519_afe_pm_ops = {
	SET_RUNTIME_PM_OPS(mt8519_afe_dev_runtime_suspend,
			   mt8519_afe_dev_runtime_resume, NULL)
};

static struct platform_driver mt8519_afe_pcm_driver = {
	.driver = {
		   .name = "mt8519-afe-pcm",
		   .of_match_table = mt8519_afe_pcm_dt_match,
#if IS_ENABLED(CONFIG_PM)
		   .pm = &mt8519_afe_pm_ops,
#endif
	},
	.probe = mt8519_afe_pcm_dev_probe,
	.remove = mt8519_afe_pcm_dev_remove,
};

module_platform_driver(mt8519_afe_pcm_driver);
