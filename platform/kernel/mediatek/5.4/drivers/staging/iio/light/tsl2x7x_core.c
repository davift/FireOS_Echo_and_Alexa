...
#define TSL2X7X_CMD_AUTO_INC           0x20
...
/* tsl2x7x configuration reg masks */
#define TSL2X7X_CONF_AGL               0x04
#define TSL2X7X_CONF_WLONG             0x02
...
#define TSL2X7X_LONG_WAIT_MULTIPLIER 12
...
/*
 * This MACRO defines the size of the string used to locally store
 * the "alscal" field from IDME.
 *
 * The size needs to be >= the max length of the "alscal" field.
 *
 * Considering a hypothetical "alscal" string,
 * "FFFF,FFFF,FFFF,FFFF,FFFF,FFFF,FFFF,FFFF,FFFF",
 * 65 was deemed large enough.
 *
 * */
#define ALSCAL_COPY_SIZE (65)
...
static const struct tsl2x7x_settings tsl2x7x_default_settings = {
	...
	.interrupts_en = 0x18 /* enable interrupt and wait timer */,
	.prox_thres_low  = 0,
	.prox_thres_high = 512,
...
static int tsl2x7x_get_lux(struct iio_dev *indio_dev)
{
	...
	ret = i2c_smbus_read_i2c_block_data(
		chip->client,
		TSL2X7X_CMD_REG | TSL2X7X_CMD_AUTO_INC | TSL2X7X_ALS_CHAN0LO, 4,
		buf);
...
static int tsl2x7x_chip_on(struct iio_dev *indio_dev)
{
	...
	ret = i2c_smbus_write_i2c_block_data(
		chip->client, TSL2X7X_CMD_REG | TSL2X7X_CMD_AUTO_INC,
		TSL2X7X_MAX_CONFIG_REG, chip->tsl2x7x_config);
	if (ret < 0) {
...
int tsl2x7x_invoke_change(struct iio_dev *indio_dev)
{
	...
	device_status = chip->tsl2x7x_chip_status;
...
static ssize_t tsl2x7x_als_wait_time_en_show(struct device *dev,
					  struct device_attribute *attr,
					  char *buf)
{
	struct tsl2X7X_chip *chip = iio_priv(dev_to_iio_dev(dev));
	...
	wen = chip->tsl2x7x_settings.interrupts_en & TSL2X7X_CNTL_WAIT_TMR_ENBL;
	return snprintf(buf, PAGE_SIZE, "%d\n", !!wen);
}

static ssize_t tsl2x7x_als_wait_time_en_store(struct device *dev,
					     struct device_attribute *attr,
					     const char *buf, size_t len)
{
	struct iio_dev *indio_dev = dev_to_iio_dev(dev);
...
static ssize_t tsl2x7x_als_wait_time_show(struct device *dev,
					  struct device_attribute *attr,
					  char *buf)
{
	struct tsl2X7X_chip *chip = iio_priv(dev_to_iio_dev(dev));
	...
	wait_time = TSL2X7X_MIN_ITIME *
		(0x100 - (u8)chip->tsl2x7x_settings.wait_time);

	/* when WLONG is asserted, wait time is 12x longer */
	if (chip->tsl2x7x_settings.prox_config & TSL2X7X_CONF_WLONG)
		wait_time *= TSL2X7X_LONG_WAIT_MULTIPLIER;
...
static ssize_t tsl2x7x_als_wait_time_store(struct device *dev,
					     struct device_attribute *attr,
					     const char *buf, size_t len)
{
	struct iio_dev *indio_dev = dev_to_iio_dev(dev);
	...
	chip->tsl2x7x_settings.wait_time = (0x100 - (u8)wait_time) & 0xFF;
					    ...
					    char *buf)
{
	...
	pers_value = chip->tsl2x7x_settings.persistence & 0x0F;
					     ...
					     const char *buf, size_t len)
{
	...
	chip->tsl2x7x_settings.persistence = pers_val;
...
static DEVICE_ATTR(in_intensity_wait_time_enable, S_IRUGO | S_IWUSR,
		tsl2x7x_als_wait_time_en_show, tsl2x7x_als_wait_time_en_store);

static DEVICE_ATTR(in_intensity_wait_time, S_IRUGO | S_IWUSR,
		tsl2x7x_als_wait_time_show, tsl2x7x_als_wait_time_store);
...
static irqreturn_t tsl2x7x_event_handler(int irq, void *private)
{
	...
	if (!(value & (TSL2X7X_STA_ALS_INTR | TSL2X7X_STA_PRX_INTR)))
		return IRQ_NONE;
...
static void idme_set_alscal_calibrated_values(void)
{
	...
	char alscal_copy[ALSCAL_COPY_SIZE];
	char *found;
	...
	/* Multiply by 1000 to aid in fixed point division */
...
static void perform_autozero(struct tsl2X7X_chip *chip)
{
	...
	/* Restore Interrupt if enabled */
	utmp |= chip->tsl2x7x_settings.interrupts_en;
	ret = i2c_smbus_write_byte_data(chip->client,
					TSL2X7X_CMD_REG | TSL2X7X_CNTRL, utmp);
	if (ret < 0) {
		dev_err(&chip->client->dev, "i2c_smbus_write_byte() to reg %x "
...
static void tsl2x7x_shutdown(struct i2c_client *client)
{
