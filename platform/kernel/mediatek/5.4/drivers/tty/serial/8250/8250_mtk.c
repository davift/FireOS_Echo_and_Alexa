...
static int mtk8250_pm_notify(struct notifier_block *nb, unsigned long event, void *_unused)
{
	struct mtk8250_data *data = container_of(nb, struct mtk8250_data, pm_nb);
	struct uart_8250_port *up = serial8250_get_port(data->line);

	if (event == PM_POST_SUSPEND)
		pinctrl_pm_select_default_state(up->port.dev);
...
static int mtk8250_probe(struct platform_device *pdev)
{
	...
	data->pm_nb.notifier_call = mtk8250_pm_notify;
	err = register_pm_notifier(&data->pm_nb);
	if (err)
		return err;
