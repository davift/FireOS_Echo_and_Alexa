...
static const int mt8519_bank_data[MT8519_NUM_ZONES][MT8519_NUM_SENSORS_PER_ZONE] = {
...
static const int mt8519_mux_values[MT8519_NUM_SENSORS] = { 0, 1, 2, 3, 4, 5 };
static int mt8519_vts_values[MT8519_NUM_SENSORS] = { 260, 260, 260, 260, 260, 260 };
...
static void __attribute__((unused)) thermal_rst_trigger(struct soc_thermal *mt)
{
	...
	for (i = 0; i < mt->therm_rst_num; i++) {
		...
		writel(temp, mt->therm_rst_ctrl[i].rst_base +
			mt->therm_rst_ctrl[i].rst_set_offset);
		pr_info("%s: 0x%x, 0x%x\n", __func__,
			(int)(mt->therm_rst_ctrl[i].rst_base +
			mt->therm_rst_ctrl[i].rst_set_offset),
					    ...
					    struct soc_thermal *mt)
{
	...
	if (IS_ERR(cell)) {
		dev_err(dev, "Failed to read thermal efuse data\n");
	...
	buf[0] = get_devinfo_with_index(149);
	buf[1] = get_devinfo_with_index(150);
	buf[2] = get_devinfo_with_index(151);
	buf[3] = get_devinfo_with_index(152);
