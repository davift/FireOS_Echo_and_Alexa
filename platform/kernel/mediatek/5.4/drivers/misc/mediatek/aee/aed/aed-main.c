...
#if defined(CONFIG_MTK_AEE_SAVE_DEBUGINFO_RESERVED_BUFFER)
...
static struct kedump_reserved_buffer *kedump_buffer;
...
{
	if (kedump_buffer->sig == KEDUMP_BUFFER_SIG) {
		seq_write(m, kedump_buffer, CONFIG_MTK_AEE_SAVE_DEBUGINFO_SIZE);
	} else {
		kedump_buffer->sig = 0xffffffff;
...
static int __init aed_init(void)
{
	...
	kedump_buffer = remap_lowmem(CONFIG_MTK_AEE_SAVE_DEBUGINFO_ADDR, CONFIG_MTK_AEE_SAVE_DEBUGINFO_SIZE);
        if (kedump_buffer == NULL) {
                pr_err("kedump_buffer: remap_lowmem failed\n");
                return 0;
        }
#endif
