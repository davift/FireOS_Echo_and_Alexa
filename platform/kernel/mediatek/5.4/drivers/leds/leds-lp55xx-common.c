...
void lp55xx_enable(struct lp55xx_platform_data *pdata, bool enable)
{
	if (gpio_is_valid(pdata->enable_gpio))
		gpio_set_value(pdata->enable_gpio, enable);
}
EXPORT_SYMBOL_GPL(lp55xx_enable);
						      ...
						      struct device_node *np)
{
	...
	pdata->play_boot_animation = of_property_read_bool(np, "play-boot-animation");
