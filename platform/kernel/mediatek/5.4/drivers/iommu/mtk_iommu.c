...
static irqreturn_t mtk_iommu_isr(int irq, void *dev_id)
{
	...
	unsigned int fault_larb = 0, fault_port, sub_comm = 0;
	...
	for (i = 0, tf_iova_tmp = fault_iova; i < TF_IOVA_DUMP_NUM; i++) {
		...
		pr_info("[iommu_debug] %s_iommu error, index:%d, falut_iova:0x%lx, fault_pa(pg):%pa\n",
			mtk_iommu_is_apu(data) ? "apu" : "mm",
	...
	report_custom_iommu_fault(fault_iova, fault_pa, regval,
				  mtk_iommu_is_apu(data) ? true : false);
#endif
...
#ifdef CONFIG_MTK_IOMMU_MISC_SEC
				   ...
				   struct device *dev)
{
	...
	if (!data->m4u_dom) {
		...
		if (ret < 0) {
			dev_err(data->dev, "%s pm get failed(%d) at %s attach\n",
...
static int mtk_iommu_of_xlate(struct device *dev, struct of_phandle_args *args)
{
	...
	/* Get the m4u device: The IOMMU must be unique for which the
	 * device belongs.
	 */
			   ...
			   struct component_match **match)
{
	...
	for (i = 0; i < larb_nr; i++) {
		...
		/* Add link for smi-common and m4u once is ok. */
		if (i || !dev->pm_domain)
			continue;
...
static const struct mtk_iommu_plat_data mt8519_data_infra = {
	.m4u_plat     = M4U_MT8519,
...
static const struct of_device_id mtk_iommu_of_ids[] = {
	...
	{ .compatible = "mediatek,mt8519-iommu-infra",
	  .data = &mt8519_data_infra},
	{}
};
