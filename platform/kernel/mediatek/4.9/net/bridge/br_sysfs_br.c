...
/* eero Change
 * Use a mark to avoid sending bridged packets to iptables.
 */
static ssize_t eero_mark_to_call_iptables_show(
	struct device *d, struct device_attribute *attr, char *buf)
{
	struct net_bridge *br = to_bridge(d);
	return sprintf(buf, "%x\n", br->eero_nf_ext.mark_to_call_iptables);
}

static int eero_set_mark_to_call_iptables(
	struct net_bridge *br, unsigned long val)
{
	br->eero_nf_ext.mark_to_call_iptables = (u32) val;
	return 0;
}

static ssize_t eero_mark_to_call_iptables_store(
	struct device *d, struct device_attribute *attr, const char *buf,
	size_t len)
{
	return store_bridge_parm(d, buf, len, eero_set_mark_to_call_iptables);
}

static DEVICE_ATTR_RW(eero_mark_to_call_iptables);
#endif
...
#ifdef CONFIG_BIFROST_MESH
	&dev_attr_eero_mark_to_call_iptables.attr,
#endif
#endif
#ifdef CONFIG_BRIDGE_VLAN_FILTERING
