				      ...
				      const struct nf_hook_state *state)
{
	...
	if (IS_IPV6(skb) || IS_VLAN_IPV6(skb) || IS_PPPOE_IPV6(skb)) {
		...
		if (!brnf_call_ip6tables && !br->nf_call_ip6tables && !br->eero_nf_ext.mark_to_call_iptables)
			return NF_ACCEPT;
		if (likely(!(skb->mark & br->eero_nf_ext.mark_to_call_iptables)))
			return NF_ACCEPT;
#else
...
#ifdef CONFIG_BIFROST_MESH
	if (!brnf_call_iptables && !br->nf_call_iptables && !br->eero_nf_ext.mark_to_call_iptables)
		return NF_ACCEPT;
	if (likely(!(skb->mark & br->eero_nf_ext.mark_to_call_iptables)))
		return NF_ACCEPT;
#else
