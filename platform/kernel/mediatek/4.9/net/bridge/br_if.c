...
#ifdef CONFIG_BIFROST_MESH
void br_eero_set_hairpin_mode(struct net_device *dev,
			      struct net_bridge_port *p)
{
	/* enabling hairpin mode on ap0's bridge port by default */
	if (!strcmp(dev->name, CONFIG_SOFTAP_INTERFACE_NAME))
		p->flags |= BR_HAIRPIN_MODE;
}
#else
static inline void br_eero_set_hairpin_mode(struct net_device *dev,
					    struct net_bridge_port *p)
{
}
#endif
...
int br_add_if(struct net_bridge *br, struct net_device *dev)
{
	...
	br_eero_set_hairpin_mode(dev, p);
