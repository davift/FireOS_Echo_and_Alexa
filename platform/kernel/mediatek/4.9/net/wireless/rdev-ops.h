...
static inline int rdev_add_mpp(struct cfg80211_registered_device *rdev,
			       struct net_device *dev, u8 *dest, u8 *proxy)
{
	int ret;

	trace_rdev_add_mpp(&rdev->wiphy, dev, dest, proxy);
	ret = rdev->ops->add_mpp(&rdev->wiphy, dev, dest, proxy);
	trace_rdev_return_int(&rdev->wiphy, ret);
	return ret;
}
#endif
...
#ifdef CONFIG_BIFROST_MESH
	ret = rdev->ops->dump_mpath(&rdev->wiphy, dev, idx, pathdev, dst,
				    next_hop, pinfo, mbss);
#else
...
#ifdef CONFIG_BIFROST_MESH
static inline int
rdev_get_mesh_eero_ext_config(struct cfg80211_registered_device *rdev,
		     struct net_device *dev, struct mesh_config *conf)
{
	int ret;
	trace_rdev_get_mesh_eero_ext_config(&rdev->wiphy, dev);
	ret = rdev->ops->get_mesh_eero_ext_config(&rdev->wiphy, dev, conf);
	trace_rdev_return_int_mesh_config(&rdev->wiphy, ret, conf);
	return ret;
}
#endif
...
#ifdef CONFIG_BIFROST_MESH
static inline int
rdev_update_mesh_eero_ext_config(struct cfg80211_registered_device *rdev,
			struct net_device *dev, u32 mask,
			const struct mesh_config *nconf)
{
	int ret;
	trace_rdev_update_mesh_eero_ext_config(&rdev->wiphy, dev, mask, nconf);
	ret = rdev->ops->update_mesh_eero_ext_config(&rdev->wiphy, dev, mask, nconf);
	trace_rdev_return_int(&rdev->wiphy, ret);
	return ret;
}
#endif
