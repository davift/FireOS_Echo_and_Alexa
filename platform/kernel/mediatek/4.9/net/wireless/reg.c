...
#ifdef CONFIG_BIFROST_MESH
static enum reg_request_treatment
reg_update_wiphy(struct wiphy *wiphy)
{
...
#else
static inline enum reg_request_treatment
reg_update_wiphy(struct wiphy *wiphy)
{
	return REG_REQ_OK;
}
				    ...
				    enum nl80211_reg_initiator initiator)
{
	...
	reg_call_notifier(wiphy, lr);
	/* update phy level regulatory rules */
	reg_update_wiphy(wiphy);
}
...
#else
static inline void wiphy_all_share_dfs_chan_state(struct wiphy *wiphy)
{
	return;
}
#endif
...
#ifdef CONFIG_BIFROST_MESH
	/* ignore country code in IE, since it's for end user stations
	 * and we don't run stations for end users
	 */
...
#define REG_PRE_CAC_MAX_TIMEOUT -1
...
unsigned long regulatory_get_pre_cac_timeout(struct wiphy *wiphy)
{
	 ...
	 */
	return REG_PRE_CAC_MAX_TIMEOUT;
}
...
static void reg_propagate_dfs_state(struct reg_dfs_state *dfs_state_info)
{
	...
	list_for_each_entry(rdev, &cfg80211_rdev_list, list) {
		...
		/* cancel CAC now if it is running on same channel. Open code
		 * to avoid the CAC-ABORT event, and we don't have an API to
		 * the driver anyway. This is to avoid a redundant CAC-FINISHED
		 * when the timer expires */
		if (dfs_state_info->event == NL80211_RADAR_CAC_FINISHED) {
			list_for_each_entry(wdev, &rdev->wiphy.wdev_list, list) {
				if (!wdev->cac_started ||
				    wdev->chandef.center_freq1 != chandef.center_freq1)
					continue;
...
#ifdef CONFIG_BIFROST_MESH
/* for upstream patch - modified */
static void eero_cancel_reg_propagate_dfs_work(void)
{
	cancel_work_sync(&reg_propagate_dfs_state_work);
}

static void eero_reg_dfs_state_list_purge(void)
{
...
#else
static inline void eero_cancel_reg_propagate_dfs_work(void)
{
}
...
void regulatory_exit(void)
{
	...
	eero_cancel_reg_propagate_dfs_work();
