...
int mtd_reset_device(struct mtd_info *mtd)
{
	if (!mtd->_reset_device)
		return -EOPNOTSUPP;

	return mtd->_reset_device(mtd);
}
EXPORT_SYMBOL_GPL(mtd_reset_device);
...
{
	if (!IS_ENABLED(CONFIG_MTD_SUPPORT_UNMARK))
		return -EINVAL;
