...
/**
 * ubi_io_reset_device - reset the flash device.
 * @ubi: UBI device description object
 *
 * This function returns zero if the reset device operation is success,
 * otherwise, the reset operation is fail or not support.
 */
int ubi_io_reset_device(const struct ubi_device *ubi)
{
	...
	ret = mtd_reset_device(mtd);
	if (ret == -EOPNOTSUPP)
		ubi_warn(ubi, "device reset operation is not support");
	else if (ret)
		ubi_err(ubi, "device reset operation is fail %d", ret);
