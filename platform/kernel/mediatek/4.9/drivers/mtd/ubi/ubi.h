...
/*
 * In case of program fail, UBI tries to repeat the erase/program several
 * times. The below constant defines how many times UBI retry reset device.
 */
#define UBI_IO_RESET_RETRIES 15

/*
 * The number of erase counts to increase when torture test fails during reset
 * retries.
 */
#define UBI_TORTURE_FAIL_RESET_EC_INC 3
...
struct ubi_debug_info {
	...
	unsigned int emulate_io_failures2:2;
	unsigned int sync_erase_retry_successes;
	unsigned int sync_erase_retry_failures;
	...
	struct dentry *dfs_emulate_io_failures2;
	struct dentry *dfs_sync_erase_retry_successes;
	struct dentry *dfs_sync_erase_retry_failures;
...
int ubi_io_reset_device(const struct ubi_device *ubi);
...
{
	/*
	 * Only zero out valid data area.  For operation with a randomizer disabled, writing zero
	 * values is a waste of time and energy as it causes higher program time and energy usage.
	 * If randomizer is enabled, programmed zero or one values don't matter so either is fine.
	 */
	if (buf) {
		memset(buf, 0, UBI_VID_HDR_SIZE);
		memset(buf + UBI_VID_HDR_SIZE, 0xFF, ubi->vid_hdr_alsize - UBI_VID_HDR_SIZE);
	}
