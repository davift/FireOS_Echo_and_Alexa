		      ...
		      int torture)
{
	...
	int retries_left = UBI_IO_RESET_RETRIES;

	dbg_wl("erase PEB %d, old EC %llu torture=%d", e->pnum, ec, torture);
...
retry:
	...
	/*
	 * emulate_io_failures2 is consist of two bits; 0 and 1.
	 * BIT(0) = emulate ubi_io_write_ec_hdr() failure.
	 * BIT(1) = emulate ubi_io_sync_erase() failure with torture = 1.
	 */

	if (ubi->dbg.emulate_io_failures2 & BIT(1)) {
		ubi->dbg.emulate_io_failures2 &= ~BIT(1);
		ubi_err(ubi, "emulate_io_failures2=0x%x (ubi_io_sync_erase) torture=%d->1",
			ubi->dbg.emulate_io_failures2, torture);
		torture = 1;
		err = -EIO;
	}

	if (err < 0) {
		if (err != -EROFS && torture == 1 && retries_left-- > 0) {
			ubi_err(ubi, "ubi_io_sync_erase err=%d torture=1 retries_left=%d at PEB %d, EC %llu",
				err, retries_left, e->pnum, ec);

			ret = ubi_io_reset_device(ubi);
			if (ret == -EOPNOTSUPP)
				ubi_warn(ubi, "No retries, not supported.");
			else {
				ec += UBI_TORTURE_FAIL_RESET_EC_INC;
				goto retry;
			}
		}
		goto out_free;
	...
	if (ubi->dbg.emulate_io_failures2 & BIT(0)) {
		ubi->dbg.emulate_io_failures2 &= ~BIT(0);
		ubi_err(ubi, "emulate_io_failures2=0x%x (ubi_io_write_ec_hdr) at PEB %d, EC %llu",
			ubi->dbg.emulate_io_failures2, e->pnum, ec);
		err = -EIO;
	}

	if (err) {
		ubi_err(ubi, "write ec hdr err=%d torture=%d retries_left=%d",
			err, torture, retries_left);

		if (retries_left-- > 0) {
			ret = ubi_io_reset_device(ubi);
			if (ret == -EOPNOTSUPP) {
				ubi_warn(ubi,
					 "No retries, not supported.");
				goto out_free;
			}

			ubi_err(ubi, "retrying erase and write_ec");
			goto retry;
		}
...
out_free:
	if (retries_left != UBI_IO_RESET_RETRIES) {
		ubi_err(ubi, "ubi retries_left=%d err=%d",
			retries_left, err);

		spin_lock(&ubi->wl_lock);
		if (err)
			ubi->dbg.sync_erase_retry_failures += 1;
		else
			ubi->dbg.sync_erase_retry_successes += 1;
		spin_unlock(&ubi->wl_lock);
	}
