...
static int nand_spi_device_reset(struct nand_base *nand)
{
	struct device_spi *dev = device_to_spi(nand->dev);
	struct nand_spi *spi = base_to_spi(nand);
	u8 mask;
	...
	if (!dev->dev.extra_reset)
		return -EOPNOTSUPP;

	ret = nand_spi_reset(nand);
	if (ret) {
		pr_err("%s, %d, device reset error: %d",
		       __func__, __LINE__, ret);
		return ret;
	}

	spi->operation_resets++;
	...
	if (ret) {
		pr_err("%s, %d, unlock block error: %d",
		       __func__, __LINE__, ret);
		return ret;
	}
...
read_retry:
	...
	if (ret < 0) {
		if (retry_cnt < SPI_NAND_CALIBRATION_CNT) {
			if (retry_cnt < (SPI_NAND_CALIBRATION_CNT / 2))
				args = SPI_NAND_DEFAULT_DELAY_MODE - (retry_cnt + 1);
			else
				args = SPI_NAND_DEFAULT_DELAY_MODE + (retry_cnt - 1);
			nfi->nfi_ctrl(nfi, SNFI_CTRL_DELAY_MODE, &args);
			pr_info("%s @ %d, read %d (%d) change delay mode to %d (%d)\n",
				__func__, __LINE__, row, ret, args, retry_cnt);
			retry_cnt++;
			goto read_retry;
		} else if (retry_cnt == SPI_NAND_CALIBRATION_CNT) {
			args = SPI_NAND_DEFAULT_DELAY_MODE;
			nfi->nfi_ctrl(nfi, SNFI_CTRL_DELAY_MODE, &args);
			pr_info("%s @ %d, read %d (%d) change delay mode to %d (%d)\n",
				__func__, __LINE__, row, ret, args, retry_cnt);
			retry_cnt++;
		...
		pr_err("%s @ %d, read fail %d (%d) retry count %d\n",
				__func__, __LINE__, row, ret, retry_cnt);
		return ret;
	} else {
		if (retry_cnt)
			pr_err("%s @ %d, read %d (%d) pass change delay mode to %d (%d)\n",
				__func__, __LINE__, row, ret, args, retry_cnt);
	}
...
static int nand_spi_program_page(struct nand_base *nand, int row)
{
	...
	ret = nand_spi_wait_ready(nand, READY_TIMEOUT);

#ifdef CONFIG_MTD_NAND_MTK_WORN_BAD
	sim_worn_page_addr = mtk_nfc_check_worn_bad_test();
	if (row == sim_worn_page_addr)
		ret = -EIO;
#endif
			      ...
			      void *args)
{
	...
	switch (cmd) {
	case CHIP_CTRL_RESET_DEVICE:
		ret = nand_spi_device_reset(nand);
		break;

	case CHIP_CTRL_GET_RESET_CNT:
		*(int *)args = spi->operation_resets;
		break;

	case CHIP_CTRL_CLR_RESET_CNT:
		spi->operation_resets = 0;
		break;
	...
	case CHIP_CTRL_GET_STATUS:
		memcpy(args, &chip->status, sizeof(struct chip_status));
		break;
