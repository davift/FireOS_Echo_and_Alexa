...
#ifdef CONFIG_BIFROST_MESH
#define MESH_FLAGS_UNICAST_FLOOD	0x8

/* XXX: This is a work around given by QCA for dropping corrupted mesh packets.
 * If mesh header u8 value is bigger than MESH_CTRL_FLAGS_MAXVAL that means
 * there are some unexpected bits set in the flags. Remember that
 * MESH_FLAGS_AE_A4 and MESH_FLAGS_AE_A5_A6 cannot be set at the same time. But
 * MESH_FLAGS_UNICAST_FLOOD can be set together with one of them.
 */
#define MESH_CTRL_FLAGS_MAXVAL		(MESH_FLAGS_AE_A5_A6| \
					MESH_FLAGS_UNICAST_FLOOD)
#endif
...
/* convert time units */
#ifdef CONFIG_BIFROST_MESH
#define TU_TO_JIFFIES(x)	(usecs_to_jiffies(ieee80211_tu_to_usec(x)))
#else
