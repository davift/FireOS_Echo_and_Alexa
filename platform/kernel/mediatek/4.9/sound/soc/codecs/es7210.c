...
struct es7210_priv {
	...
	bool regulator_available;
};

/* Global ADC reset controller */
struct regulator *g_reset_adc_regulator;
...
/* Function to toggle RST_MIC_ADC_L */
int es7210_adc_reset(bool enable)
{
	int ret = 0;

	pr_info("%s+ %d %p\n", __func__, enable, g_reset_adc_regulator);

	if (!g_reset_adc_regulator) {
		pr_err("%s- ES7210 g_reset_adc_regulator is NULL\n", __FUNCTION__);
		return REGULATOR_NULL;
	}

	if (!enable) {
		ret = regulator_disable(g_reset_adc_regulator);
		if (ret) {
			pr_err("%s- failed to disable %p  (%d)\n", __FUNCTION__,
				g_reset_adc_regulator, ret);
			return REGULATOR_ERROR;
		}

		pr_info("%s- %p\n", __func__, g_reset_adc_regulator);
		return REGULATOR_TURNED_OFF;
	}

	if (regulator_is_enabled(g_reset_adc_regulator)) {
		pr_info("%s- %p already enabled\n", __func__, g_reset_adc_regulator);
		return REGULATOR_ALREADY_ON;
	}

	ret = regulator_enable(g_reset_adc_regulator);
	if (ret) {
	...
	pr_info("%s- %p\n", __func__, g_reset_adc_regulator);
	return REGULATOR_TURNED_ON;
}
EXPORT_SYMBOL(es7210_adc_reset);
...
// do not use digital mute, it causes erratic behavior.
#if 0
	if (mute) {
...
static int es7210_suspend(struct snd_soc_codec *codec)
{
	...
	regcache_cache_only(es7210->regmap, true);
	regcache_mark_dirty(es7210->regmap);
...
static int es7210_resume(struct snd_soc_codec *codec)
{
	...
	regcache_cache_only(es7210->regmap, false);
	regcache_sync(es7210->regmap);

	es7210_multi_chips_update_bits(0x01, 0x1a, 0x18);
	es7210_multi_chips_write(0x4b, 0x00);
	es7210_multi_chips_update_bits(0x01, 0x7F, 0x40);
	es7210_multi_chips_write(0x4c, 0x00);
	es7210_multi_chips_write(0x00, 0x01);
//check if 10ms sleep is needed here
...
//HARDCODE, TODO DEE-263881
	es7210_multi_chips_write(0x02, 0x81);
	es7210_multi_chips_write(0x06, 0x4);
	es7210_multi_chips_write(0x03, 0x2);
	/*
	...
	//HARDCODE, TODO DEE-263881
	es7210_multi_chips_update_bits(ES7210_MODE_CFG_REG08, 0xFF, 0x17);
...
struct snd_soc_codec_driver soc_codec_dev_es7210 = {
	.probe = es7210_probe,
			    ...
			    const struct i2c_device_id *i2c_id)
{
	...
	g_reset_adc_regulator = devm_regulator_get(&i2c->dev,
						"reset-adc-regulator");
	if (IS_ERR(g_reset_adc_regulator)) {
		pr_err("%s: ES7210 failed to get adc regulator\n", __FUNCTION__);
		g_reset_adc_regulator = NULL;
	}

	i2c_set_clientdata(i2c, es7210);
...
*/
		ret = snd_soc_register_codec(&i2c->dev,
			&soc_codec_dev_es7210, es7210_dai[i2c_id->driver_data], 1);
	...
	if (device_property_read_bool(&i2c->dev, "disable-suspend")) {
		es7210->disable_suspend = true;
	}
...
#if ES7210_CHANNELS_MAX > 0
	{.compatible = "es,es7210",},	//es7210_0
#endif

#if ES7210_CHANNELS_MAX > 4
	{.compatible = "es,es7210_1",},	//es7210_1
#endif

#if ES7210_CHANNELS_MAX > 8
	{.compatible = "es,es7210_2",},	//es7210_2
#endif

#if ES7210_CHANNELS_MAX > 12
	{.compatible = "es,es7210_3",},	//es7210_3
#endif
};
#endif
