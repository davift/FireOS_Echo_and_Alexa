...
static const char *EVEREST_ADC_DAI_NAME = "es7210-aif";
...
struct mt8512_brownie_priv {
	...
	bool lineout_gain;
	bool lineout_enable;
	bool reset_adc_regulator;
	bool enable_tdm_clks;
...
static int brownie_i2s_enable_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol);

static int brownie_i2s_enable_set(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol);
...
static const struct snd_kcontrol_new mt8512_brownie_controls[] = {
	...
	SOC_SINGLE_BOOL_EXT("Force I2S Enable",
		0,
		brownie_i2s_enable_get,
		brownie_i2s_enable_set),
...
static int mt8512_brownie_hw_free(struct snd_pcm_substream * substream) {
	struct snd_soc_pcm_runtime *rtd;
	...
	/* turn off bclk before putting AMP in sleep state */
	if (strstr(rtd->codec_dai->name, "tas5805m")) {
		mutex_lock(&priv->amp_lock);
		if (!priv->force_i2s_enable) {
			error = mt8512_afe_force_etdm2_out_enable(rtd->platform, 0);
...
static struct snd_soc_ops mt8512_brownie_etdm_ops = {
	...
	.hw_free = mt8512_brownie_hw_free,
};
	...
	struct mt8512_brownie_priv *priv)
{
	...
	for (i = 0; i < num_codecs; i++) {
		...
		if (strcmp(EVEREST_ADC_DAI_NAME,
			dai_link->codecs[i].dai_name) == 0) {
			pr_info("%s Found EVEREST\n", __func__);
			priv->reset_adc_regulator = true;
		} else
...
single_codec:
	...
	if (strcmp(EVEREST_ADC_DAI_NAME,
		dai_link->codec_dai_name) == 0) {
		pr_info("%s Found EVEREST\n", __func__);
		priv->reset_adc_regulator = true;
	}

	pr_info("%s+ Single codec_name %s\n", __func__,
				...
				struct device_node *np)
{
	...
	for (i = 0; i < ARRAY_SIZE(of_dai_links_etdm); i++) {
		...
		/* parse etdm1 late clk enables */
		if (of_property_read_bool(np,
			"mediatek,etdm1-late-clk-enable")) {
			priv->enable_tdm_clks = true;
			dev_info(card->dev, "%s Found etdm1-late-clk-enable!\n",
				__func__);
		}

		mt8512_brownie_parse_of_codec(card->dev, np, dai_link,
			of_dai_links_etdm[i].name, priv);
	...
	for (i = 0; i < ARRAY_SIZE(of_dai_links_int_adda); i++) {
		...
		mt8512_brownie_parse_of_codec(card->dev, np, dai_link,
			of_dai_links_int_adda[i].name, priv);
	}
#endif
...
static int mt8512_brownie_dev_probe(struct platform_device *pdev)
{
	...
	int reset_adc_status = REGULATOR_NULL;
	...
	if (priv->reset_adc_regulator) {
		/* Everest ADC: reset before setting TDM Clock mode */
		reset_adc_status = es7210_adc_reset(true);
		pr_info("%s es7210 adc reset %d\n", __func__,
			reset_adc_status);

		if (reset_adc_status > REGULATOR_TURNED_OFF)
			select_brownie_pin_mux(PIN_ETDM1_CLK_ENABLE,
				card);
	} else if (priv->enable_tdm_clks) {
		/* AKM ADC: Set TDM Mode */
		select_brownie_pin_mux(PIN_ETDM1_CLK_ENABLE,
			card);
	}
