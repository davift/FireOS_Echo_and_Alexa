/*
 * mt8512-laser.c  --  MT8512 laser machine driver
 *
...
#include "../../codecs/tlv320adc5140.h"
...
static const char * const mt8512_laser_pin_str[PIN_STATE_MAX] = {
	"default",
	"line_mute_on",
	"line_mute_off",
	"line_out_sel",
...
struct mt8512_laser_etdm_ctrl_data {
...
struct mt8512_laser_dmic_ctrl_data {
	unsigned int fix_rate;
	unsigned int fix_channels;
	unsigned int fix_bit_width;
};

struct mt8512_laser_multi_in_ctrl_data {
	unsigned int fix_bit_width;
};

struct mt8512_laser_int_adda_ctrl_data {
	unsigned int fix_rate;
};

struct mt8512_laser_priv {
	struct pinctrl *pinctrl;
	struct pinctrl_state *pin_states[PIN_STATE_MAX];
	unsigned int ext_amp_warmup_time_us;
	unsigned int ext_amp_shutdown_time_us;
	struct mt8512_laser_etdm_ctrl_data etdm_data[MT8512_ETDM_SETS];
	struct mt8512_laser_dmic_ctrl_data dmic_data;
	struct mt8512_laser_multi_in_ctrl_data multi_in_data;
	struct mt8512_laser_int_adda_ctrl_data int_adda_data;
	bool line_mute;
	bool lineout_direction_sel;
	bool linein_pwr_on;
	bool lineout_enable;
	bool lineout_gain;
	bool spkr_pwr;
...
static int laser_i2s_enable_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol);

static int laser_i2s_enable_set(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol);

static int laser_tas5805m_reset_set(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol);

static int mt8512_laser_ext_lineout_amp_wevent(
	struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_dapm_context *dapm = w->dapm;
	struct snd_soc_card *card = dapm->card;
	struct mt8512_laser_priv *card_data = snd_soc_card_get_drvdata(card);
...
static const struct snd_soc_dapm_widget mt8512_laser_widgets[] = {
	...
	SND_SOC_DAPM_SPK("Lineout Amp", mt8512_laser_ext_lineout_amp_wevent),
};

static const struct snd_soc_dapm_route mt8512_laser_routes[] = {
...
static int select_laser_pin_mux(uint16_t pin_state,
...
/* Line Mute On/off */
static int laser_line_mute_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_card *card = snd_kcontrol_chip(kcontrol);
	struct mt8512_laser_priv *card_data = snd_soc_card_get_drvdata(card);

	ucontrol->value.integer.value[0] = card_data->line_mute;
	return 0;
}

static int laser_line_mute_set(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	int retval = 0;
	struct snd_soc_card *card = snd_kcontrol_chip(kcontrol);
	struct mt8512_laser_priv *card_data = snd_soc_card_get_drvdata(card);
	uint32_t line_mute_new = ucontrol->value.integer.value[0];
	uint16_t pin_state = line_mute_new ? PIN_LINE_MUTE_ON :
		PIN_LINE_MUTE_OFF;

	dev_info(card->dev, "%s value %d\n", __func__, line_mute_new);

	retval = select_laser_pin_mux(pin_state, card);
	if (retval)
		return retval;

	card_data->line_mute = line_mute_new;

	return 0;
}

/* Line In/Out Direction Select */
static int laser_line_direction_select_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_card *card = snd_kcontrol_chip(kcontrol);
	struct mt8512_laser_priv *card_data = snd_soc_card_get_drvdata(card);

	ucontrol->value.integer.value[0] = card_data->lineout_direction_sel;
	return 0;
}

static int laser_line_direction_select_set(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	int retval = 0;
	struct snd_soc_card *card = snd_kcontrol_chip(kcontrol);
	struct mt8512_laser_priv *card_data = snd_soc_card_get_drvdata(card);
	uint32_t lineout_dir = ucontrol->value.integer.value[0];
	uint16_t pin_state = lineout_dir ? PIN_LINE_OUT_SELECT :
		PIN_LINE_IN_SELECT;

	dev_info(card->dev, "%s value %d\n", __func__, lineout_dir);

	retval = select_laser_pin_mux(pin_state, card);
	if (retval)
		return retval;

	card_data->lineout_direction_sel = lineout_dir;

	return 0;
}

/* LineIn 18V Enable */
static int laser_linein_pwr_on_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_card *card = snd_kcontrol_chip(kcontrol);
	struct mt8512_laser_priv *card_data = snd_soc_card_get_drvdata(card);

	ucontrol->value.integer.value[0] = card_data->linein_pwr_on;
	return 0;
}

static int laser_linein_pwr_on_set(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	int retval = 0;
	struct snd_soc_card *card = snd_kcontrol_chip(kcontrol);
	struct mt8512_laser_priv *card_data = snd_soc_card_get_drvdata(card);
	uint32_t linein_pwr_on_new = ucontrol->value.integer.value[0];
	uint16_t pin_state = linein_pwr_on_new ? PIN_LINEIN_PWR_ON :
		PIN_LINEIN_PWR_OFF;

	dev_info(card->dev, "%s value %d\n", __func__, linein_pwr_on_new);

	retval = select_laser_pin_mux(pin_state, card);
	if (retval)
		return retval;

	card_data->linein_pwr_on = linein_pwr_on_new;

	return 0;
}

/* LineOut HPA Enable */
static int laser_lineout_enable_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_card *card = snd_kcontrol_chip(kcontrol);
	struct mt8512_laser_priv *card_data = snd_soc_card_get_drvdata(card);
...
static int laser_lineout_enable_set(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	int retval = 0;
	struct snd_soc_card *card = snd_kcontrol_chip(kcontrol);
...
/* LineOut HPA Gain */
static int laser_lineout_gain_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_card *card = snd_kcontrol_chip(kcontrol);
	struct mt8512_laser_priv *card_data = snd_soc_card_get_drvdata(card);
...
static int laser_lineout_gain_set(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	int retval = 0;
	struct snd_soc_card *card = snd_kcontrol_chip(kcontrol);
...
/* Tweeter/MidRange Speaker Buck Enable */
static int laser_spkr_pwr_enable_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_card *card = snd_kcontrol_chip(kcontrol);
	struct mt8512_laser_priv *card_data = snd_soc_card_get_drvdata(card);

	ucontrol->value.integer.value[0] = card_data->spkr_pwr;
	return 0;
}

static int laser_spkr_pwr_enable_set(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	int retval = 0;
	struct snd_soc_card *card = snd_kcontrol_chip(kcontrol);
	struct mt8512_laser_priv *card_data = snd_soc_card_get_drvdata(card);
	uint32_t spkr_pwr_new = ucontrol->value.integer.value[0];
	uint16_t pin_state;

	if (card_data->no_spkr_power) {
		dev_info(card->dev, "%s no spkr power needed\n", __func__);
		return 0;
	}

	// TODO: 12V spkr pwr based off spkr usage
	spkr_pwr_new = 1;
	dev_info(card->dev, "%s value %d\n", __func__, spkr_pwr_new);

	pin_state = spkr_pwr_new ? PIN_SPKR_PWR_ON : PIN_SPKR_PWR_OFF;
	retval = select_laser_pin_mux(pin_state, card);
	if (retval)
		return retval;

	card_data->spkr_pwr = spkr_pwr_new;

	return 0;
}

static int laser_tas5805m_reset_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
...
static const struct snd_kcontrol_new mt8512_laser_controls[] = {
	/* Line mute On/Off */
	SOC_SINGLE_BOOL_EXT("Line_InOut_Mute",
		0,
		laser_line_mute_get,
		laser_line_mute_set),
	/* Line In/Out Select */
	SOC_SINGLE_BOOL_EXT("LineOut_Direction_Select",
		0,
		laser_line_direction_select_get,
		laser_line_direction_select_set),
	/* LineIn Power Enable */
	SOC_SINGLE_BOOL_EXT("LineIn_Power_Enable",
		0,
		laser_linein_pwr_on_get,
		laser_linein_pwr_on_set),
	/* LineOut Enable */
	SOC_SINGLE_BOOL_EXT("LineOut_Enable",
		0,
		laser_lineout_enable_get,
		laser_lineout_enable_set),
	/* LineOut HPA Gain */
	SOC_SINGLE_BOOL_EXT("LineOut_Gain_Enable",
		0,
		laser_lineout_gain_get,
		laser_lineout_gain_set),
	/* Speaker Power Enable */
	SOC_SINGLE_BOOL_EXT("Speaker_Power_Enable",
		0,
		laser_spkr_pwr_enable_get,
		laser_spkr_pwr_enable_set),
	/* Force Disable/Enable I2S */
	SOC_SINGLE_BOOL_EXT("Force I2S Enable",
		0,
		laser_i2s_enable_get,
		laser_i2s_enable_set),
	/* Reset TAS5805m with I2S Disabled */
	SOC_SINGLE_BOOL_EXT("Reset TAS5805m",
		0,
		laser_tas5805m_reset_get,
		laser_tas5805m_reset_set),
};
...
static int laser_i2s_enable_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol) {
	struct snd_soc_card *card = snd_kcontrol_chip(kcontrol);
	struct mt8512_laser_priv *priv = snd_soc_card_get_drvdata(card);

	mutex_lock(&priv->amp_lock);
	ucontrol->value.integer.value[0] = priv->force_i2s_enable;
	mutex_unlock(&priv->amp_lock);
...
static int laser_i2s_enable_set(struct snd_kcontrol *kcontrol,
	...
	mutex_lock(&priv->amp_lock);
	if (!priv->amp_playing) {
		error = mt8512_afe_force_etdm2_out_enable(rtd->platform,
				ucontrol->value.integer.value[0]);
	}
	if (!error) {
		priv->force_i2s_enable = ucontrol->value.integer.value[0];
	}
	mutex_unlock(&priv->amp_lock);
...
static int mt8512_laser_hw_params(struct snd_pcm_substream *substream,
				struct snd_pcm_hw_params *params)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mt8512_laser_priv *priv = snd_soc_card_get_drvdata(rtd->card);
	struct snd_soc_dai *cpu_dai = rtd->cpu_dai;
	struct snd_soc_dai *codec_dai;
	int id = rtd->dai_link->id;
	struct mt8512_laser_etdm_ctrl_data *etdm;
	...
	if (id == DAI_LINK_ETDM1_IN) {
		for (i = 0; i < rtd->num_codecs; i++) {
			...
			if (codec_dai == NULL) {
				pr_err("%s invalid dai parameter. i %d\n",
					__func__, i);
				return -EINVAL;
			}
	...
	if (id == DAI_LINK_ETDM2_OUT) {
		/* turn on bclk before putting AMP in play state */
		if (strstr(rtd->codec_dai->name, "tas5805m")) {
			mutex_lock(&priv->amp_lock);
			if (!priv->force_i2s_enable) {
				error = mt8512_afe_force_etdm2_out_enable(rtd->platform, 1);
				if (error) {
					dev_warn(rtd->card->dev, "%s I2S enable error %d\n",
						__func__, error);
				}
			}
			priv->amp_playing = true;
			mutex_unlock(&priv->amp_lock);
		}
	}
...
static int mt8512_laser_be_hw_params_fixup(struct snd_soc_pcm_runtime *rtd,
	struct snd_pcm_hw_params *params)
{
	struct mt8512_laser_priv *priv = snd_soc_card_get_drvdata(rtd->card);
	int id = rtd->dai_link->id;
	struct mt8512_laser_etdm_ctrl_data *etdm;
	struct mt8512_laser_dmic_ctrl_data *dmic;
	struct mt8512_laser_multi_in_ctrl_data *multi_in;
	struct mt8512_laser_int_adda_ctrl_data *int_adda;
...
static int mt8512_laser_hw_params_free(struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtd;
	struct snd_soc_dai *codec_dai;
	int i;
	int error = 0;
	struct mt8512_laser_priv *priv;
	...
	if (rtd->dai_link->id == DAI_LINK_ETDM1_IN) {
		/* Need to power down slave before master */
		for (i = rtd->num_codecs - 1; i >= 0; i--) {
			codec_dai = rtd->codec_dais[i];
			if (codec_dai == NULL) {
				pr_err("%s invalid dai parameter. i %d\n",
					__func__, i);
	...
	if (strstr(rtd->codec_dai->name, "tas5805m")) {
		...
		if (error) {
			dev_warn(rtd->card->dev, "%s I2S disable error %d\n",
				__func__, error);
		}
		priv->amp_playing = false;
		mutex_unlock(&priv->amp_lock);
		dev_info(rtd->card->dev, "%s\n", __func__);
	}

	return 0;
}

static struct snd_soc_ops mt8512_laser_etdm_ops = {
	.hw_params = mt8512_laser_hw_params,
	.hw_free = mt8512_laser_hw_params_free,
};

#ifdef CONFIG_MTK_HIFIXDSP_SUPPORT
static int laser_adsp_hostless_va_init(struct snd_soc_pcm_runtime *rtd)
{
...
static int laser_adsp_hostless_va_startup(struct snd_pcm_substream *substream)
{
...
static void laser_adsp_hostless_va_shutdown(
...
static struct snd_soc_ops laser_adsp_hostless_va_ops = {
	.startup = laser_adsp_hostless_va_startup,
	.shutdown = laser_adsp_hostless_va_shutdown,
};
#endif

/* Digital audio interface glue - connects codec <---> CPU */
static struct snd_soc_dai_link mt8512_laser_dais[] = {
	/* Front End DAI links */
	...
	[DAI_LINK_FE_DSP_HAL_ASR] = {
		...
		.dpcm_capture = 1,
		.ignore_suspend = 1,
		.init = laser_adsp_hostless_va_init,
		.ops = &laser_adsp_hostless_va_ops,
	},
	[DAI_LINK_FE_DSP_HAL_TAP] = {
...
static int mt8512_laser_gpio_probe(struct snd_soc_card *card)
{
	struct mt8512_laser_priv *priv = snd_soc_card_get_drvdata(card);
	int ret = 0;
	int i;
	...
	for (i = 0 ; i < PIN_STATE_MAX ; i++) {
		priv->pin_states[i] = pinctrl_lookup_state(priv->pinctrl,
			mt8512_laser_pin_str[i]);
		if (IS_ERR(priv->pin_states[i])) {
			ret = PTR_ERR(priv->pin_states[i]);
			dev_warn(card->dev, "%s Can't find pin state %s %d\n",
				 __func__, mt8512_laser_pin_str[i], ret);
		}
	}
	...
	/* Line Muted */
	ret = pinctrl_select_state(priv->pinctrl,
				   priv->pin_states[PIN_LINE_MUTE_ON]);
	if (ret)
		dev_info(card->dev, "%s failed to select line mute %d\n",
			__func__, ret);
	priv->line_mute = true;

	/* Line In Selected */
	ret = pinctrl_select_state(priv->pinctrl,
				   priv->pin_states[PIN_LINE_IN_SELECT]);
	if (ret)
		dev_info(card->dev, "%s failed to select line in %d\n",
			__func__, ret);
	priv->lineout_direction_sel = false;

	/* LineIn Power Disable */
	ret = pinctrl_select_state(priv->pinctrl,
				   priv->pin_states[PIN_LINEIN_PWR_OFF]);
	if (ret)
		dev_info(card->dev, "%s failed to select line in power %d\n",
			__func__, ret);
	priv->linein_pwr_on = false;

	/* Line Out Off */
	ret = pinctrl_select_state(priv->pinctrl,
				   priv->pin_states[PIN_LINEOUT_DISABLE]);
	if (ret)
		dev_info(card->dev, "%s failed to select line out disable %d\n",
			__func__, ret);
	priv->lineout_enable = false;

	/* LineOut HPA Gain Off */
	ret = pinctrl_select_state(priv->pinctrl,
				   priv->pin_states[PIN_LINEOUT_GAIN_OFF]);
	if (ret)
		dev_warn(card->dev, "%s failed to select line out gain %d\n",
			__func__, ret);
	else
	...
	/* Speaker buck off */
	ret = pinctrl_select_state(priv->pinctrl,
				   priv->pin_states[PIN_SPKR_PWR_OFF]);
	if (ret)
		dev_info(card->dev, "%s failed to select spkr pwr %d\n",
			__func__, ret);
	priv->spkr_pwr = false;
...
static void mt8512_laser_parse_of_codec(struct device *dev,
...
static void mt8512_laser_parse_of(struct snd_soc_card *card,
				struct device_node *np)
{
	struct mt8512_laser_priv *priv = snd_soc_card_get_drvdata(card);
	size_t i, j;
	...
	for (i = 0; i < ARRAY_SIZE(of_dai_links_daifmt); i++) {
		struct snd_soc_dai_link *dai_link =
			&mt8512_laser_dais[of_dai_links_daifmt[i].link_id];
	...
	for (i = 0; i < ARRAY_SIZE(of_dai_links_etdm); i++) {
		...
		struct snd_soc_dai_link *dai_link = &mt8512_laser_dais[link_id];
		struct mt8512_laser_etdm_ctrl_data *etdm;
		...
		if (ret == 0 && mt8512_afe_rate_supported(val,
			...
			dai_link->be_hw_params_fixup =
				mt8512_laser_be_hw_params_fixup;
		}
		...
		mt8512_laser_parse_of_codec(card->dev, np, dai_link,
			of_dai_links_etdm[i].name);
	...
	if (ret == 0 && (val == 32 || val == 16)) {
		...
		mt8512_laser_dais[DAI_LINK_MULTI_IN].be_hw_params_fixup =
			mt8512_laser_be_hw_params_fixup;
	}
	...
	priv->no_spkr_power = of_property_read_bool(np,
		"no-spkr-power");
}

static struct snd_soc_card mt8512_laser_card = {
	.name = "mt-snd-card",
	.owner = THIS_MODULE,
	.dai_link = mt8512_laser_dais,
	.num_links = ARRAY_SIZE(mt8512_laser_dais),
	.controls = mt8512_laser_controls,
	.num_controls = ARRAY_SIZE(mt8512_laser_controls),
	.dapm_widgets = mt8512_laser_widgets,
	.num_dapm_widgets = ARRAY_SIZE(mt8512_laser_widgets),
	.dapm_routes = mt8512_laser_routes,
	.num_dapm_routes = ARRAY_SIZE(mt8512_laser_routes),
};

static int mt8512_laser_dev_probe(struct platform_device *pdev)
{
	struct snd_soc_card *card = &mt8512_laser_card;
	...
	int ret, id;
	size_t i;
	size_t dais_num = ARRAY_SIZE(mt8512_laser_dais);
	...
	for (i = 0; i < dais_num; i++) {
		...
		if (is_adsp_dai_link(i))
			continue;
#endif
		if (mt8512_laser_dais[i].platform_name)
			continue;

		id = mt8512_laser_dais[i].id;

		if ((id >= DAI_LINK_AFE_FE_BASE &&
		     id <= DAI_LINK_AFE_FE_END) ||
		    (id >= DAI_LINK_AFE_BE_BASE &&
		     id <= DAI_LINK_AFE_BE_END)) {
			mt8512_laser_dais[i].platform_of_node = platform_node;
		}
	}
	...
	for (i = 0; i < ARRAY_SIZE(adsp_dai_links); i++) {
		...
		if (mt8512_laser_dais[dai].platform_name)
			continue;
		mt8512_laser_dais[dai].platform_of_node = platform_node;
	}
#endif

	card->dev = dev;

	priv = devm_kzalloc(dev, sizeof(struct mt8512_laser_priv),
			    GFP_KERNEL);
	if (!priv) {
		ret = -ENOMEM;
	...
	mt8512_laser_gpio_probe(card);

	mt8512_laser_parse_of(card, np);
	...
	if (ret) {
		...
		mutex_init(&priv->amp_lock);
		priv->amp_playing = false;
		priv->force_i2s_enable = 0;
	}

	return ret;
}

static const struct of_device_id mt8512_laser_dt_match[] = {
	{ .compatible = "mediatek,mt8512-laser", },
	{ }
};
MODULE_DEVICE_TABLE(of, mt8512_laser_dt_match);

static struct platform_driver mt8512_laser_driver = {
	.driver = {
		   .name = "mt8512-laser",
		   .of_match_table = mt8512_laser_dt_match,
#ifdef CONFIG_PM
		   .pm = &snd_soc_pm_ops,
#endif
	},
	.probe = mt8512_laser_dev_probe,
};

module_platform_driver(mt8512_laser_driver);

/* Module information */
MODULE_DESCRIPTION("MT8512 Laser SoC machine driver");
MODULE_AUTHOR("Mengge Wang <mengge.wang@mediatek.com>");
MODULE_LICENSE("GPL v2");
MODULE_ALIAS("platform:mt8512-laser");
