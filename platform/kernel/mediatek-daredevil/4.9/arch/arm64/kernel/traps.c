		 ...
		 struct pt_regs *regs)
{
	...
	if (!user_mode(regs)) {
		...
		/*
		 * print backtrace of current as kernel panic signature to
		 * generate kernel panic hashcode.
		 *
		 * This is a temporary walkaround as FireOS team needs to parse
		 * the kernel panic stack trace. This might not be a good
		 * solution, but better than printing in panic() function in
		 * kernel/panic.c as here it will print the true stack trace
		 * upon panic to trace back to the origin of the cause.
		 */
