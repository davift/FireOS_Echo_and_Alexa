...
#ifdef CONFIG_IDME
#define IDME_MACADDR "/idme/eth_mac_addr"
static int idme_get_mac_addr(struct usbnet *dev, unsigned char *mac_addr,
			     size_t addr_len)
{
	...
	if (!mac_addr || addr_len < IFHWADDRLEN) {
		netdev_err(dev->net, "invalid mac_addr ptr or buf");
		return -1;
	}

	node = of_find_node_by_path(IDME_MACADDR);
	if (node) {
		buf = (const char *)of_get_property(node, "value", NULL);
	} else {
		netdev_err(dev->net, "idme of_find_node_by_path failed");
		return -1;
	}
	...
	netdev_info(dev->net, "use IDME MAC address: %pM", mac_addr);
	return 0;

bailout:
	netdev_err(dev->net, "wrong IDME MAC address");
	return -1;
...
#else
	ret = idme_get_mac_addr(dev, (unsigned char *)buf, (sizeof(u8) * ETH_ALEN));
#endif
