...
static int musb_pm_prepare(struct device *dev)
{
	struct musb *musb = dev_to_musb(dev);

	/* Shutdown vbus power as host in suspend mode */
	if (musb && musb->is_host)
		musb_platform_set_vbus(musb, 0);

	return 0;
}

static void musb_pm_complete(struct device *dev)
{
	struct musb *musb = dev_to_musb(dev);

	/* Turn on vbus power as host during resume */
	if (musb && musb->is_host)
		musb_platform_set_vbus(musb, 1);
}

static const struct dev_pm_ops musb_dev_pm_ops = {
	.prepare = musb_pm_prepare,
	.complete = musb_pm_complete,
