...
static DECLARE_TLV_DB_SCALE(tlv_adc_vol, -1200, 50, 0);
	...
	case AIC32X4_STICKY:
static const struct soc_enum aic32x4_mfp_mux_enable_enum =
		SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(control_enable), control_enable);
static int aic32x4_set_mfp_mux(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ucontrol->value.enumerated.item[0] >
		...
		pr_err("%s: MFP4 Gpio Mux. Invalid value=%d\n", __func__,
	...
	if (ucontrol->value.enumerated.item[0]) {
		/*MFP4 one */
		snd_soc_write(codec, AIC32X4_GPIOMFP4CTL,
			      MFP4_GPIO_ENABLE|MFP_GPIO_HI);
		aic32x4->mfp4_mux = true;
	} else {
		/* MFP4 zero*/
		snd_soc_write(codec, AIC32X4_GPIOMFP4CTL,
			     MFP4_GPIO_ENABLE);
		aic32x4->mfp4_mux = false;
	}

	dev_info(codec->dev, "%s: Playback_Audio line_mux=%d codec_unmute=%d\n",
		__func__, aic32x4->mfp4_mux, aic32x4->unmuted);

	return 0;
}

static int aic32x4_get_mfp_mux(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct aic32x4_priv *aic32x4 = snd_soc_codec_get_drvdata(codec);

	dev_dbg(codec->dev, "%s line_mux=%d channels=%d unmute=%d\n",
		__func__, aic32x4->mfp4_mux,
...
static const struct snd_kcontrol_new aic32x4_snd_controls[] = {
	...
	SOC_SINGLE("Left DAC Overflow", AIC32X4_STICKY, 7, 1, 0),
	SOC_SINGLE("Right DAC Overflow", AIC32X4_STICKY, 6, 1, 0),
	SOC_SINGLE("Left ADC Overflow", AIC32X4_STICKY, 3, 1, 0),
	SOC_SINGLE("Right ADC Overflow", AIC32X4_STICKY, 2, 1, 0),
	...
	SOC_DOUBLE_R("AGC Signal Debounce", AIC32X4_LAGC7, AIC32X4_RAGC7,
			0, 0x0F, 0),
	SOC_ENUM_EXT("MFP4 Gpio Mux", aic32x4_mfp_mux_enable_enum,
		aic32x4_get_mfp_mux, aic32x4_set_mfp_mux),
};
...
static const struct aic32x4_rate_divs aic32x4_divs[] = {
	...
	{AIC32X4_FREQ_24576000,  8000, 0, 0,    0, 512, 2, 3, 128, 1, 4, 0},
	{AIC32X4_FREQ_24576000, 16000, 0, 0,    0, 256, 2, 3, 128, 1, 4, 0},
...
static int aic32x4_codec_probe(struct snd_soc_codec *codec)
{
	...
	/* Set Mux to low */
	snd_soc_write(codec, AIC32X4_GPIOMFP4CTL, MFP4_GPIO_ENABLE);
	aic32x4->mfp4_mux = false;
