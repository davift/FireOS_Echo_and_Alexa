...
static const struct aic31xx_rate_divs aic31xx_divs[] = {
	/* 8k rate */
	{AIC31XX_FREQ_12000000, 8000, 1, 7, 6800, 768, 5, 3, 128, 5, 18, 24},
	{AIC31XX_FREQ_24000000, 8000, 2, 7, 6800, 768, 15, 1, 64, 45, 4, 24},
	{AIC31XX_FREQ_25000000, 8000, 2, 7, 3728, 768, 15, 1, 64, 45, 4, 24},
	/* 11.025k rate */
	{AIC31XX_FREQ_12000000, 11025, 1, 7, 5264, 512, 8, 2, 128, 8, 8, 16},
	{AIC31XX_FREQ_24000000, 11025, 2, 7, 5264, 512, 16, 1, 64, 32, 4, 16},
	/* 16k rate */
	{AIC31XX_FREQ_12000000, 16000, 1, 7, 6800, 384, 5, 3, 128, 5, 9, 12},
	{AIC31XX_FREQ_24000000, 16000, 2, 7, 6800, 384, 15, 1, 64, 18, 5, 12},
	{AIC31XX_FREQ_25000000, 16000, 2, 7, 3728, 384, 15, 1, 64, 18, 5, 12},
	/* 22.05k rate */
	{AIC31XX_FREQ_12000000, 22050, 1, 7, 5264, 256, 4, 4, 128, 4, 8, 8},
	{AIC31XX_FREQ_24000000, 22050, 2, 7, 5264, 256, 16, 1, 64, 16, 4, 8},
	{AIC31XX_FREQ_25000000, 22050, 2, 7, 2253, 256, 16, 1, 64, 16, 4, 8},
	/* 32k rate */
	{AIC31XX_FREQ_12000000, 32000, 1, 7, 1680, 192, 2, 7, 64, 2, 21, 6},
	{AIC31XX_FREQ_24000000, 32000, 2, 7, 1680, 192, 7, 2, 64, 7, 6, 6},
	/* 44.1k rate */
	{AIC31XX_FREQ_12000000, 44100, 1, 7, 5264, 128, 2, 8, 128, 2, 8, 4},
	{AIC31XX_FREQ_24000000, 44100, 2, 7, 5264, 128, 8, 2, 64, 8, 4, 4},
	{AIC31XX_FREQ_25000000, 44100, 2, 7, 2253, 128, 8, 2, 64, 8, 4, 4},
	/* 48k rate */
	{AIC31XX_FREQ_9600000,  48000, 5, 48, 0,   128, 5, 3, 128, 5, 3, 1},
	{AIC31XX_FREQ_12000000, 48000, 1, 8, 1920, 128, 2, 8, 128, 2, 8, 4},
	{AIC31XX_FREQ_24000000, 48000, 2, 8, 1920, 128, 8, 2, 64, 8, 4, 4},
	{AIC31XX_FREQ_24576000, 48000, 0, 0,    0, 128, 1, 4, 128, 1, 4, 0},
	{AIC31XX_FREQ_24576000,  8000, 0, 0,    0, 512, 2, 3, 128, 1, 4, 0},
	{AIC31XX_FREQ_24576000, 16000, 0, 0,    0, 256, 2, 3, 128, 1, 4, 0},
	{AIC31XX_FREQ_25000000, 48000, 2, 7, 8643, 128, 8, 2, 64, 8, 4, 4},
	/* BCLK Interface */
	{AIC31XX_FREQ_1536000,  48000, 1, 4,    0, 128, 1, 1, 128, 1, 1, 1},
};
...
static inline int aic31xx_get_divs(int mclk, int rate)
{
	...
	pr_err("aic31xx:%s master clock %d and sample rate %d is not supported\n",
		__func__, mclk, rate);
	return -EINVAL;
}
			     ...
			     struct snd_pcm_hw_params *params)
{
	...
	if (i < 0) {
		dev_err(codec->dev, "%s: no supported freq. Using BCLK\n",
			__func__);
		i = ARRAY_SIZE(aic31xx_divs) - 1;
		clk_mux = AIC31XX_BCLKCLKIN;
	}
	aic31xx->rate_div_line = i;

	if (aic31xx_divs[i].p_val) {
		/* Use PLL as CODEC_CLKIN */
		snd_soc_write(codec, AIC31XX_CLKMUX, clk_mux);

		/* We will fix R = 1 and will make P & J=K.D as varialble */
		snd_soc_write(codec, AIC31XX_PLLPR,
			((aic31xx_divs[i].p_val << 4) | 0x01 | AIC31XX_PLLEN));
	...
	/* NDAC divider value */
	snd_soc_write(codec, AIC31XX_NDAC,
		aic31xx_divs[i].ndac | AIC31XX_NDACEN);

	/* MDAC divider value */
	snd_soc_write(codec, AIC31XX_MDAC,
		aic31xx_divs[i].mdac | AIC31XX_MDACEN);

	/* DOSR MSB & LSB values */
	snd_soc_write(codec, AIC31XX_DOSRMSB,
		aic31xx_divs[i].dosr >> 8);
	snd_soc_write(codec, AIC31XX_DOSRLSB,
		(aic31xx_divs[i].dosr & 0xff));

	/* NADC divider value */
	snd_soc_write(codec, AIC31XX_NADC,
		aic31xx_divs[i].nadc | AIC31XX_NADCEN);

	/* MADC divider value */
	snd_soc_write(codec, AIC31XX_MADC,
		aic31xx_divs[i].madc | AIC31XX_MADCEN);

	/* AOSR value */
	snd_soc_write(codec, AIC31XX_AOSR, aic31xx_divs[i].aosr);

	if (aic31xx_divs[i].blck_N) {
		/* Set and Enable BCLK N divider */
		snd_soc_write(codec, AIC31XX_BCLKN,
			aic31xx_divs[i].blck_N | AIC31XX_BCLKEN);
	}
	...
	dev_info(codec->dev,
		...
		aic31xx_divs[i].madc, aic31xx_divs[i].blck_N);
			       ...
			       unsigned int fmt)
{
	...
	case SND_SOC_DAIFMT_CBS_CFS:
		iface_reg2 |= AIC31XX_DACMOD2BCLK;
		break;
	default:
				  ...
				  int clk_id, unsigned int freq, int dir)
{
	...
	case AIC31XX_FREQ_25000000:
	case AIC31XX_FREQ_1536000:
		aic31xx->sysclk = freq;
		return 0;
	}

	pr_err("aic31xx:%s invalid frequency %u to set DAI system clock\n",
		__func__, freq);
