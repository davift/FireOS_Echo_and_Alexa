...
#if defined(UFBL_FEATURE_IDME)
	ret = fdt_increase_size(images.ft_addr, CONFIG_IDME_SIZE);
	if (ret) {
		puts("fdt_increase_size failed while trying to make space for IDME \n");
		return 1;
	}
	images.ft_len += CONFIG_IDME_SIZE;

	insert_device_tree_properties_from_idme(idme_dt_prop_table,
			idme_dt_prop_table_prop_count);

	idme_device_tree_initialize(images.ft_addr);
...
#if IMAGE_ENABLE_FIT
	case IMAGE_FORMAT_FIT:
		if (sec_boot_setup_fit_keys(gd_fdt_blob()) < 0) {
			printf("Failed to setup fit keys\n");
			return NULL;
		}
