...
static int initr_env(void)
{
	...
	/* Add nand partition sparse */
#if defined(CONFIG_MTK_SPI_NAND)
	board_mtdparts_init();

	/* Set bootimage size to 12.11MB */
	env_set_hex("bootimage_size", 0xC1C000);
#if CONFIG_CMD_UBI
	/* Since boot_a doesn't exist in GPT, It needs to be marked */
	env_set("boot_partition", (CONFIG_BOOT_NAME));

	/* the userdata is the only UBI device in the system */
	if (ubi_part((CONFIG_UBI_NAME), NULL)) {
