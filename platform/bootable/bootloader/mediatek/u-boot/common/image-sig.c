		...
		const void *sig_blob)
{
	...
	fdt_for_each_subnode(noffset, sig_blob, sig_node) {
		...
		/* Extract the key_name_hint */
		key_name_hint = fdt_getprop(sig_blob, noffset, "key-name-hint", NULL);
		if (!key_name_hint)
			continue;
		...
		if (ret) {
			printf("Failed to verify required signature '%s'\n",
			       fit_get_name(sig_blob, noffset, NULL));
			// Commenting as we fail only when all validations have exhausted
			//return ret;
		} else {
			// Passed verification at least once
			status = 0;
		}
	}
