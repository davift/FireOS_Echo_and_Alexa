...
#ifdef CONFIG_ENABLE_UFBL_SECURITY
...
void fit_image_print(const void *fit, int image_noffset, const char *p)
{
	...
	printf("%s  Description:  ", print_addr(p));
	if (ret)
		printf("unavailable\n");
	else
		printf("%s\n", print_addr(desc));
	...
	if (IMAGE_ENABLE_TIMESTAMP) {
		...
		ret = fit_get_timestamp(fit, 0, &timestamp);
		printf("%s  Created:      ", print_addr(p));
		if (ret)
			printf("unavailable\n");
		else
			genimg_print_time(print_addr(timestamp));
	}

	fit_image_get_type(fit, image_noffset, &type);
	printf("%s  Type:         %s\n", print_addr(p), print_addr(genimg_get_type_name(type)));

	fit_image_get_comp(fit, image_noffset, &comp);
	printf("%s  Compression:  %s\n", print_addr(p), print_addr(genimg_get_comp_name(comp)));

	ret = fit_image_get_data_and_size(fit, image_noffset, &data, &size);

#ifndef USE_HOSTCC
	printf("%s  Data Start:   ", print_addr(p));
	if (ret) {
		printf("unavailable\n");
	} else {
		void *vdata = (void *)data;

		printf("0x%08lx\n", print_addr((ulong)map_to_sysmem(vdata)));
	}
#endif

	printf("%s  Data Size:    ", print_addr(p));
	if (ret)
		printf("unavailable\n");
	else
		genimg_print_size(print_addr(size));
	...
	if ((type == IH_TYPE_KERNEL) || (type == IH_TYPE_STANDALONE) ||
		...
		printf("%s  Architecture: %s\n", print_addr(p), print_addr(genimg_get_arch_name(arch)));
	}

	if ((type == IH_TYPE_KERNEL) || (type == IH_TYPE_RAMDISK) ||
	    (type == IH_TYPE_FIRMWARE)) {
		fit_image_get_os(fit, image_noffset, &os);
		printf("%s  OS:           %s\n", print_addr(p), print_addr(genimg_get_os_name(os)));
	}
	...
	if ((type == IH_TYPE_KERNEL) || (type == IH_TYPE_STANDALONE) ||
		...
		printf("%s  Load Address: ", print_addr(p));
		if (ret)
			printf("unavailable\n");
		else
			printf("0x%08lx\n", print_addr(load));
	...
	if (type == IH_TYPE_FLATDT && !fit_image_get_load(fit, image_noffset, &load))
		printf("%s  Load Address: 0x%08lx\n", print_addr(p), print_addr(load));

	if ((type == IH_TYPE_KERNEL) || (type == IH_TYPE_STANDALONE) ||
	    (type == IH_TYPE_RAMDISK)) {
		ret = fit_image_get_entry(fit, image_noffset, &entry);
		printf("%s  Entry Point:  ", print_addr(p));
		if (ret)
			printf("unavailable\n");
		else
			printf("0x%08lx\n", print_addr(entry));
...
int fit_all_image_verify(const void *fit)
{
	...
	for (ndepth = 0, count = 0,
		...
		if (ndepth == 1) {
			 ...
			 * i.e. component image node.
			 */
			printf("   Hash(es) for Image %u (%s): ", print_addr(count),
			       print_addr(fit_get_name(fit, noffset, NULL)));
			count++;
...
int fit_check_format(const void *fit, ulong size)
{
	...
	if (ret) {
		printf("Wrong FIT format: not a flattened device tree (err=%d)\n",
			  ret);
		return -ENOEXEC;
	}
	...
	if (1) {
		 ...
		 */
#if CONFIG_FIT_SIGNATURE
		if (!ret) {
			ret = fdt_check_no_at(fit, 0);

			if (ret) {
				printf("FIT check error %d\n", ret);
				return ret;
			}
		}
#endif
		   ...
		   enum fit_load_op load_op, ulong *datap, ulong *lenp)
{
	...
	printf("## Loading %s from FIT Image at %08lx ...\n", print_addr(prop_name), print_addr(addr));
...
#ifdef CONFIG_ENABLE_UFBL_SECURITY
			if (!amzn_target_is_unlocked() && !amzn_target_is_onetime_unlocked()) {
#endif
				puts("   Verifying Hash Integrity ... ");
				...
				puts("Device is unlocked, kernel verification is skipped...\n");
			}
#endif
		}
	...
	if (load_op == FIT_LOAD_IGNORED) {
		...
		printf("   Loading %s from 0x%08lx to 0x%08lx\n",
		       print_addr(prop_name), print_addr(data), print_addr(load));
