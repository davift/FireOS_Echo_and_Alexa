...
#if defined(CONFIG_MTK_I2C_BEBUG)
int do_mtk_i2c_write(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	u8 bus_num, dev_addr, cnt_num;
	u8 wr_temp[3] = {0};
	u8 *wr_buf;
	u32 speed, wr_len;
	int w_ret;
	...
	bus_num = (u8)simple_strtoul(argv[1], NULL, 10);
	dev_addr = (u8)simple_strtoul(argv[2], NULL, 16);
	speed = (u32)simple_strtoul(argv[3], NULL, 10);
	wr_len = (u32)simple_strtoul(argv[4], NULL, 10);

	wr_buf = (u8 *)malloc(wr_len + 1);
	memset(wr_buf, 0, (wr_len + 1));
	for (cnt_num = 0; cnt_num < wr_len; cnt_num++) {
		memcpy(wr_temp, (argv[5] + cnt_num * 2), 2);
		*(wr_buf + cnt_num) = (u8)simple_strtoul(wr_temp, NULL, 16);
	}

	w_ret = mtk_i2c_write(bus_num, dev_addr, speed, wr_buf, wr_len);
	if (w_ret != 0) {
		printf("bus_num=%d, w_ret:%d, mtk_i2c_write(%x) fail --> end \n", bus_num, w_ret, wr_buf[0]);
		goto exit;
	}
	printf("bus_num=%d, mtk_i2c_write(%x) done!!! \n", bus_num, wr_buf[0]);
...
int do_mtk_i2c_read(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	u8 bus_num, dev_addr, cnt_num;
	u8 *rd_buf;
	u32 speed, rd_len;
	...
	dev_addr = (u8)simple_strtoul(argv[2], NULL, 16);
	speed = (u32)simple_strtoul(argv[3], NULL, 10);
	rd_len = (u32)simple_strtoul(argv[4], NULL, 10);

	rd_buf = malloc(rd_len + 1);
	memset(rd_buf, 0, (rd_len + 1));

	r_ret = mtk_i2c_read(bus_num, dev_addr, speed, rd_buf, rd_len);
	if (r_ret != 0) {
		printf("bus_num=%d, r_ret:%d, mtk_i2c_read fail --> end \n", bus_num, r_ret);
		goto exit;
	}
	printf("bus_num=%d, mtk_i2c_read done.\nread data: ", bus_num);
	for (cnt_num = 0; cnt_num < rd_len; cnt_num++) {
		printf("%02x ", *(rd_buf + cnt_num));
	}
	printf("\n");
...
int do_mtk_i2c_write_read(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	u8 bus_num, dev_addr, cnt_num;
	u8 wr_temp[3] = {0};
	u8 *wr_buf;
	u8 *rd_buf;
	u32 speed, wr_len, rd_len;
	int r_ret;
	...
	speed = (u32)simple_strtoul(argv[3], NULL, 10);
	wr_len = (u32)simple_strtoul(argv[4], NULL, 10);
	rd_len = (u32)simple_strtoul(argv[5], NULL, 10);

	wr_buf = malloc(wr_len + 1);
	memset(wr_buf, 0, (wr_len + 1));
	for (cnt_num = 0; cnt_num < wr_len; cnt_num++) {
		memcpy(wr_temp, (argv[6] + cnt_num * 2), 2);
		*(wr_buf + cnt_num) = (u8)simple_strtoul(wr_temp, NULL, 16);
	}
	...
	r_ret = mtk_i2c_write_read(bus_num, dev_addr, speed, wr_buf, rd_buf, wr_len, rd_len);
	if (r_ret != 0) {
		printf("bus_num=%d, r_ret:%d, mtk_i2c_write_read(%x) fail --> end \n", bus_num, r_ret, wr_buf[0]);
		goto exit;
	}
	printf("bus_num=%d, mtk_i2c_write_read(%x) done.\nread data: ", bus_num, wr_buf[0]);
	for (cnt_num = 0; cnt_num < rd_len; cnt_num++) {
		printf("%02x ", *(rd_buf + cnt_num));
	}
	printf("\n");
...
int do_mtk_i2c_scanbus(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	u8 rd_buf;
	u8 bus_num, first, last;
	int r_ret;
	int i, j;
	struct i2c_msg msgs;
	...
	if (argc == 2) {
		...
		first = (u8)simple_strtoul(argv[2], NULL, 16);
		last = 0x7f;
	} else if (argc == 4) {
		first = (u8)simple_strtoul(argv[2], NULL, 16);
		last = (u8)simple_strtoul(argv[3], NULL, 16);
	} else {
		return CMD_RET_USAGE;
	}
	...
	for (i = 0; i < 128; i += 16) {
		...
		for(j = 0; j < 16; j++) {
			...
			msgs.addr = (i + j);
			r_ret = mtk_i2c_transfer(i2c, &msgs, 1);
...
#if defined(CONFIG_MTK_I2C_DEBUG)
	U_BOOT_CMD_MKENT(mtk_i2c_w, 6, 1, do_mtk_i2c_write,
	"mtk i2c write test.",
	"mtk_i2c_w bus_num dev_addr speed_k wr_len wr_data \n"),

	U_BOOT_CMD_MKENT(mtk_i2c_r, 5, 1, do_mtk_i2c_read,
	"mtk i2c read test.",
	"mtk_i2c_r bus_num dev_addr speed_k rd_len \n"),

	U_BOOT_CMD_MKENT(mtk_i2c_w_r, 7, 1, do_mtk_i2c_write_read,
	"mtk i2c write_read test(first write reg addr, then read data).",
	"mtk_i2c_w_r bus_num dev_addr speed_k wr_len rd_len wr_data \n"),

	U_BOOT_CMD_MKENT(mtk_i2c_scan, 4, 1, do_mtk_i2c_scanbus,
	"mtk i2c scan bus test.",
	"mtk_i2c_scan bus_num [first_addr] [last_addr] \n"),
#endif
};
