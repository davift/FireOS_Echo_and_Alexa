...
static void display_ubi_info(struct ubi_device *ubi)
{
	...
	ubi_msg("number of PEBs reserved for bad PEB handling: %d",
			ubi->beb_rsvd_pebs);
	ubi_msg("top limit of expected bad PEBs: %d", ubi->bad_peb_limit);
	ubi_msg("max/mean erase counter: %d/%d", ubi->max_ec, ubi->mean_ec);
}

#ifdef CONFIG_FASTBOOT_FORMAT_UBI
int ubi_info(int layout)
#else
static int ubi_info(int layout)
#endif
{
	if (ubi == NULL) {
		printf("No ubi device attached\n");
		return ENODEV;
	}
...
#endif
{
	...
	if (ubi == NULL) {
		printf("No ubi device attached so cannot search for %s\n", volume);
		return NULL;
	}
...
int ubi_volume_read(char *volume, char *buf, size_t size)
{
	...
	/* if the vtbl_slots value is illegal, find volume will cause crash */
	...
	printf("Read %zu bytes from volume %s to %p\n", print_addr(size), print_addr(volume), print_addr(buf));
...
int ubi_volume_resize(char *volume, size_t size)
{
	...
	printf("Resize volume %s from %d to %d LEBs\n", volume, vol->used_ebs, size);
	vol_desc.vol = vol;
	/* Mode is not used in ubi_resize_volume, but setting explicitly anyways. */
	vol_desc.mode = UBI_READONLY;
	err = ubi_resize_volume(&vol_desc, size);
	if (err < 0) {
		printf("Failed to resize volume %d\n", err);
	}
...
static int do_ubi(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	if (strcmp(argv[1], "resize") == 0) {
		...
		if (argc == 3) {
			return ubi_volume_resize(argv[2], size);
		}
	}
...
U_BOOT_CMD(
	...
	"ubi remove[vol] volume"
		" - Remove volume\n"
	"ubi resize volume peb_count"
		" - Resize volume to new number of pebs\n"
	"[Legends]\n"
