...
static struct part_driver *part_driver_lookup_type(struct blk_desc *dev_desc)
{
	...
	if (dev_desc->part_type == PART_TYPE_UNKNOWN) {
		for (entry = drv; entry != drv + n_ents; entry++) {
			...
			/*
			 * WORKAROUND: entry->test() seems to be always failing
			 * but it's a false positive error.
			 * While we investigate it we will bypass the error.
			 */
			if (ret) {
				printf("%s: applying workaround to bypass entry->test() failure\n",
					__func__);
				ret = 0;
			}
