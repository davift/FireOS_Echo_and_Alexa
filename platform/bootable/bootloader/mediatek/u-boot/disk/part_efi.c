...
static int fix_gpt_last_part_ending_lba(gpt_header *gpt_h, gpt_entry *gpt_e,
		lbaint_t last_usable_lba)
{
	char efi_str[PARTNAME_SZ + 1];
	int i, last_part = -1;
	uint32_t calc_crc32;
	...
	if (last_part == -1) {
		pr_err("%s: Last part not found\n", __func__);
		return -1;
	}

	printf("%s: Update last part(%d) endling_lba 0x%08llx -> 0x%08lx\n",
		__func__,
		last_part,
		le64_to_cpu(gpt_e[last_part].ending_lba),
		last_usable_lba);

	/* extend the last partition to maximuim */
	gpt_e[last_part].ending_lba = cpu_to_le64(last_usable_lba);
...
static int fix_gpt_header_last_usable_lba(gpt_header *gpt_h,
		lbaint_t last_usable_lba)
{
	uint32_t calc_crc32;

	printf("%s: Update last_usable_lba 0x%08llx -> 0x%08lx\n",
		__func__,
		le64_to_cpu(gpt_h->last_usable_lba),
		last_usable_lba);
...
int fix_gpt_last_usable_lba(struct blk_desc *dev_desc, void *buf)
{
	gpt_header *gpt_h;
	gpt_entry *gpt_e;
	lbaint_t lastlba = dev_desc->lba;
	/* reserve 34 blocks for secondary GPT in the bottom of MMC device */
	lbaint_t last_usable_lba = lastlba - 34;
	...
	/*
	 * Update the ending lba of last GPT part entry and regenerate
	 * entry CRC in GPT header
	 */
	if (fix_gpt_last_part_ending_lba(gpt_h, gpt_e, last_usable_lba))
		return -1;

	/* Update last usable lba and regenrate CRC in GPT header*/
	if (fix_gpt_header_last_usable_lba(gpt_h, last_usable_lba))
		return -1;
