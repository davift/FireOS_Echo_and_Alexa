		   ...
		   bootm_headers_t *images)
{
	...
	const char *env;
	char lk_time_buf[12];
	char uboot_time_buf[12];

	/* No need for those on ARM */
	if (flag & BOOTM_STATE_OS_BD_T || flag & BOOTM_STATE_OS_CMDLINE)
		return -1;

	lk_t = get_lk_boottime_from_boot_args();

	env = env_get("bootargs");
	strcpy(g_cmdline, env);

	sprintf(lk_time_buf, "%ld", lk_t);
	sprintf(uboot_time_buf, "%ld", get_timer(0));
	strcat(g_cmdline, " bootprof.lk_t=");
	strcat(g_cmdline, lk_time_buf);
	strcat(g_cmdline, " bootprof.uboot_t=");
	strcat(g_cmdline, uboot_time_buf);

	env_set("bootargs", g_cmdline);
