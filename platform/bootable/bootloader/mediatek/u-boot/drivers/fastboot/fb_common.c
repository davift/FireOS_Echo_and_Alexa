		       ...
		       const char *format, ...)
{
	...
	if (format) {
		...
		int n = FASTBOOT_RESPONSE_LEN - strlen(response) - 1;
		int ret = vsnprintf(p, n, format, args);
		if (ret < 0 || ret >= n)
			strncpy(response, "vsnprintf error", FASTBOOT_RESPONSE_LEN - 1);
		va_end(args);
	}
}
...
/**
 * fastboot_info() - Write a INFO response of the form "INFO$reason".
 *
 * @reason: Pointer to returned reason string
 * @response: Pointer to fastboot response buffer
 */
...
void fastboot_boot(void)
{
	...
	if (s) {
		...
		int n = sizeof( boot_addr_start );
		n = snprintf(boot_addr_start, sizeof(boot_addr_start) - 1,
			 "0x%p", fastboot_buf_addr);
		if (n < 0 || n >= sizeof(boot_addr_start))
			strncpy(boot_addr_start, "unknown error", sizeof(boot_addr_start) - 1);
