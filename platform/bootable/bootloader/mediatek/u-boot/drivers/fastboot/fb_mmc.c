...
#define FASTBOOT_MAX_BLK_WRITE 1048576
...
static int fb_mmc_erase_mmc_hwpart(struct blk_desc *dev_desc)
{
	...
	printf("........ erased %u bytes from mmc hwpart[%u]\n",
...
#ifdef CONFIG_AMZ_WD_EMMC_FIX

#define WD_EMMC_MID (0x45)

static bool is_wd_emmc(struct mmc *mmc)
{
	int mid;
	mid = mmc->cid[0] >> 24;
	return mid == WD_EMMC_MID;
}
...
#ifdef CONFIG_AMZ_WD_EMMC_FIX
		if (is_wd_emmc(find_mmc_device(CONFIG_FASTBOOT_FLASH_MMC_DEV))) {
			if (fix_gpt_last_usable_lba(dev_desc, download_buffer)) {
				pr_err("%s: fix last usable lba failed\n",
				       __func__);
				fastboot_fail("fix GPT partition failed", response);
				return;
			}
			printf("%s: Found WD eMMC and the GPT has been adjusted\n", __func__);
		}
#endif
