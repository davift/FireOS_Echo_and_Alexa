...
static void fastboot_send_info(const char* msg) {
			    ...
			    unsigned interface, unsigned alt)
{
	...
	fastboot_set_led_indicator();
	fastboot_set_progress_callback(fastboot_send_info);
...
{
	if (!run_command("boot", 0))
		pr_err("Command 'boot' fail!!\n");
