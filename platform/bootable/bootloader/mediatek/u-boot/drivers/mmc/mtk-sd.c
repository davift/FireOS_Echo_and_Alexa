...
static int msdc_pio_read(struct msdc_host *host, u8 *ptr, u32 size)
{
	...
	while (1) {
		...
		if (status & MSDC_INT_DATCRCERR) {
			pr_err("DAT CRC ERR status=%x\n", status);
			ret = -EIO;
			break;
		}

		if (status & MSDC_INT_DATTMO) {
			pr_err("DAT TMO ERR status=%x\n", status);
			ret = -ETIMEDOUT;
			break;
		}
...
static inline void msdc_set_cmd_delay(struct msdc_host *host, u32 value)
{
	...
	if (host->top_base)
		clrsetbits_le32(&host->top_base->emmc_top_cmd, PAD_CMD_RXDLY_M,
...
static int msdc_ofdata_to_platdata(struct udevice *dev)
{
	...
	if (host->base == (void *)FDT_ADDR_T_NONE)
		return -EINVAL;

	host->top_base = (void *)dev_read_addr_index(dev, 1);
	if (host->top_base == (void *)FDT_ADDR_T_NONE)
		host->top_base = NULL;
...
static const struct msdc_compatible mt8519_compat = {
	.clk_div_bits = 12,
	.pad_tune0 = true,
	.async_fifo = true,
	.data_tune = true,
...
static const struct udevice_id msdc_ids[] = {
	...
	{ .compatible = "mediatek,mt8519-mmc", .data = (ulong)&mt8519_compat },
	{}
};
