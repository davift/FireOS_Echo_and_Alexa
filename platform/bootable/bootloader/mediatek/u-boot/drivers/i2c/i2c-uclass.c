...
int i2c_get_chip_compat(struct udevice *bus, uint chip_addr, uint offset_len,
		 const char *compat, struct udevice **devp)
{
	struct udevice *dev;
	...
	for (device_find_first_child(bus, &dev); dev;
		...
		if ((chip->chip_addr == chip_addr) && device_is_compatible(dev, compat)) {
			ret = device_probe(dev);
			debug("found, ret=%d\n", ret);
			if (ret)
				return ret;
...
int i2c_get_chip_for_busnum_compat(int busnum, int chip_addr, uint offset_len,
			    const char *compat, struct udevice **devp)
{
	struct udevice *bus;
	int ret;
	...
	ret = i2c_get_chip_compat(bus, chip_addr, offset_len, compat, devp);
	if (ret) {
		debug("Cannot find I2C chip %02x on bus %d\n", chip_addr,
		      busnum);
		return ret;
