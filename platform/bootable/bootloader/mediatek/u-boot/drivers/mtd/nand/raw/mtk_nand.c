...
struct mtk_nfc_bad_mark_ctl {
	void (*bm_swap)(struct mtd_info *mtd, u8 *buf, int raw);
	u32 sec;
	u32 pos;
};
...
static void mtk_nfc_hw_reset(struct mtk_nfc *nfc)
{
	...
	ret = readl_poll_timeout_atomic(NFC_REG(nfc, NFI_MASTER_STA),
...
static inline u8 mtk_nfc_read_byte(struct mtd_info *mtd)
{
	...
	if (reg != NFI_FSM_CUSTDATA) {
		...
		if (nfc->chip_data->fdm_ecc_size)
			reg = (nfc->chip_data->max_sector << CON_SEC_SHIFT)
			    | CON_BRD;
		else
			reg = (MTK_MAX_SECTOR << CON_SEC_SHIFT)
			    | CON_BRD;
...
#else
	ret = readl_poll_timeout_atomic(NFC_REG(nfc, NFI_INTR_STA),
		 reg, reg & INTR_AHB_DONE_EN, 10, MTK_TIMEOUT);
#endif
	if (ret) {
...
#endif
	rc = readl_poll_timeout_atomic(NFC_REG(nfc, NFI_INTR_STA),
		 reg, reg & INTR_AHB_DONE_EN, 10, MTK_TIMEOUT);
	if (rc)
		dev_warn(nfc->dev, "read ahb/dma done timeout\n");

	rc = readl_poll_timeout_atomic(NFC_REG(nfc, NFI_BYTELEN),
		reg, ADDRCNTR_SEC(reg) >= sectors, 10,
		MTK_TIMEOUT);
	if (rc < 0) {
...
static int mtk_nfc_ecc_init(struct device *dev, struct mtd_info *mtd)
{
	...
	if (!nand->ecc.size || !nand->ecc.strength) {
		...
		if (nand->ecc.size < 1024) {
			if ((mtd->writesize > 512) &&
			    (nfc->chip_data->max_ecc_size > 512)) {
				nand->ecc.size = 1024;
				nand->ecc.strength <<= 1;
			} else {
				nand->ecc.size = 512;
		...
		free = ((nand->ecc.strength
		    * nfc->chip_data->parity_bit) + 7) >> 3;
		free = spare - free;
		/*
		...
		if (free > NFI_FDM_MAX_SIZE) {
			...
			nand->ecc.strength =
			    (spare << 3) / nfc->chip_data->parity_bit;
		} else if (free < 0) {
...
static int mtk_nfc_nand_chip_init(struct udevice *dev, struct mtk_nfc *nfc,
				  struct device_node *np)
#else
static int mtk_nfc_nand_chip_init(struct device *dev,
				struct mtk_nfc *nfc,
				struct device_node *np,
...
#endif
{
...
#else
	nand->flash_node = dev_of_offset(dev);
	...
	if (ret)
		dev_err(dev, "Nand register failed\n");

	/* Remove to board init */
...
#else
static const struct udevice_id mtk_nfc_id_table[] = {
#endif
	{
...
int board_nand_init(struct nand_chip *nand)
{
	...
	ret = uclass_get_device_by_driver(UCLASS_MTD,
					  DM_GET_DRIVER(mtk_nand_drv),
					  &dev);
	if (ret) {
		pr_err("Failed to get mtk_nand_drv device. (error %d)\n", ret);
		return ret;
	}
	...
	nfc->ecc = mtk_ecc_nand_chip_init();
	if (IS_ERR(nfc->ecc))
		return PTR_ERR(nfc->ecc);
	else if (!nfc->ecc)
		return -ENODEV;
	...
	ret = mtk_nfc_nand_chip_init(dev, nfc, ofnode_to_np(dev->node), nand);
	if (ret) {
		pr_err("failed to init nfc (error %d)\n", ret);
		return ret;
	}
...
static int mtk_nand_ofdata_to_platdata(struct udevice *dev)
{
	struct mtk_nfc *nfc = dev_get_priv(dev);
	int ret = 0;

	nfc->regs = (void *)dev_read_addr(dev);
	nfc->chip_data = (struct mtk_nand_type *)dev_get_driver_data(dev);
	pr_debug("mtk_nand_ofdata regs:0x%x\n", nfc->regs);

	memset(&nfc->clk, 0, sizeof(struct mtk_nfc_clk));
	nfc->clk.nfi_clk =
	    kmalloc(sizeof(*nfc->clk.nfi_clk), GFP_KERNEL);
	nfc->clk.pad_clk =
	    kmalloc(sizeof(*nfc->clk.pad_clk), GFP_KERNEL);
	nfc->clk.nfi_top_clk =
	    kmalloc(sizeof(*nfc->clk.nfi_top_clk), GFP_KERNEL);
	nfc->clk.nfi_clk_sel =
	    kmalloc(sizeof(*nfc->clk.nfi_clk_sel), GFP_KERNEL);
	nfc->clk.nfi_clk_parent =
	    kmalloc(sizeof(*nfc->clk.nfi_clk_parent), GFP_KERNEL);

	if (!nfc->clk.nfi_clk || !nfc->clk.pad_clk ||
	    !nfc->clk.nfi_top_clk || !nfc->clk.nfi_clk_sel |
	    !nfc->clk.nfi_clk_parent) {
		dev_err(dev, "clk malloc failed\n");
		ret = -ENOMEM;
		goto err;
	}
	...
	ret = clk_get_by_name(dev, "pad_clk", nfc->clk.pad_clk);
	if (IS_ERR(nfc->clk.pad_clk)) {
		dev_err(dev, "no pad clk\n");
		ret = PTR_ERR(nfc->clk.pad_clk);
		goto err;
	...
	ret = clk_get_by_name(dev, "nfi_top_clk", nfc->clk.nfi_top_clk);
	if (IS_ERR(nfc->clk.nfi_top_clk)) {
		dev_info(dev, "no top clk\n");
		ret = PTR_ERR(nfc->clk.nfi_top_clk);
		goto err;
	...
	ret = clk_get_by_name(dev, "nfi_clk_sel", nfc->clk.nfi_clk_sel);
	if (IS_ERR(nfc->clk.nfi_clk_sel)) {
		ret = PTR_ERR(nfc->clk.nfi_clk_sel);
		goto err;
	}

	ret = clk_get_by_name(dev, "nfi_clk_parent", nfc->clk.nfi_clk_parent);
	if (IS_ERR(nfc->clk.nfi_clk_parent)) {
		ret = PTR_ERR(nfc->clk.nfi_clk_parent);
		goto err;
	}
...
err:
	...
	if (nfc->clk.pad_clk)
		kfree(nfc->clk.pad_clk);
	if (nfc->clk.nfi_top_clk)
		kfree(nfc->clk.nfi_top_clk);
	if (nfc->clk.nfi_clk_sel)
		kfree(nfc->clk.nfi_clk_sel);
	if (nfc->clk.nfi_clk_parent)
		kfree(nfc->clk.nfi_clk_parent);

	return ret;
}

U_BOOT_DRIVER(mtk_nand_drv) = {
	.name = "mtk_nand",
	.id = UCLASS_MTD,
	.of_match = mtk_nfc_id_table,
	.ofdata_to_platdata = mtk_nand_ofdata_to_platdata,
	.priv_auto_alloc_size = sizeof(struct mtk_nfc),
};
