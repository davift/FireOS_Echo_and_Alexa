...
static void mtk_ecc_config(struct mtk_ecc *ecc, struct mtk_ecc_config *config)
{
	...
	if (config->op == ECC_ENCODE) {
		...
		if (ecc->ecc_data->mode_shift)
			reg = ecc_bit
			    | (config->mode << ecc->ecc_data->mode_shift);
		else
...
int mtk_ecc_wait_done(struct mtk_ecc *ecc, enum mtk_ecc_operation op)
{
	...
	if (op == ECC_DECODE) {
		ret = readl_poll_timeout_atomic(ECC_REG(ecc, ECC_DECDONE),
		     dec, dec & ecc->sectors, 1, ECC_TIMEOUT);
		/*read clear irq stat*/
		readw(ECC_REG(ecc, ECC_DECIRQ_STA));
		ecc->sectors = 0;
	} else {
...
#else
static const struct udevice_id mtk_ecc_dt_match[] = {
#endif
	{
...
struct mtk_ecc *mtk_ecc_nand_chip_init(void)
{
	struct udevice *dev;
	struct mtk_ecc *ecc;
	int ret;
	...
	ret = uclass_get_device_by_driver(UCLASS_MTD,
					  DM_GET_DRIVER(mtk_ecc_drv),
					  &dev);
	if (ret) {
		pr_err("Failed to get mtk_ecc_drv. ret:%d\n", ret);
		return NULL;
	}

	ecc = dev_get_priv(dev);
	clk_enable(ecc->clk);
...
static int mtk_ecc_ofdata_to_platdata(struct udevice *dev)
{
	struct mtk_ecc *ecc = dev_get_priv(dev);
	int ret = 0;

	ecc->regs = (void *)dev_read_addr(dev);
	ecc->ecc_data =
		(struct mtk_ecc_comp *)dev_get_driver_data(dev);

	pr_debug("mtk ecc regs:0x%x\n", ecc->regs);

	ecc->clk = kmalloc(sizeof(*ecc->clk), GFP_KERNEL);
	if (!ecc->clk)
		return -ENOMEM;

	ret = clk_get_by_name(dev, "nfiecc_clk", ecc->clk);
	if (IS_ERR(ecc->clk)) {
		dev_err(dev, "no clk for mtk ecc\n");
		ret = PTR_ERR(ecc->clk);
		kfree(ecc->clk);
		return ret;
	}
...
U_BOOT_DRIVER(mtk_ecc_drv) = {
	.name = "mtk_ecc",
	.id = UCLASS_MTD,
	.of_match = mtk_ecc_dt_match,
	.ofdata_to_platdata = mtk_ecc_ofdata_to_platdata,
