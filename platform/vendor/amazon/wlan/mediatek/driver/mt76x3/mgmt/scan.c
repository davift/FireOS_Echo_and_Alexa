				       ...
				       IN struct SW_RFB *prSwRfb)
{
	...
	if (prBssDesc) {
		...
		/* A bit tricky here. SwRfb has already been returned if mesh
		 * processes beacon or probe response successfully, so return
		 * WLAN_STATUS_PENDING to prevent return SwRfb again.
		 */
		if (!scanProcessBcnProbeRespMesh(prAdapter, prSwRfb))
...
#if CFG_ENABLE_WIFI_MESH
uint32_t scanProcessBcnProbeRespMesh(IN struct ADAPTER *prAdapter,
					IN struct SW_RFB *prSwRfb)
{
	uint32_t u4Status;
