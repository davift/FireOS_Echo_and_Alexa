...
static int btmtk_fifo_thread(void *ptr)
{
	...
	typedef bool (*selinux_set_enforce_func) (bool enforce);
	bool cur_enforce;
	selinux_set_enforce_func selinux_set_enforce_p = (selinux_set_enforce_func)
							 kallsyms_lookup_name("selinux_set_enforce");
	BTUSB_INFO("%s: selinux_set_enforce_p = %p", __func__, selinux_set_enforce_p);
#endif
...
#ifdef CONFIG_SECURITY_SELINUX_KERN_PERMISSIVE
		if (selinux_set_enforce_p)
			cur_enforce = selinux_set_enforce_p(false);
#endif
...
#ifdef CONFIG_SECURITY_SELINUX_KERN_PERMISSIVE
		if (selinux_set_enforce_p)
			selinux_set_enforce_p(cur_enforce);
#endif
